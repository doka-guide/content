...
Учитывайте, что `aria-label` перезаписывает другое текстовое содержимое элементов, в том числе из HTML-атрибутов. Одно исключение — другой атрибут `aria-labelledby`. Он всегда перезаписывает `aria-label`, поэтому не стоит задавать их для одного элемента.
