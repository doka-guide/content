...
Посмотреть на дерево доступности можно в инспекторах браузеров во вкладке с доступностью. Например, так выглядит [`<img>`](/html/img/) в виде объекта дерева в Firefox. У него роль `graphic` (в Chrome будет `img`), а имя — это описание картинки из атрибута `alt`.
...
Режим фокуса иногда называют режимом форм. В нём скринридеры работают с разными элементами форм — полями, чекбокcами, радиокнопками, выпадающими списками и обычными кнопками для отправки данных.
