...
`<button>` обязательно быть вложен внутрь [`<form>`](/html/form/). Только в этом случае данные формы отправляться при нажатии на <kbd>Enter<kbd>.
...
- Неявными, когда они вложен.
- Явными или прямыми, когда теги расположены рядом и связаны через `for` и `id`. В целом они равносильны, но с неявными могут быть проблемы с объявлением некоторыми скринридерами. [Пруф.](https://css-tricks.com/html-inputs-and-labels-a-love-story/#aa-how-to-pair-a-label-and-an-input)
