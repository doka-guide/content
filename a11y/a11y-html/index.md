...
Другая популярная ошибка—неправильный порядок заголовков — теги `<h1>`, `<h2>` и т.д. Часто выбор уровня заголовка основывается на его стиле, а не на логичном порядке. В результате, [42% сайтов имеют неправильную иерархию заголовков](https://almanac.httparchive.org/en/2022/accessibility#landmarks). Для доступности важно, чтобы на странице был один `<h1>` тег, а все остальные заголовки соблюдали правильный порядок (смотрите [правило доступности «Порядок заголовков»](https://dequeuniversity.com/rules/axe/4.4/heading-order)). Это облегчает навигацию по сайту, позволяя передвигаться от заголовка к заголовку с помощью горячих клавиш.
