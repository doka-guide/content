...
Программы и приложения для распознавания речи не всегда понимают речь человека. Например, из-за сильного акцента, заикания и других особенностей речи. Чтобы решить эту проблему, технологические компании объединили усилия в проекте [Speech Accessibility Project](https://speechaccessibilityproject.beckman.illinois.edu). Его цель — собрать как можно больше образцов речи людей с разными особенностями и научить акустические и языковые модели понимать их. То есть сделать их персонализированными.
...
Также можно называть элементы не по имени, а по порядковому номеру на странице. Если в меню не так много пунктов, можно сказать «Перейти два» и открыть вторую ссылку из четырёх.

Другая важная команда — «Показать сетку». _Сетка (grid)_ визуально делит интерфейс сайта на несколько пронумерованных ячеек. На картинке показано, как она примерно выглядит. Страница разделена на шесть ячеек с помощью вертикальных и горизонтальных линий. В углу каждой ячейки подписан её номер: 1, 2, 3, 4, 5 и 6.

![Сетка на сайте Американского клуба собаководства. Описание перед примером.](images/grid.png)

Пользователи голосового управления включают сетки, когда на странице есть кнопки с графикой или ссылки с одним и тем же названием. Предположим, на странице три кнопки с иконкой денежной купюры, шестерёнкой и корзиной. Пользователь хочет перейти в настройки, но команда «Нажать настройки» не работает. Чтобы решить проблему, сначала он использует команду «Показать сетку», потом назовёт номер ячейки с кнопкой и команду «Нажать».

Если сложно всё это представить, посмотрите [видео про навигацию по странице голосом](https://www.youtube.com/watch?v=yGOP2fOp7TA).
...
**Видимое имя**. Имена — это то, как называются ссылки, кнопки и другие интерактивные элементы. Они могут быть видимыми всем или только скринридерам. [`aria-label`](/a11y/aria-label/) и визуально скрытый CSS-стилями текст — два распространённых способа добавить невидимое имя.
...
```html
<!-- Так плохо 😩 -->
<button aria-label="Получить подарок">
  🎁
</button>
```
...
**Одно имя**. Вы могли встречать кнопки и ссылки с видимым именем и `aria-label` одновременно.
...
Технически, у кнопки есть видимое имя «Отменить». На самом деле оно «Удалить», так как `aria-label` перезаписывает видимое имя. Пользователь голосового управления не отменит действие командой «Нажать отменить, потому что для браузера и программы распознавания речи на странице нет такого элемента.
...
Контекст не поможет пользователям голосового управления, когда на странице одни «тут». Не всегда так просто посчитать количество ссылок, чтобы назвать её порядковый номер. Показ сетки тоже отнимает время.

Самый лучший выход из ситуации с неуникальными ссылками — называть их по-разному. Это не только упростит голосовое управление, но порадует пользователей скринридеров и поисковых роботов.
...
**Дизайн**. Не будем вдаваться в дизайнерские подробности. Просто напомним, что шрифт должен быть достаточно большим, интерактивные элементы — крупными, уровень контрастности — достаточным. В общем, интерфейс сайта и его содержимое должно быть видно.
