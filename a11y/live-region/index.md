...
[`aria-atomic`](/a11y/aria-atomic/) рассказывает скринридеру об объёме изменений, о которых надо рассказать. Это вся изменяющаяся область область или её часть. Например, атрибут полезен для таймеров.
...
```html
<ul id="friend-list" aria-live="polite" aria-relevant="additions removals">
...
`log` по умолчанию заданы `aria-live="polite"` и `aria-atomic="false"`, поэтому скринридеры не рассказывают сразу о последних изменениях. Когда нужно, чтобы пользователи узнали обо всех изменениях в логах, используйте вместе с ролью `aria-atomic="true"`.
...
`marquee` задан атрибут `aria-live="off"`, так что у роли нет приоритета объявления. Скринридеры не расскажут об изменениях в такой области, даже если пользователи сейчас не взаимодействуют со страницей.

```html
<ul role="marquee" aria-labelledby="label">
  <h3 id="label">Текущие курсы валют</h3>
