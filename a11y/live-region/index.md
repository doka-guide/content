...
Атрибут пригодится для сложных компонентов. Например, для фида [с ролью `feed`](/a11y/role-feed/) или ленты новостей как в социальных сетях. В этом случае можно задать на вермя загрузки `aria-busy="true"` всей области с новостями, а после удалить. Так скринридеры будут рассказывать об изменениях только тогда, когда пользователь нажмёт кнопку «Обновить ленту» и все элементы загрузятся.
