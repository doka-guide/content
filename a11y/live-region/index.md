...
Чаще всего достаточно задать нужному элементу [`aria-live`](/a11y/aria-live/) со значением `polite`. Такие объявления не будут врываться без проса и прерывать действия пользователей.

Другое значение `aria-live` — `assertive`. Его используют для срочных оповещений, которые выбивают дверь с ноги. Так что используйте `aria-live="assertive"` с осторожностью. Оно подойдёт для критически важных ошибок, которые прерывают действия пользователей. Например, когда прервалось интернет-соединение.

### ARIA-роли

HTML пока не покрывает все случаи, когда нужна «живая» область. В этих ситуация можно использовать специальные [ARIA-роли](/a11y/aria-roles/). Лучше всего задавать их семантически нейтральным [`<div>`](/html/div/) и [`<span>`](/html/span/), но бывают случаи, когда их можно использовать и для семантических тегов. Например, [`<p>`](/html/p/).

Задать элементу роль очень просто. Добавьте атрибут `role` с нужным значением.

```html
<div role="alert">
  <!-- Содержимое, которое как-то изменяется -->
</div>
```

Роль [`status`](/a11y/role-status/) уже есть у `<output>`. Так что лучше максимально часто использовать этот тег вместо того, чтобы явно задавать её тегам.
...
В демке собраны все ARIA-роли и атрибуты «живых» областей. Можете выбрать нужные сочетания и послушать, как скринридеры объявляют в этом случае появляющийся или удалающийся контент.
