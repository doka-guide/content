...
Главная цель «живой» области — указать вспомогательным технологиям как правильно обрабатывать изменения содержимого страницы. А выгода для пользователей — они точно узнают обо всех изменениях без лишних нажатий на клавиши.
...
В тег по умолчанию встроена роль [`progressbar`](/a11y/role-progressbar/). Она не считается ролью «живой» области, но скринридеры всё равно объявляют о прогрессе загрузки в процентах или при помощи особого звукового оповещения.
...
Посмотрим на прогресс-бар на практике. Для подписи используем `<label>`, с помощью JavaScript обновляем значение атрибута `val` и визуальный индикатор в виде числа в процентах и заливки, а ещё показываем подсказку об успешности действия под кнопкой «Очистить». Текст подсказки тоже делаем «живой» областью при помощи `aria-live` со значением `polite`.
...
Ещё один способ сделать часть страницы «живой» областью — специальные [ARIA-атрибуты](/a11y/aria-attrs/).

Чаще всего достаточно задать элементу [`aria-live`](/a11y/aria-live/) со значением `polite`. Такие объявления не будут врываться без проса и прерывать действия пользователей. Хорошие случаи использования — сообщение о том, что данные сохранились, подписка произошла, на введённую почту отправлено письмо и похожие ситуации.
...
Другое значение `aria-live` — `assertive`. Используйте его для срочных оповещений, о которых надо рассказать прямо сейчас. Так что применяйте `aria-live="assertive"` с осторожностью. Оно подойдёт для критически важных ошибок, которые прерывают действия пользователей. Например, когда пропало интернет-соединение и пользователь из-за этого не может отправить данные на сервер.
