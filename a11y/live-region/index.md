...
Не забывайте про атрибуты `max` и `val`. Без них скринридеры не смогут отследить прогресс загрузки, когда у неё есть чёткие начало и конец. Так что не забывайте изменять значение `val` с помощью JavaScript.
...
Прогресс-бар на практике. Для подписи используем `<label>`, с помощью JavaScript обновляем значение атрибута `val` и визуально, а ещё показываем подсказку об успешности выполнения действия под кнопкой «Очистить». Текст подсказки тоже сделали «живой» областью при помощи `aria-live` со значением `polite`.
...
<form>
  ...
  <span aria-live="polite">Мы получили вашу заявку. На обработку уйдёт 3 рабочих дня.</span>
</form>
```
...
<form>
  <!-- Поля, чекбоксы и всё, что душе угодно -->
  <button type="submit">Отправить</button>
  <span aria-live="assertive">У нас проблемы с сервером, так что заявка не отправилась. Нам нужно 5 часов, чтобы устранить неполадки.</span>
</form>
```
