...
Можете встретить вариант декоративной картинки, когда пропускают `alt`:

```html
<img role="presentation" src="img/decorative-pic-1.svg">
```

Хотя этот пример есть в спецификации, такой код не пройдёт валидацию. Дело в том, что у элементов с атрибутом `role` обязательно должны быть имена. В случае тега `<img>` имя элемента берётся из `alt`. Также подобные картинки пока плохо поддерживают и вспомогательные технологии.

<aside>

🐶 Рекомендуем всегда использовать возможности HTML. Просто оставьте `alt` пустым, если нужна декоративная картинка: `<img src="img/decorative-pic-1.svg" alt="">`.
