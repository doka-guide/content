...
Добавьте `role="presentation"` или `role="none"` к семантическому тегу — HTML-элементу со встроенной ролью. Например, [`<h1>`](/html/h1-h6/), [`<table>`](/html/tables/) или [`<li>`](/html/li/). Исключение — интерактивные элементы, с которыми взаимодействуют пользователи. Это могут быть кнопки [`<button>`](/html/button/), ссылки [`<a>`](/html/a/) или поля [`<input>`](/html/input/) и [`<textarea>`](/html/textarea/).
...
Если загляните в [дерево доступности](/a11y/a11y-tree/) в инструменте разработчика в Chrome, заметите, что у `<th>` внутри `<table role="presentation">` вычисляется [роль `generic`](/a11y/role-generic/). Это значит, что для браузеров и других программ это элемент без семантики.
