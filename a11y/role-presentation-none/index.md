...
Хотя этот пример из спецификации, такой код не пройдёт валидацию. Дело в том, что у элементов с атрибутом `role` обязательно должны быть имена. В случае тега `<img>` имя элемента берётся из `alt`. Также вспомогательные технологии пока плохо подобные картинки.
