...
После `@media` задайте в скобках условие `prefers-reduced-transparency` с одним из значений:
...
Если любите глассморфизм, то `prefers-reduced-transparency` тоже спешит на помощь. Давайте на этот раз используем будем показывать эффект «матового стекла» только тогда, когда не изменена настройка прозрачности.
...
В Chrome эмуляция `prefers-reduced-transparency` находится в дополнительной вкладке «Рендеринг» (Rendering) в инструменте разработчика. Чтобы до неё добраться, откройте список с дополнительными настройками (кнопка с тремя точками рядом с кнопкой закрытия), а потом разверните дополнительный список «Больше инструментов» (More tools).

Настройки прозрачности в операционных системах находятся разделах доступности и специальных возможностей. В некоторых системах прозрачность можно уменьшить, в других полностью отключить.
...
Прозрачный фон увеличивает когнитивную нагрузку на пользователей и уменьшет уровень читаемости текста. Этой настройкой пользуются:
