...
Также, чтобы объявления не были назойливыми, рекомендуют устанавливать интервалы между объявлениями. Это удобно делать [методом `setInterval()`](/js/setinterval/). Так можно добавлять и удалять `aria-live="true"` через нужные промежутки времени.
