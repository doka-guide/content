...
Тут снова ARIA спешит на помощь. Сделаем этот элемент изменяющейся областью с помощью `aria-live="polite"`. Благодаря атрибуту `aria-live` скринридер автоматически объявит сообщение об успешной очистке формы.
...
Состояния и свойства похожи друг на друга и задаются через атрибут `aria-*=""`. Их часто объединяют и называют _ARIA-атрибутами_. Основная разница между ними в том, что значения атрибутов свойств часто не так сильно изменяются, как у атрибутов состояний. К примеру, значение свойства [`aria-label`](/a11y/aria-label/) изменяется реже, чем состояния `aria-checked`.

ARIA-атрибуты тоже бывают разными. Какие-то атрибуты можно сочетать практически со всеми ролями элементов, например, `aria-live` и `aria-label`. Какие-то подходят только для интерактивных элементов, например, [`aria-hidden`](/a11y/aria-hidden/) и `aria-pressed`. Есть атрибуты, которые обязательно нужны для определённых ролей. К примеру, у элемента с ролью `combobox` обязательно должно быть состояние `aria-expanded`.
