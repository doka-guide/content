...
`aria-describedby` связывает с помощью одинакового ID два элемента — поле и подсказку из тега [`<span>`](/html/span/). Скринридер озвучит текст подсказки после лейбла и основной информации о поле.
...
- `aria-dropeffect` (свойство) — что произойдет, когда объект «бросят». Устарело.
...
- `aria-hidden` (состояние) — элемент скрыт из [дерева доступности](/html/screenreaders/#derevo-dostupnosti) и недоступен для вспомогательных технологий.
...
- `aria-labelledby` (свойство) — один элемент или несколько содержат лейбл для другого элемента.
- `aria-live` (свойство) — указание на то, что элемент изменится, тип и важность этого изменения.
- `aria-owns` (свойство) — один элемент или несколько связаны с другими как родители или дети визуально, функционально или контекстно, но это не отражено в [DOM](/js/dom/).
- `aria-relevant` (свойство) — вспомогательная технология **может** объявить об изменениях в «живой» области после всех изменений в дереве доступности.
...
- `aria-description` (свойство) — дополнительное подробное описание элемента.
...
- `aria-disabled` (состояние) — элемент есть на странице, но с ним нельзя взаимодействовать. Эквивалент в HTML — [`disabled`](/html/disabled/).
- `aria-errormessage` (свойство) — элемент содержит сообщение об ошибке.
- `aria-expanded` (состояние) — элемент раскрыт или свёрнут.
- `aria-haspopup` (свойство) — элемент раскрывает поп-ап.
- `aria-hidden` (состояние) — элемент скрыт из [дерева доступности](/html/screenreaders/#derevo-dostupnosti) и недоступен для вспомогательных технологий.
...
- `aria-multiselectable` (свойство) — можно выбрать нескольких элементов.
- `aria-orientation` (свойство) — ориентация элемента на странице.
- `aria-placeholder` (свойство) — подсказка по заполнению поля. Эквивалент в HTML — [`placeholder`](/html/placeholder/).
...
- `aria-atomic` (свойство) — о каком объёме изменений сообщит вспомогательная технология на основе значения `aria-relevant`. Это вся «живая» область или её часть.
- `aria-busy` (состояние) — вспомогательная технология **должна** подождать пока элемент окончательно изменятся.
- `aria-live` (свойство) — указание на то, что элемент изменится, тип и важность этого изменения.
- `aria-relevant` (свойство) — вспомогательная технология **может** объявить об изменениях в «живой» области после всех изменений в [дереве доступности](/html/screenreaders/#derevo-dostupnosti).
...
- `aria-dropeffect` (свойство) — что произойдет, когда объект «бросят».
...
💡 Если изменяете роль семантического тега, то вместе с ролью перезапишутся его свойства и состояния.
