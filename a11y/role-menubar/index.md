...
Только у одного пункта из `menubar` может быть `tabindex="0"` — у первого элемента до того, как на строке сделали фокус, и у пункта, на котором сейчас фокус. Остальные элементы должны быть с `tabindex="-1"`, пока их не выбрали. В том числе отрицательный `tabindex` должен быть у контейнера с подменю.
...
В классической, «настоящей» строке меню размещают кнопки, чекбоксы, радиокнопки и другие интерактивные элементы, которые раскрывают подменю, изменяют внешний вид и содержимое других элементов на странице. В этом случае и пригодится роль `menubar`.
