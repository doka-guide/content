...
- `mixed` — некоторые, но не все, дочерние элементы выбраны (доступен только для `role="checkbox"` и `role="menuitemcheckbox"`).
...
Если вам нужен компонент вроде <input type="checkbox" switch>, но у вас пользователи используют браузеры отличные от Safari или вам нужен компонент меню с возможностью выбора нескольких позиций внутри, то вам будет полезен `aria-checked` для корректной обработки состояния скринридерами.
