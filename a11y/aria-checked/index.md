...
Атрибут нужен для случаев, когда вы делаете свой чекбокс, переключатель или пункт меню не с помощью стандартного [`<input>`](/html/input/), а самостоятельно — например, на основе [`<div>`](/html/div/) или [`<span>`](/html/span/).

## Пример

Использование `<span role="checkbox">` и HTML-атрибута [`<label>`](/html/label/).
...
Использование JavaScript-свойства у [DOM-элемента](/js/dom/).
...
Если `aria-checked` отсутствует, пользователь [скринридера](/a11y/screenreaders/) не сможет узнать, что элемент можно выбрать или он уже выбран.
...
Скринридер сообщает, выбран ли чекбокс с [`role="checkbox"`](/a11y/role-checkbox/) или нет, находится ли в смешанном состоянии (`mixed`). Например:
...
Озвучка похожа на объявление чекбокса, но с акцентом на включение или выключение элемента с [`role="switch"`](/a11y/role-switch/):
...
Озвучка происходит в контексте группы радиокнопок с [`role="radio"`](/a11y/role-radio/):
...
Если опция c ролью [`option`](/a11y/role-option/) находится внутри [`listbox`](/a11y/role-listbox/), скринридер озвучит, выбрана ли она:
...
Если опция вложена внутрь [`group`](/a11y/role-group/), скринридер озвучит её как часть группы:
...
☝️ Если у элемента нет подходящей роли, скринридер проигнорирует значение `aria-checked`.
...
<iframe title="Кастомный переключатель с поддержкой aria-checked" src="demos/switch-example/" height="300"></iframe>
...
<iframe title="Меню с множественным выбором и промежуточным состоянием для групповых чекбоксов" src="demos/menu-item-checkbox-example/" height="500"></iframe>
...
💡 [Атрибут `switch`](https://webkit.org/blog/15054/an-html-switch-control/) для `<input>` работает только в Safari. Для кроссбраузерного переключателя можно использовать кастомный компонент с `aria-checked`.
