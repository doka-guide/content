...
`aria-checked` используется для собственных интерактивных элементов — например, чекбоксов, переключателей или пунктов меню — чтобы они были доступны для всех пользователей, включая тех, кто пользуется скринридерами. Этот атрибут позволяет вспомогательным технологиям узнать, выбран элемент или нет.
...
Если создаётся свой компонент вроде чекбокса или переключателя (switch) — например, для особого дизайна или поведения, — и не используется стандартный `<input type="checkbox">`, лучше добавить к нему `aria-checked`. Это поможет скринридерам правильно озвучивать состояние элемента.
...
Если нужно реализовать выпадающее меню, в котором можно выбрать сразу несколько пунктов и поддерживать промежуточное состояние (например, выбраны не все опции), лучше использовать `aria-checked` со значениями `true`, `false` и `mixed`.
...
💡 Атрибут `type="checkbox" switch` работает только в Safari. Для кроссбраузерного переключателя можно использовать кастомный компонент с `aria-checked`.
