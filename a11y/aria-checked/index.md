...
Если вам нужен компонент вроде `<input type="checkbox" switch>`, но у вас пользователи используют браузеры отличные от Safari или вам нужен компонент выпадающего меню с возможностью выбора нескольких позиций внутри, то вам будет полезен `aria-checked` для корректной обработки состояния скринридерами.
