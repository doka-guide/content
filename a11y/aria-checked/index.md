...
`aria-checked` пригодится, если вы делаете собственные интерактивные компоненты, которые должны быть доступны для всех пользователей, включая тех, кто использует скринридеры и другие вспомогательные технологии.

Вот когда стоит использовать этот атрибут:
...
_Демо: Кастомный переключатель с поддержкой aria-checked и доступности._
...
_Демо: Меню с множественным выбором и промежуточным состоянием для групповых чекбоксов._

> Для стандартных чекбоксов и радиокнопок всегда используйте обычный `<input type="checkbox">` или `<input type="radio">` — так браузер и вспомогательные технологии сами позаботятся о доступности и поведении.
