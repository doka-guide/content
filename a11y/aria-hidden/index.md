...
💡Лучше добавлять `aria-hidden` к тегу с помощью [JavaScript](/js/), а не задавать его сразу в разметке со значением `true`. Если скрипт не сработает, элемент останется виден для скринридеров.
