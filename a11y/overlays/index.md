...
После выхода WCAG 1.0 появились продукты, которые обещали автоматическое соответствие рекомендациям руководств.

Ранние продукты представляли собой дополнительные элементы управления интерфейсом, которые зачитывали содержимое страницы вслух. Сегодня их сменили оверлеи, которые работают как вспомогательные технологии и дополнительные настройки, встроенные прямо на страницу сайта.

Оверлеи позволяют увеличивать семейство и размер шрифта, изменять цвет и выравнивание текста, настраивать контрастность цветов или межбуквенное пространство (трекинг). Примеры дополнительных функций оверлеев — увеличение элементов интерфейса, режим симуляции дальтонизма, выделение отдельных строк текста для людей с дислексией и синдромом дефицита внимания, добавление индикатора фокуса, если он выключен в стилях. Также оверлеи дают возможность изменить внешний вид некоторых типов содержимого на странице - выделяют фокус у интерактивных элементов, выделяют отдельные строки для чтения и тд.

![Примеры возможностей оверлеев](images/overlays.png)

Несколько примеров оверлеев: AccessiBe, AccessiWay, AudioEye и UserWay.

Производители оверлеев обещают автоматически устранить проблемы с качеством дизайна и кода интерфейса, а также привести сайты в соответствие с требованиями законов о веб-доступности. Дополнительно компании утверждают, что оверлеи стали лучше работать благодаря машинному обучению. Попробуем разобраться, так ли это на самом деле.
...
Ещё использование оверлеев плохо влияет на производительность сайта, так как это дополнительный код, требующий дополнительного времени на загрузку. Пользовательский опыт от этого ухудшается, как и SEO-метрики. Более медленная скорость загрузки страницы и худшие показатели обработки страницы специальными поисковыми программами - краулерами - просаживают выдачу сайта в поисковой выдаче.
...
![Отдельная версия портала для слабовидящих](images/page-version-for-weak-vision.png)

На первом скриншоте обычная версия сайта, с цветными изображениями и тд. На втором скриншоте отдельная версия для слабовидящих, на которой убрали изображения, а также увеличили шрифт и сделали чёрно-белую схему. В отдельной версии можно включить или отключить отрисовку изображений, поменять цветовую схему сайта, величину шрифта и межбуквенное пространство, выбрать шрифт с засечками или без, изменить величину межстрокового интервала.

### Достоинства и недостатки

Единственный плюс оверлеев — возможность настроить шрифты и цвета под себя. Шрифты можно выбирать с засечками или без, разных размеров, с разными интервалами и отступами. Кроме нескольких стандартных цветовых схем есть дополнительные темы страницы.
