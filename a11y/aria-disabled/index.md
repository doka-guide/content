...
Для элементов [`<a>`](/html/a/), которые не содержат атрибут `href`, `aria-disabled="true"` может быть использован для указания, что ссылка в текущем состоянии нефункциональна. Такой трюк используют для хлебных крошек, когда надо вроде и дать понять пользователям клавиатуры, где они находятся на сайте сейчас, и при этом не оставлять ссылку на текущую страницу активной. Например:

```html
<a role="link" aria-disabled="true">Товары для котов</a>
```
