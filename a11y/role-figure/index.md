...
Для [скринридеров](/a11y/screenreaders/) и других вспомогательных технологий `figure` означает, что это дополнительное содержимое страницы. Оно не важно для понимания другого контента, так что его можно удалить или пропустить без потери смысла текста.
...
Задайте любому тегу `role="figure"`. Лучше всего подойдут семантически нейтральные [`<div>`](/html/div/) или [`<span>`](/html/span/). В большинстве случаев используйте `<figure>`. Явная роль `figure` пригодится, когда не можете использовать HTML-тег или хотите обеспечить обратную совместимость со старыми браузерами и вспомогательными технологиями.

У контейнера с ролью `figure` может быть подпись или дополнительное описание. Если используете возможности HTML, то просто вложите внутрь `<figure>` другой тег `<figcaption>` с текстом. Это создаст связь между контейнером и подписью, о которой узнают все современные вспомогательные технологии. Для кастомного элемента `figure` нужно приложить больше усилий.

<aside>

🧬 Подписи и описания к картинкам внутри `figure` должны быть уникальными и не повторять текст, который уже есть на странице. Подробнее об этом узнаете статьи «[Как описывать картинки](/a11y/how-to-describe-pictures/)».

</aside>

Есть два способа добавить общую подпись к кастомному элементу с явной ролью:
...
Дополнительное расширенное описание к `figure` также добавляют с помощью ARIA-атрибутов:

- [`aria-descrtion`](/a11y/aria-description/) для описания, которое доступно только скринридерам;
...
Если оставить `figure` без подписи или описания, скринридеры никак не расскажут об элементе и его содержимом. Это похоже на поведение [тега `<img>`](/html/img/) с пустым атрибутом `alt`.
...
Не все браузеры и скринридеры правильно рассказывают о теге `<figure>`, особенно когда в него вложены картинки с пустыми атрибутами `alt`. Лучше всего с этим справляется скринридер JAWS в Chrome.

Если поддерживаете большой зоопарк технологий, задавайте тегу `<figure>` явную роль `figure` и продублируйте подпись из `<figcaption>` в `aria-label`.
