...
Авторы — это все, кто создаёт, проверяет и улучшает интерфейсы и их содержимое. Например, разработчики, дизайнеры, контент-менеджеры, тестировщики и др.
...
- [WCAG 2.2](https://www.w3.org/TR/WCAG22/). Черновик кандидата в рекомендации, станут рекомендацией в первой половине 2023.
- [WCAG 3](https://www.w3.org/TR/wcag-3.0/). Рабочий черновик, станут рекомендацией не раньше 2025.

Подробнее об истории и основных отличиях между версиями узнаете из [WCAG Version History](https://accessibleweb.com/wcag/wcag-version-history/).
...
W3C советует ориентироваться на последнюю версию WCAG со статусом рекомендации.

## Как устроены WCAG 2

WCAG 2 состоит из нескольких частей:

- базовые принципы (principles);
- сами руководства (guidelines);
- критерии успешности (success criteria, коротко SC) с разными уровнями соответствия (conformance levels);
- введение (introduction), подробный разбор критериев (undestarding criteria), техники (techniques), ошибки (failures), глоссарий (glossary) и др.
...
Части WCAG 2 бывают нормативными (normative) и ненормативными (not-normative). Об этом предупреждают в самом тексте руководств.

Нормативный означает, что эту часть обязательно соблюдать. Во WCAG 2 это принципы, руководства и критерии.

Противоположность нормативному — ненормативное. Другие названия — информативное (informative), не требующееся (not required) и описательное (descriptive).
...
- чем руководствовались авторы WCAG при создании критерия (intending);
- другая дополнительная информация с отсылками к исследованиям, другим критериям и пр.;
- каким пользователям помогает (benefits);
- примеры элементов, которые соответствуют или не соответствуют критерию (examples);
- дополнительные источники информации (related resources);
- техники (techniques);
- ошибки (failures);
- [правила тестирования](https://www.w3.org/WAI/WCAG21/Understanding/understanding-act-rules.html) (test rules);
- ключевые термины из текста критерия (key terms).
...
- Вместо критериев успешности появятся результаты (outcomes), изменится их дополнительное описание, внутри результатов будут критические ошибки (critical errors).
- В результатах также появятся функциональные категории (functional categories) — на каких пользователей с инвалидностью влияет результат.
- Техники заменят на методы (methods).
...
Подробнее обо всех изменениях во WCAG 3 узнаете из «[Что но­во­го в WCAG 3.0](https://web-standards.ru/articles/wcag3-changes/)».
...
W3C собрало [список законов о веб-доступности в разных странах и не только](https://www.w3.org/WAI/policies/).
