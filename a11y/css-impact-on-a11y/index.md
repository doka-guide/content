...
description: "Разбираемся, какие CSS-свойства влияют на доступность сайтов для пользователей скринридеров и других вспомогательных технологий."
authors:
  - IPogorelova
keywords:
  - доступность
...
Мы привыкли, что в структуре сайта [CSS](/css/) отвечает только за визуальное представление, а всё касающееся контента задаётся в [HTML](/html/).
...
Также в тексте статьи много демок. Можете протестировать их сами, чтобы не верить мне на слово. Для этого нужно установить или включить скринридер — в [статье про скринридеры](/a11y/screenreaders/#vidy-skrinriderov) есть список существующих программ для каждой ОС. Инструкции по скачиванию и подключению обычно есть на сайтах скринридеров.

Наконец, все демки я протестировала со скринридером NVDA на Windows в Google Chrome. Если тестируете в другом скринридере, его поведение может немного отличаться от описанного в статье.
...
Допустим, в этом случае нам не важно знать, сколько пунктов в списке, поэтому мы используем ненумерованный список — [`<ul>`](/html/ul/).
...
- У списка с [`list-style: none`](/css/list-style/) количество элементов произносится, но слово «маркер» опускается. Элементы списка просто зачитываются подряд:
  > Список из четырёх элементов. Апельсины. Хлеб…

В случае со скринридером VoiceOver свойство `list-style: none` может повлечь ещё большую путаницу. В Safari список с `list-style: none` вовсе не озвучивается как список. Мы не услышим ни количество элементов, ни слово «маркер».
...
Список собрали — теперь самое время отправляться за покупками. Допустим, в нашем магазине карточки товаров выглядят как кнопки с эмодзи и текстами.
...
Если заглянуть в код демки, мы увидим, что элементы в HTML стоят ровно в том порядке, в каком их читает скринридер. Но при этом, каждому из них задано свойство [`order`](/css/order/). Как раз оно и определяет их визуальный порядок.
...
Но верстать таблицы мало кто любит. А что, если сверстать то же самое, но с помощью [`display: grid`](/css/grid-guide/)?

Посмотрим на второй вариант чека. Визуально всё выглядит почти в точности так же, как и обычная таблица. Но внутри теперь не семантический тег `<table>`, а обычные [`<div>`](/html/div/) и [`<p>`](/html/p/).
...
Продолжаем экспериментировать и теперь навесим свойство `display: grid` на настоящую семантическую таблицу. Зачем? Во-первых, почему бы и нет. А во-вторых, чтобы проверить утверждение, которое встречалось мне в нескольких статьях. В этом случае свойство `display: grid` должно сломать семантику таблицы.
...
На удивление, NVDA справился с такой путаницей отлично. Он прочитал элемент как настоящую таблицу и озвучил все строки и столбцы.
...
В Доке есть отдельная [статья «Как скрыть содержимое от скринридеров»](/a11y/content-hidden/). В ней подробно описаны все способы скрытия и показа содержимого. Например, только визуально, только для скринридеров или всё вместе.
...
Кстати, в Доке уже есть [материал о `prefers-reduced-motion`](https://doka.guide/a11y/prefers-reduced-motion/).
...
- Контент псевдоэлементов `::before` и `::after` зачитывается скринридером. Если внутри свойства `content` ссылка (например, на картинку), он её не прочитает.
- Контент псевдоэлемента `::marker` невидим для скринридера. Но список с таким псевдоэлементом всё равно не читается как список, если ему задан `list-style: none`.
...
- `display: table` не сделает для скринридера таблицу из обычных `<div>`-контейнеров. А `display: grid` у `<table>` вовсе может сломать всю семантику для некоторых скринридеров.
...
Сложно заранее представить, как скринридер озвучит интерфейс. Даже если ты прочитал кучу статей о вспомогательных технологиях и, кажется, знаешь уже все подводные камни.

Единственный способ проверить как _действительно_ звучит содержимое — сесть и протестировать его с любым скринридером вручную. Это всегда очень полезно, а часто ещё и довольно весело.
