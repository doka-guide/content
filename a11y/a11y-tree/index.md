...
Дерево доступности создаётся параллельно с [DOM-деревом](/js/dom/#iz-chego-sostoit-dom). Браузеры ждут, пока страница окончательно отрисуется, а потом решают, что забрать в дерево доступности. Кроме информации о семантических тегах, в них, к примеру, попадает содержимое псекдокласов [`::before`](/css/before/) и [`::after`](/css/after/) и дополнительная разметка для вспомогательных технологий.
...
В отдельных узлах найдёте подробную ифнормацию про доступный объект: его ARIA-атрибуты и вычисляемые свойства, к примеру, имя и роль.

![Вкладка «Доступность» в Chrome. Описание перед скриншотом.](images/chrome-accessibility-tab.png)

Давайте теперь посмотрим на реальный доступный объект. Возьмём поле [`<input type="email">`](/html/input/) для почты из попапа с подпиской на рассылку Доки.
