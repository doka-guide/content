...
1. Код в тексте (названия тегов, свойств, консольных команд и т. п.) выделяйте при помощи обратных апострофов (бэктиков):
    ...
    Цифровые значения обычно не выделяются — цифры и так достаточно заметны в поле текста. Единицы измерения отделяются от числа пробелом, их можно записывать как в сокращеённом виде (16 px), так и в развернутом (16 пикселей).
