...
Чтобы создать новый материал, сделайте несколько простых шагов:

1. Создайте Fork от репозитория и клонируйте его себе на компьютер.
1. Откройте Fork в терминале и введите
    ```bash
    ./new.sh
    ```
1. Ответьте на несколько вопросов, скрипт создать нужные папки и файлы, плюс переключит на новую ветку.
1. Начинайте писать.
