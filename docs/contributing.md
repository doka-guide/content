...
1. Запустите проверку буквы «ё» командой `node .github/scripts/check-yo.mjs css/ваша-статья/index.md`. Она же умеет чинить найденное — с ключом `--fix`. Подробности в [инструкции по проверке орфографии](speller.md).
