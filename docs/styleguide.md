...
Если вам нужна картинка с подписью, то добавьте подпись текстом на следующей строке. Вот так:
...
Это не подпись к картинке
```

и платформа превратит обернёт картинку в `<figure>` с текстом в виде `<figcaption>`.
