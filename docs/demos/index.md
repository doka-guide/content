...
В демке может понадобиться изобразить поведение для ссылок. Пожалуйста, не добавляйте в адрес ссылки несуществующие страницы или якоря внутри Доки. Это ломает проверку на корректность ссылок. Воспользуйтесь следующими рекомендациями.

Переопределите в CSS стандартную стилизацию для ссылок, которую обычно подставляет браузер в `a:-webkit-any-link`, потому что в результате рекомендации тег `<a>` перестанет быть ссылкой:
