...
Теперь поменяем состояние радио-кнопок переключателей в соответствии с тем, что записано в хранилище. Ищем кнопку с классом `snow-toggle__control` и значением атрибута `value`, совпадающим с текущим значением из хранилища и задаём ей атрибут `checked` со значением `true`. Технически значение этому атрибуту не нужно, но в JS иначе не получится. Предварительно при помощи [`if`](/js/if-else/) проверим, что значение переменной `currentStorage` не [`undefined`](/js/undefined/):
