...
В начале 2000-х было очень можно добавлять на сайты падающий снег. Тогда это была нетривиальная задача, требовала много усилий разработчика и сильно нагружала и без того слабые компьютеры пользователей.
...
Напишем оптимизированный и производительный падающий снег для сайта.

## Разметка

Создадим разметку для снежинок. Понадобиться родительский контейнер с классом `snow`. Внутри будут блоки с классом `snow__flake` и символом `﹡` в контенте.
...
Для начала сделаем блок `.snow` флекс-контейнером чтобы все вложенные блоки встали в один ряд. Используем свойство [`justify-content`](/css/justify-content) чтобы все снежинки распределились на всю ширину экрана:
...
Поднимем снежинки чуть выше края экрана. Чтобы не мазолили глаза пользователю пока не начнут своё движение:
...
Элемент, к которому мы применим эту анимацию будет при помощи [трансформации](/css/transform) перемещаться по вертикали от верхнего края до 100 единиц [высоты экрана](/css/vw-vh/) и ещё плюс 1.5 em, чтобы снежинки скрывались за нижний край страницы.
...
Обратите внимание, что CSS-свойство [`animation-duration`](/css/animation-duration) пишется не через тире. Это особенность [работы со стилями](/js/element-style/) в JavaScript.

Теперь все снежинки падают с разной скоростью, но начинают своё движение в одно время. Добавим рандомности и в этом. Каждой снежинке пропишем свойство [`animation-delay`](/css/animation-delay/) со значением от -1 до числа, равного количеству блоков со снежинками, делённое на два. Тут можно смело экспериментировать и подбирать разные значение.
...
Используем в качестве переключаетелей радио-кнопки. Обязательно обернём их [`<label>`](/html/label/) чтобы кнопки были доступными для пользователей со [скринридерами](/html/screenreaders/).
...
Чтобы радио-кнопки знали о состоянии друг друга важно задать им одинаковое значение свойства `name`. Значение свойства `value` тоже важно. Мы будем манипулировать им при помощи JS. По умолчанию снег будет идти, поэтому у первой радиокнопки используем атрибут `checked`.
...
Первам делом объявим константу `snowToggle`, в которую запишем найденый в HTML элемент `.snow-toggle`.
...
Ниже добавляем обработчик события клика мыши. При клике на любой элемент, вложенный в блок `.snow-toggle`, будет вызываться функция `changeSnowAnimation`. Внутрь ункции передаём значение атрибута `value` того элемента, по которому произошёл клик.
...
Теперь поменяем состояние радио-кнопок переключателей в соостветствии с тем, что записано в хранилище. Ищем кнопку с классом `snow-toggle__control` и значением атрибута `value`, совпадающим с текущим значением из хранилища и задаём ей атрибут `checked` со значением `true`. Технически значение этому атрибуту не нужно, но в JS иначе не получится. Предварительно при помощи [`if`](/js/if-else/) проверим, что значение переменной `currentStorage` не [`undefined`](/js/undefined/):
...
Последний шаг. Добавим обработчик события на изменение записей в локальном хранилище. Как только запись меняется, так сразу вызывается функция изменения анимации, передавая в неё текущее значение из хранилища:
