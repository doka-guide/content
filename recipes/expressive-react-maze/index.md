...
Перенесём наработки из предыдущей статьи в новый проект. Создадим файл `src/components/Rectangle.tsx` и добавим в него следующий код:
...
Теперь подробнее взглянем на аргументы установщика состояния `setWalls`. Есть два способа использовать эту функцию: передавать в качестве аргумента новое состояние (мы так не делаем) или передавать функцию, которая принимает текущее состояние и возвращает новое, которое будет установлено в качестве текущего.
