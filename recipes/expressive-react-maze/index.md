...
> Здесь и далее предполагаем, что вы используете Mac, Linux или Windows с WSL.
...
☝️ Префикc генерируется из хеша содержимого файла. Вы можете добавить специальные правила в файл настройки _vite.config.js_, чтобы изменить способ генерации префикса.
...
1. Не пытайтесь генерировать уникальные ключи на каждом рендере (например, используя `Math.random()`). Приблизительно можно представить ситуацию с ключами так: React привязывает ключи к элементам. Если у элемента изменился ключ, React считает что это новый элемент.
1. Ипользуйте индексы в качестве ключей только если вы уверены что в вашем списке не будет изменений.
