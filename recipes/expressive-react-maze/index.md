...
Обратите внимание как добавляются стили. Мы описываем стили с файле `Maze.module.css` и импортируем их в компонент. Файл стилей можно называть как угодно, важно чтобы он заканчивался на `.module.css`. Когда vite видит такое окончание файла, он будет добавлять к каждому классу уникальных префикс, так что стили не будут пересекаться.
...
Теперь подробнее взглянем на агрумент хука `useState`. Есть два способа использовать этот хук: передавать в качестве аргумента новое состояние (мы так не делаем) или передавать функцию. Функция принимает текущее состояние и возвращает новое, которое будет установлено в качестве текущего.
