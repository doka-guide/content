...
Хук [`useState`](https://react.dev/reference/react/useState) позволяет определить переменную-состояние и даёт функцию для изменения этого состояния. React гарантирует что в после изменения состояния компонент перерендерится, но не гарантирует что это произойдёт мгновенно.
...
Мы добавили собачку и завернули все элементы во фрагмент. Фрагмент не генерирует DOM-элемент, это удобный способ объединить несколько компонентов в один.
