...
💬 Ещё атрибут `method` может иметь значение `dialog`. В таком случае дефолтное поведение - попробовать закрыть «[модальное окно](/html/dialog/)». Страница перезагружаться не будет.
