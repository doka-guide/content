...
Для обработки файла используется контейнер с идентификатором `upload-zone`. Внутри этого блока помещается форма [`<form>`](/html/form/) с элементами, которые обеспечивают информационное взаимодействие с пользователем. Например, с помощью изменения цвета фона этой области при перестаскивании элемента.
