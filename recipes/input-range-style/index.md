---
title: "Стилизация элемента `<input type="range"` (Слайдер)"
...
Элемент `<input type="range"` представляет собой интерактивный компонент пользовательского интерфейса, позволяющий выбирать числовые значение из заданного диапазона. Стандартный вид этого элемента, предоставляемый браузерами, часто не соответствует дизайну и функционалу веб-приложений, что создает необходимость в его кастомизации. В данной статье мы сверстаем `<input type="range"`, рассмотрим особенности работы с несколькими ползунками, обсудим вопросы кроссбраузерной совместимости и уделим особое внимание аспектам доступности учитывая рекомендации WCAG.

"Демка со всеми примерами input range в статье"

## Краткий обзор input range

Сначала разберем наш слайдер по деталям.

"Демка, где подписаны элементы input range"

Глоссарий:

- трэк - полоска во всю длинну слайдера;
- прогресс бар - полоска заполнения слайдера;
- ползунок - элемент, которым пользователь меняет значение слайдера;
- шкала значений - подписи значений слайдера рядом с трэком.
...
В данный момент стилизовать ползунок можно только через псевдоэлементы с префиксов (В webkit и firefox они отличаются).
Добавляем стили ползунку. Лучше раздельно для браузеров, т.к иначе могут возникуть проблемы с отображением.
...
В Webkit и Chrome пока нет поддержки стилизации прогресс бара. Для обхода этих ограничений можно воспользоваться css-трюком. (Для firefox лучше использовать предыдущий метод, т.к могут быть артифакты)
...
Для удобной кроссбраузерной и читабельной версти используем отдельные тэги `<div>` для трэка и прогресс бара. А также css переменные для изменения заполненности прогресс бара с помощью javascript.
...
А также атрибут `oninput="handleInputRange()"`. Здесь указывается слушатель события "oninput", то есть событие изменения значения input. И на каждое изменение будет вызываться функция `handleInputRange()`
...
Добавляем стили для обертки нашего слайдера. Инициализируем кастомные переменные для размера и положения элментов слайдера. Эти переменные пригодяться в дальнейшем для стилизации. Указываем `relative` позиционирование и грид для последовательного расположения элементов.
...
Чтобы не использовать подобные ухищрения можно указать тип переменной через css правило `@property`. ВОЗМОЖНО НУЖНО ПЕРЕПИСАТЬ НА НЕГО ДЕМКИ, но мне немного лень =P
...
<div class="range" style="--value: 80;">
  ...
  <div class="progress"></div>
  <div class="thumbs">
    <input class="range-input" id="tailmetr" type="range" min="0" max="100" value="20" step="1" oninput="handleInputRange1()" />
...
Добавляем переменные расположения и смещения для тэга `<output>` с текущим значением. Смещение нужно, чтобы число распологалось точно над ползунком, так как оно увеличивается с однозначного 0, до трёхзначных 100.
...
Часто требуется сделать слайдер с двумя ползунками, чтобы указать диапозон чего-то. Например диапозон цен в фильтре поиска товаров.
...
Обратим внимание, что помимо добавления второго тэга `<input>`, также был добавлени тэг `<output>` для текущего значения второго элемента. А в обертку нашего слайдера добавлена инициализация сразу двух css переменных для обоих ползунков `--value-1` и `--value-2`. Также были изменены функции указанные в атрибуте `oninput`. Для каждого ползунка будет своя функция обработки `handleInputRange1()` и `handleInputRange2()`.

Напишим новые функции для обработки изменения значения ползунков.
...
function handleInputRange2() {
  ...
  // Если значение текущего(правого) ползунка равно 0(минимальному значению), то он должен находиться выше левого, чтобы пользователь мог изменить его значения, иначи ползунки будут блокировать друг друга. (css deadlock, почти как в потоках)))
...
Ещё одним частым случаем является реализации слайдера с разным колличеством пользунков.
