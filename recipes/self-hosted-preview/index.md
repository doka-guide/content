...
📚 В этой статье используем сертификаты Let’s Encrypt для нашего сайта. Если не знаете, как с ними работать, прочитайте рецепт «[Установка сертификатов Let’s Encrypt](/recipes/lets-encrypt-nginx/)». Если хотите узнать о сертификатах в общем, прочитайте статью «[SSL-сертификаты](/recipes/lets-encrypt-nginx/)».
