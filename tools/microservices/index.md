...
## Когда микросервисы отлично походят
...
Результат можно посмотреть в браузере, введя адрес _http://localhost:8000/image.jpg_, поскольку картинка сохраняется в ту же директорию, где находится микросервис. Имя файла картинки сохраняется, меняется только расширение на `.jpg`.

Набор подобных микросервисов может положительно сказаться на масштабируемости всего приложения или сервиса. Поддерживать и развивать микросервисы проще, можно применять разные архитектуры и стек технологий, для оптимального с точек зрения разработки и бизнеса решения.
