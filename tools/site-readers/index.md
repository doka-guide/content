...
Для этого необходимо разобраться с тем, как различные сервисы, службы, приложения «читают» ваш сайт, как подготовить его для них и как сделать это правильно.
...
Попасть в индекс — это только первый шаг. Рейтинг вашей страницы в интернете всё время меняется. На положение страницы в рейтинге оказывают влияние не только количество страниц, которые ссылаются на вашу страницу, но и контент. Большое значение имеет поведение пользователя на страницы. Чем больше пользователь взаимодействует со страницей, тем лучше. Поэтому важно уделять внимание содержимому, дизайну, пользовательскому опыту.

Веб-краулер регулярно проходит по существующим адресам в базе и проверяет наличие страницы или ответ сервера при запросе страницы. Если статус страницы корректен, но содержимое изменилось, то бот производит индексацию заново обновляет информацию о странице в базе данных. Период проверок меняется, как правило, в зависимости от популярности страницы у пользователей.
...
Для людей с нарушениями зрения внешний вид вашего сайта может не иметь никакого значения. Незрячие люди вообще его не увидят, дальтонизм может помещать увидеть какие-то элементы или блоки, которые не обладают достаточной контрастностью, корректным с точки зрения другого цветового восприятия цветом и оттенком. Есть люди, которые не могут использовать мышь или любой другой способ указатель (перо, стилус или даже свой палец).

Очень важно не только как выглядит ваш сайт, но и как он «звучит». Существуют специальные расширения для браузера (скринридеры), которые позволяют прочитать в слух контент вашего сайта. Это помогает услышать ваш сайт — выход из положения для многих.
...
- [ ] Принять необходимость перечисленных ниже пунктов.
- [ ] Сделать сайт адаптивным.
- [ ] Сделать сайт контрастным.
- [ ] Сформировать структуру заголовков.
- [ ] Сделать ссылки доступными.
- [ ] Сделать контентные картинки доступными.
- [ ] Сделать аудио и видео доступными.
- [ ] Сделать формы доступными (если есть формы).
- [ ] Сделать таблицы доступными (если есть таблицы).
- [ ] Сделать заголовки страниц (тег `<title>`).
- [ ] Сделать описания страниц (тег `<meta name="description" content="...">`).
- [ ] Сделать мета-описание в рамках OpenGraph.
- [ ] Сделать мета-описание в рамках Twitter (если необходимо).
- [ ] Сделать микроразметку в синтаксисе JSON-LD.
