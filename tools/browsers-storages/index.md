...
Так, например, при переполнении оно полностью очищается. А Safari очищает localStorage, если к нему не обращались в течение семи дней. То есть если пользователь не посещал ваш сайт больше 7 дней, то данные, хранящиеся в нем, будут удалены.
