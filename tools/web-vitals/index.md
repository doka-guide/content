...
Браузер качает файл асинхронно, не блокируя рендер, а `onload` подключает стили, как только они готовы; `noscript` - запасной вариант для отключённого JS. В проде обычно берут готовую обёртку вроде `loadCSS`, а не пишут этот трюк руками;
