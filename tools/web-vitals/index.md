...
- **Отменяйте устаревшие запросы через [`AbortController`](/js/abort-controller).** Если пользователь быстро печатает текст в поле поиска, каждое изменение может вызывать отправку нового запроса на [`сервер`](/js/fetch). Отменяйте предыдущий запрос перед отправкой нового - иначе главный поток обрабатывает колбэки ответов, которые уже никому не нужны:
...
Есть и второе значение — **`content-visibility: hidden`**. В отличие от `display: none`, оно скрывает элемент, но сохраняет результаты предыдущей отрисовки, поэтому повторное отображение обычно обходится дешевле. Это полезно для скрытых вкладок или виртуального скролла.

Из подводных камней:

- вызов DOM-методов вроде `getBoundingClientRect()` или `offsetHeight` на скрытом поддереве принудительно запускает его layout и paint, сводя выгоду на нет;
...
- **Сначала измерьте.** Не оптимизируйте вслепую - посмотрите, из чего состоит бандл. `webpack-bundle-analyzer` или `@next/bundle-analyzer` рисуют интерактивную карту, где сразу видно самые большие пакеты. Далее можно проанализировать эту картинку и понять, что можно оптимизировать;
