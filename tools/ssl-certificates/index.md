...
В этой статье мы говорим про шифрование данных. Если вам не очень понятно, зачем оно нужно и как работает, прочитайте статью «[Шифрование](/tools/encoding/)»
...
То есть SSL-сертификат — это открытый ключ, способ применения которого имеет юридическую силу и право владения которым подтверждается авторизованным провайдером. Применяются сертификаты в рамках протокола SSL на этапе формирования сеанса передачи данных.
...
Рассмотрим схему обмена данными двух сторон _А_ и _Б_ с помощью SSL-сертификата. Принимается, что обе стороны доверяют третьей стороне _Т_, которая соответствует центру сертификации. У сторон _А_ и _Т_ есть пары ключей — открытого и закрытого.

Сторона _А_ пересылает стороне _Т_ свой открытый ключ. Сторона _Т_ с помощью определённых механизмов устанавливает соответствие между открытым ключом стороны _А_ и самой стороной _А_ (физическое или юридическое лицо). После этого сторона _Т_ пересылает стороне _А_ сертификат, который содержит:

- Открытый ключ стороны _А_.
- Информацию о стороне _А_, которая позволяет её идентифицировать.
- Информацию о стороне _Т_, которая позволяет её идентифицировать.
- Электронную подпись стороны _Т_ (хэш-сумму), которая генерируется по содержимому сертификата и затем зашифровывается с использованием закрытого ключа стороны _Т_.
- Другие данные.

Когда сторона _А_ пересылает стороне _Б_ свой сертификат, сторона _Б_ сравнивает сгенерированную хэш-сумму на основе содержимого сертификата и хэш-сумму, которая получается при расшифровке электронной подписи стороны _Т_ с помощью открытого ключа. Если хэш-суммы совпадают, то открытый ключ стороны _А_ действительно принадлежит именно ей. После этого сторона _Б_ может шифровать данные с использованием открытого ключа стороны _А_ и пересылать ей эти данные в зашифрованном виде. И только сторона _А_ может расшифровать их.
...
Можно выбирать любой понравившейся центр, но нужно принимать во внимание несколько факторов:
...
- Отзывы клиентов центра. Сюда же относится и репутация. Есть несколько международных организаций: которые могут поддержать доверия к центру включением в специальный список доверенных. Аккредитация может быть произведена и контролирующим органом внутри страны, в юридическом поле которой действует компания.
...
SSL-сертификаты бывают нескольких классов, принадлежность к которым, как правило, является в основном маркетинговой стратегией той или иной компании. Но всегда действует принцип: с повышением класса сертификата, растёт степень доверия, поскольку сертификат становится более надёжен как с точки зрения защиты (используются более совершенные шифры), так и с точки зрения постоянной проверки со стороны центра сертификации. Чем класс выше, тем больше внимания уделяется сертификату со сторона удостоверяющего центра.
...
Выше приведены лишь самые распространённые сертификаты, которые активно используются в мире. Есть сертификаты, которые можно оформить только на юридическое лицо, а бывают е, которые допускают использование и физическими лицами.
