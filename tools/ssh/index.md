...
Доступ по IP-адресу не является эффективным и безопасным, так как из-за нехватки IPv4 адресов за одним IP-адресом может скрываться несколько пользователей.
...
Рассмотрим пример: хостинг-провайдер создал сервер для вас, IPv4-адрес которого _192.0.2.50_ и IPv6-адрес _2001:0DB8::5_.
...
IPv4 вариант
```bash
ssh-copy-id -i ~/.ssh/id_rsa.pub root@192.0.2.50
```
IPv6 вариант
```bash
ssh-copy-id -i ~/.ssh/id_rsa.pub root@2001:0DB8::5
```
...
IPv4 вариант
```bash
scp ~/.ssh/id_rsa.pub root@192.0.2.50:/tmp
```
IPv6 вариант
```bash
scp ~/.ssh/id_rsa.pub root@[2001:0DB8::5]:/tmp
```
