...
description: "Web Application Firewall, он же WAF"
authors:
  - disi
keywords:
  - веб-сервер
...
WAF существуют в виде ПО, которое мы разворачиваем на своей стороне, либо, которое может разворачиваться у облачного провайдера, например, CloudFlare.

Мы рассмотрим настройку базового WAF на своей стороне.

## Как понять

Firewall (межсетевой экран), в понимании работы сети, подразумевает ПО, которое выполняет защиту и разграничение доступа.
Если в контексте операционных систем, межсетевой экран предупреждает и блокирует доступ по портам и типу трафика, то WAF это делает по входящим запросам и их содержимому.

Стоит отметить, вам не придётся детально настраивать первоначальную работу WAF, он обычно работает от набора базовых сигнатур, которые обеспечивают должный уровень безопасности.
...
Самый популярный набор это [coreruleset](https://github.com/coreruleset/coreruleset/), данный набор соответствует OWASP Top 10.
