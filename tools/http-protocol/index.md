...
Например, браузер посылает запрос на открытие главной страницы сайта, тогда стартовая строка запроса будет такой:

```bash
GET / HTTP/1.1
```

Стартовая строка ответа будет такой, если страница существует и к ней есть доступ:
