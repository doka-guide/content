...
HTTP был разработан как протокол обмена данными между веб-сервером и веб-браузером. Это протокол прикладного уровня модели OSI, который используется для передачи данных (HTML, CSS, JS, API, картинок, аудио, видео, введённых пользователем данных в форме и других) между клиентом и сервером. Это означает, что клиент (веб-браузер) отправляет серверу (веб-серверу) запросы и получает от него ответы, то есть сервер в рамках протокола HTTP занимает практически всегда пассивную позицию.
...
3. Прокси (прокси-сервер) — программа, которая, работая на сервере, пропускает через себя запросы и ответы и выступает в роли посредника между клиентом и сервером.
...
Формат для HTTP-сообщения используется текстовый. Структура сообщения строго определена:

1. Стартовая строка.
2. Заголовки, передают сервисную информацию.
3. Тело сообщения, представляет данные в текстовом виде.

В случае запроса стартовая строка содержит информацию о методе запроса, относительном адресе и версии протокола в формате `Метод URI HTTP/Версия`. Стартовая строка ответа содержит также версию протокола, код ответа и статус ответа сервера в формате `HTTP/Версия Код Статус`.
...
Можно прочитать обо всех статусах в документации или на специальном [ресурсе](https://httpstatuses.com), который даёт информацию в сжатом и удобном для разработчика виде.
...
Принято заголовки группировать в соответствии со списком выше и посылать их в соответствующем списку порядке. В стандартах для каждой версии HTTP описано довольно много заголовков, но можно использовать и свои.
