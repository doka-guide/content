...
В Next.js компонент `next/script` позволяет выбрать момент загрузки стороннего кода: `beforeInteractive`, `afterInteractive` (по умолчанию), `lazyOnload` или `worker`.
...
- **Включите tree shaking.** Сборщик сможет удалить неиспользуемый код, если библиотека поддерживает tree shaking и вы импортируете только нужные модули. `import { debounce } from 'lodash-es'` вместо `import _ from 'lodash'`, используйте точечные импорты из `date-fns` вместо подключения целой библиотеки вроде `moment`, или используйте нативные аналоги. Импорт «всего сразу» убивает tree shaking в обычных случаях.

- **Берегитесь barrel-файлов.** Удобный `index.ts`, который реэкспортит всё из папки (`export * from './...'`), может помешать tree shaking: импортируете одну кнопку - загружается вся UI-библиотека. В Next.js это лечит опция **`optimizePackageImports`**: импорты разворачиваются в точечные автоматически;

- **Выносите вычисления на сервер.** Самый большой клиентский бандл часто делает работу, которой там не место - например, рендер Markdown. В Next.js App Router это уносится в **серверные компоненты** - значительная часть логики выполняется на сервере, а объём клиентского JavaScript уменьшается;
