...
⚠️ Не используйте `isInputPending()` для проверки «не ждёт ли ввод», спецификация признала её ненадёжной: она может вернуть `false`, хотя пользователь уже нажал. Полагайтесь на `scheduler.yield()`.
