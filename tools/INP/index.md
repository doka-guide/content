...
- **Не рендерите большие куски HTML на клиенте через `innerHTML` или аналоги.** В отличие от серверного HTML, который браузер может разбирать и отображать потоково, у клиентской вставки нет автоматической нарезки на части - парсинг и вставка большой разметки выполняются как одна длинная задача и блокирует кадр. Плюс ресурсы (картинки, скрипты) внутри такой разметки не увидит preload-сканер, это бьёт уже по LCP. Максимизируйте серверный рендеринг (SSR/RSC) и старайтесь, чтобы объём клиентской отрисовки оставался небольшим;
...
- вызов DOM-методов вроде `getBoundingClientRect()` или `offsetHeight` на скрытом поддереве принудительно запускает layout, сводя часть выигрыша на нет;
