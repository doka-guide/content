...
Браузер cкачивает файл асинхронно, не блокируя рендер, а `onload` подключает стили, как только они готовы; [`noscript`](/html/noscript/) - запасной вариант для отключённого JS. Такой приём позволяет загрузить некритичные стили без блокировки первоначальной отрисовки страницы.

- **Старайтесть не добавлять синхронный JavaScript в `<head>`** - переносите вниз `<body>` страницы или добавляйте [`async`](/html/defer-async/) по возможности;
...
- Следите, чтобы тяжёлые задачи на главном потоке не блокировали отрисовку сразу после загрузки картинки (смотрите про длинные задачи в статье про [INP](/tools/INP/)). Учтите - браузеры отрисовывают изображения именно на главном потоке, поэтому даже полностью загруженная картинка не появится на экране, пока главный поток занят выполнением JavaScript;
