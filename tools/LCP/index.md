...
- Если LCP-изображение задаётся через CSS `background-image`, браузер обнаружит её только после загрузки и разбора CSS. По возможности используйте `<img>`, а если это невозможно - добавьте `<link rel="preload" as="image" fetchpriority="high">`;
...
Для фотореалистичных LCP-изображений - hero-баннеров и обложек - SVG не подходит: используйте растровые форматы AVIF, WebP или PNG.
...
Этот приём имеет свои компромиссы (FOUC, сложнее поддерживать). Браузер скачивает файл асинхронно, не блокируя рендер, а `onload` подключает стили, как только они готовы. [`Noscript`](/html/noscript/) - запасной вариант для отключённого JS. Такой приём позволяет загрузить некритичные стили без блокировки первоначальной отрисовки страницы.

- **Старайтесь не добавлять синхронный JavaScript в `<head>`** - переносите вниз `<body>` страницы или добавляйте [`async или defer`](/html/defer-async/) по возможности;
...
- **Не задерживайте отображение текста веб-шрифтами.** LCP-элемент нельзя назначить вручную: браузер определяет его во время загрузки страницы. Проверьте в PageSpeed Insights или DevTools, какой элемент браузер определил как LCP. Если это текст, используйте `font-display: swap` (или `optional`) и предзагружайте критические шрифты. Иначе браузер может отложить отображение текста до завершения загрузки шрифта.
