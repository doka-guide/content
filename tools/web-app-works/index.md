...
При нажатии браузер пошлёт [запрос на сервер](/tools/api/), в котором сообщит, что ему необходимо отсортировать такой-то список по такому-то критерию.
...
Такое общение между сервером и клиентом очень напоминает [паттерн MVC](/tools/architecture-mvc/).
