...
[Gulp](https://gulpjs.com/) (англ. «глоток» [/gʌlp/, /галп/]) — менеджер для организации и выполнения задач при разработке приложений с использованием платформы [Node.js](/tools/nodejs/). Gulp может выполнять разные задачи: сформировать файлы стилей, объединить и минифицировать файлы, оптимизировать изображения, транспилировать код и т. д. При выполнении этих задач происходит взаимодействие с рабочим окружением: обратиться к файловой системе, создать или сохранить итоговый файл, запустить процесс локального web-сервера, показать результат в браузере.

## Как понять

Gulp является менеджером задач, который выполняет объявленные инструкции в указанной последовательности. Он не предоставляет разработчику готовые механизмы для сборки проекта, но позволяет настроить такую сборку с помощью необходимых модулей.
...
- проверить HTML-разметку и CSS-правила;
- синтаксис CSS-препроцессоров преобразовать в «чистый» CSS;
- проверить JS-код на соответствие требуемым стандартам;
- при необходимости объединить несколько файлов (CSS или JS) в один;
- минифицировать большие файлы;
- посмотреть в браузере результат, желательно по принципу «live-reload» («живая перезагрузка»);
- сформировать итоговую сборку проекта.
...
Будут использоваться следующие NPM-модули:
