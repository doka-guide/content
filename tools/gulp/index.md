...
Часто в своих проектах разработчики используют Webpack, но это — сборщик модулей для приложений, написанных на JavaScript. Он «из коробки» предлагает решения для сборки проектов с использованием уже подготовленных механизмов и настроек. Более подробно об этом инструменте можно почитать в [статье Доки](/tools/webpack/).
...
Эта функция регистрирует объявленную инструкцию в качестве общедоступного задачи с названием `styles`, который может взаимодействовать с рабочим окружением. Теперь такую задачу можно запускать на выполнение как отдельно — командой `gulp styles`, так и в составе сводной задачи (`watch`, `default` и т. п.).
...
Только после этого функция станет доступной для запуска и выполнения как отдельной командой `gulp styles`, так и составе сводной задачи.
...
Поскольку название `default` для Gulp является служебным, запуск этой задачи в терминале можно выполнить командой `gulp`.
