...
Метод `search()` использует асинхронную функцию `fetch()`. Это нужно будеть учесть в тесте. Мы уже знаем как начать: создаём папку `__tests__`, закидываем в неё `search-api-client.js`. Так как поиск асинхронный, тест у нас тоже будет асинхронный.
...
На этот раз `Jest` не доволен значением, c которым мы релозолвим промис. У нас есть статья которая [подскажет что же должен возвращать `fetch()`](/js/fetch/). Прочитав ее, мы уверенно правим наш тест:
...
Сначала мы проверям что класс `article__aside--offset` не добавлен к элементу, потом вызываем `callFixed` и проверяем что класс добавлен. Как всегда, не надо думать, надо написать тест!
