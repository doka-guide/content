...
Если вы не любите читать, но любите смотреть, предлагаем вам три коротких видео: [раз](videos/part-1.mp4), [два](videos/part-2.mp4), [три](videos/part-3.mp4). В них показано все, что мы будем делать в этом рецепте.
...
В реальных приложениях конфигурация тестов [более затейливая](https://github.com/apollographql/apollo-client/blob/78f6d27d2d926c56cefd54d6f3e2371eb7e890d1/package.json#L53), вам может понадобиться несколько команд для запуска разных тестов. Или вам может понадобиться запускать разные тесты с разными параметрами.
...
Для второго упражнения попробуем потестировать функционал поиска. Он живёт в файле [src/scripts/core/search-api-client.js](https://github.com/doka-guide/platform/blob/main/src/scripts/core/search-api-client.js) платформы доки. Мы будет тестировать функцию `search()`.
...
Метод `search()` использует асинхронную функцию `fetch()`. Это нужно будет учесть в тесте. Мы уже знаем как начать: создаём папку `__tests__`, закидываем в неё `search-api-client.js`. Так как поиск асинхронный, тест у нас тоже будет асинхронный.
...
Наша заглушка для `fetch()` всегда возвращает `Promise`, который релозолвится числом `42`. Тест по-прежнему не проходит.

![Тест продолжает падать](images/fetch-keep-failing.png)

На этот раз `Jest` не доволен значением, c которым мы релозолвим промис. У нас есть статья которая [подскажет что же должен возвращать `fetch()`](/js/fetch/). Прочитав её, мы уверенно правим наш тест:
...
Теперь потренируемся писать тесты для функий работы с DOM. Мы будем тестировать функцию `init()` в файлике[_article-aside.js_](https://github.com/doka-guide/platform/blob/main/src/scripts/modules/article-aside.js) репозитория платформы. Внутри эта функция использует объект `headerComponent`, который является чем-то вроде _EventEmitter_. Мы навешиваем на `headerComponent` два обработчика событий: `fixed` и `unfixed`. Меняем класс нашего компонента в момент когда одно из этих событий происходит.
...
Если вам приходится изменять код под тесты обычно означает, что вы делаете что-то не то или что ваш код написан не совсем правильно. Нам пришлось дописать `export`. Это может значить две вещи:

- Функцию `init()` тестировать не нужно.
- Мы просто забыли экспортировать функцию `init()`.
...
Но вернёмся к нашему тесту. Нужно как-то добавить _DOM_ чтобы он не падал. Для этого нужно просто поменять тестовое окружение. Это можно сделать в настройках тестов _jest.config.js_ или использовав специальный doc-комментарий в начале файла с тестом.
...
Окружение [_jsdom_](https://github.com/jsdom/jsdom) позволяет вам эмулировать браузерный контекст в Node.js. Вам становится доступна переменная `document`, вы можете использовать многие _DOM_ API. Если элемент присутствует в HTML, переданном в _jsdom_ вы можете работать с ним точно так же как в браузере.

После добавления нужного комментария наш тест начнёт проходить. Теперь нужно убедиться что функция `init()` сработала так как нужно: для этого проверяем, что для элемента с классом `article__aside` добавился класс `article__aside--offset` когда событие `fixed` произошло. Но как вызвать событие `fixed`? 🤔

Заглянув в [_header.js_](https://github.com/doka-guide/platform/blob/main/src/scripts/modules/header.js) мы увидим аж 250 строчек кода. И мы не очень-то хотим разбираться что этот код делает. Давайте просто заменим настоящий _header.js_ заглушкой (моком). Для этого нам пригодится магия [`jest.mock()`](https://jestjs.io/docs/bypassing-module-mocks).
...
В качестве первого аргумента мы передаём путь до модуля, который хотим замокать, а в качестве второго – реализацию этого модуля.
...
Сначала мы проверяем что класс `article__aside--offset` не добавлен к элементу, потом вызываем `callFixed` и проверяем что класс добавлен. Как всегда, не надо думать, надо написать тест!
