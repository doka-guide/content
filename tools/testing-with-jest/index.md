...
Метод `search()` использует асинхронную функцию `fetch()`. Это нужно будет учесть в тесте. Мы уже знаем как начать: создаём папку *__tests__*, закидываем в неё _search-api-client.js_. Так как поиск асинхронный, тест у нас тоже будет асинхронный.
