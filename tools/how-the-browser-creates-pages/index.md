...
Иногда анимация, применённая к элементу, не вызывает выделение в отдельный слой, потому что не распознаётся браузером как триггер для композитинга. Однако мы всё равно хотим стриггерить композитинг, чтобы добиться плавности. Для этого есть несколько решений.

Во-первых, можно воспользоваться хаком и вызвать на элементе пустую анимацию, являющуюся триггером. Например:
...
Другой способ — CSS-свойство [`will-change`](/css/will-change/). Оно предупреждает браузер о том, что элемент в ближайшее время ожидают изменения.
...
Каждый слой в документе занимает дополнительную память, поэтому держать их без надобности — плохая практика. Следовательно, оптимальной стратегией использования `will-change` будет:

1. Навешивать свойство на элемент прямо перед началом анимации.
1. Ставить его в `false` сразу после окончания анимации.
