...
Иногда анимация, примененная к элементу, не вызывает выделение в отдельный слой, потому что не распознается браузером как триггер для композитинга.

Однако мы всё равно хотим стриггерить на композитинг, чтобы добиться плавности. Для этого есть несколько решений.

Во-первых, можно воспользоваться хаком и вызвать на элементе пустую анимацию, являюущуюся триггером из списка выше. Например:
...
Даже если мы не планируем запускать трехмерную анимацию на объекте или реально работать с `z-index`, наличие подобного свойства у элемента достаточно, чтобы выделять его на отдельный слой.

Другим способом является CSS-свойство `will-change`. Оно предупреждает браузер о том, что элемент в ближайшее время ожидают изменения.
...
2. Ставить его в false прямо после окончания анимации.
