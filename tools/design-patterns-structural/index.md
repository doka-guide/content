...
🧅 В [трёхслойной архитектуре](/tools/clean-architecture/) адаптеры занимают аж целый отдельный слой.
...
Он немного похож на адаптер, потому что тоже может делать несовместимое API совместимым, но его основная цель всё же — [инкапсулировать](/tools/oop/#inkapsulyaciya) часть связанной логики и дать к ней доступ через один метод.
