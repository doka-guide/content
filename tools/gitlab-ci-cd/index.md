...
Вы можете формировать релиз приложения в одноименно разделе при помощи GitLab CI. Для этого используется отдельный скрипт `release-cli`. В примере мы получим его с помощью Docker от GitLab.
