...
### Создаём `.gitlab-ci.yml` и указываем общие параметры
...
GitLab даёт доступ к большому количеству переменных окружения с полезной информацией. Например, `$CI_COMMIT_BRANCH` содержит текущую ветку, `$CI_COMMIT_SHORT_SHA` - короткий хеш коммита, `$CI_PIPELINE_SOURCE` - источник вызова текущего пайплайна и так далее. При помощи их мы можем запускать определённые джобы при соблюдении заданных условий. Для этого нужно объявить одну или несколько секций `rules`.
...
Ещё одна типичная задача - прогнать тесты в разных версиях Node.js. Можно для каждой версии создать вручную джоб, а можно указать список из переменных:
