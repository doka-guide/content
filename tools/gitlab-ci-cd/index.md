...
Для передачи результов сборки (бинарных файлов, логов, конфигураций и т.д.) между джобами используются **артефакты**. **Артефакт** - общий ресурс пайплайна, хранящийся отдельно и передающийся между этапами сборки. 
...
### Работа с подмодулями на своем сервере Gitlab
...
Для добавление файла к артефактам используют следующий синтакс файла `.gitlab-ci.yml`. 
...
Можно использовать шаблонами поиска (wildcards): 
...
Все артефакты можно посмотреть в разделе «Build» -> «Artifacts». Однако время жизни артефактов ограничено (по умолчанию 14 дней), поэтому не используйте этот раздел для долговременного хранения сборок. Для этого лучше использовать Package Registry.
...
При работе в локальном домене, может использовать самоподписанный сертификат. Чтобы докер внутри Gitlab CI узнал о нём, укажите переменную `ADDITIONAL_CA_CERT_BUNDLE`:
