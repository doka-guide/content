...
<details>
  ...
  Если же прогрессивное веб-приложение не способно работать без доступа к интернету, но при этом обеспечивает пользовательский опыт наравне с нативными приложениями, оно не перестаёт быть прогрессивным. Далеко не каждому приложению это нужно. Например, приложение для видеозвонков в целом бесполезно без доступа к интернету.
...
Ранее браузеры на основе Chromium требовали также наличия service worker (специального файла JavaScript, который работает в фоновом режиме независимо от веб-страницы). Он позволяет выполнять задачи, даже если вкладка или браузер закрыты (например, получение push-уведомлений, фоновая синхронизация, перехват и обработка сетевых запросов, что как раз и позволяет реализовать кеширование и работу без доступа к интернету).
