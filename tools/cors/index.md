...
Сервер может разрешить только один конкретный `Origin`, нельзя указать несколько, если доступ необходим сразу с нескольких доменов, то сервер должен проверить, что `Origin` из запроса клиента разрешен и динамически подставить этот `Origin` в ответ, а также добавить заголовок `Vary`, чтобы браузер понимал, что в зависимости от источника, ответ может быть другим:
...
Но лучше избегать разрешения доступа с любого домена (`*`), это повышает уязвимость вашего сервера к [атакам](/tools/web-security/), например, CSRF.
<aside>
