...
- после этого выполняется блок первой программы, пока не программа не отправит данные на печать;
- пока печатаются данные, выполняется блок второй программы.

Следующим шагом было внедрение специального диспетчера операционной системы. Главная задача диспетчера — максимально эффективно использовать процессор. Для этого все программы бились уже не на блоки, а на отдельные операции (команды для процессора). Процессор заблокирован, пока программа не выполнится полностью. На схеме показано использование диспетчером очереди готовых к запуску программ (процессов в терминах операционной системы):

![Очередь готовых к запуску процессов в многопоточных операционных системах](images/2.png)

Диспетчер операционной системы распределяет операции процессов в очереди в порядке приоритетов. Такая очередь называется _очередью готовых к запуску_. Приоритет служит основанием для постановки процесса в очередь. Место в очереди определяется приоритетом процесса. Например, системным процессам назначается наивысший приоритет. В некоторых операционных системам приоритетом процессов можно управлять.
...
В операционной системе процесс — это не просто совокупность операций, то есть код программы. Для того чтобы процессор мог возвращаться к выполнению программы, в операционной системе должны хранится промежуточные результаты выполнения программы и состояние процессора на момент перехода к другому процессу (контекст) и значения переменных (данные программы):
