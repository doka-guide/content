...
В этой статье мы говорим про веб-сервер Apache. Если вам не очень понятно, зачем он нужен и как работает, прочитайте статью «[Веб-сервер](/tools/articles/webserver/)».
...
Для указания директории, к которой веб-сервер должен перенаправить HTTP-запросов от клиента, используется следующий список директив:

- <Directory directory-path> ... </Directory> — устанавливаются правила поведения веб-сервера для файлов и поддиректорий внутри пути `directory-path`;
- <DirectoryMatch regex> ... </DirectoryMatch> — тоже самое, что и `Directory`, но применяется ко всем путям, которые удовлетворяют регулярному выражению `regex`;
- <Files filename> ... </Files> — устанавливаются правила поведения веб-сервера для файлов с именем `filename`;
- <FilesMatch regex> ... </FilesMatch> — тоже самое, что и `Files`, но применяется ко всем файлам, имена которых удовлетворяют регулярному выражению `regex`;
- <Location URL-path|URL> ... </Location> — устанавливаются правила поведения веб-сервера для определённых абсолютных и относительных ссылок `URL-path|URL`;
- <LocationMatch regex> ... </LocationMatch> — тоже самое, что и `Location`, но применяется ко всем ссылкам, которые удовлетворяют регулярному выражению `regex`;
- <VirtualHost addr[:port] [addr[:port]] ...> ... </VirtualHost> — устанавливаются правила поведения веб-сервера для определённых имён хостов или IP-адресов.
