...
Используя этот подход реактивности, каждый раз, когда мы вносим изменения в свойство, оно уведомляет наблюдателей и зависимости, которым необходимо знать, что произошло изменение. Настройка этого свойства происходит при инициализации компонента или при явном вызове `Vue.set` или `vm.$set` .
...
Такой способ модификации массива не вызовет обновления и компоненты интерфейса не узнает о том, что массив изменился. Почему так происходит? Потому что сеттеры для массивов не имеют средств обнаружения присвоений по индексу.
...
Но разработчики Vue 2 обернули для нас [`несколько методов массивов`](https://github.com/vuejs/vue/blob/dev/dist/vue.js#L863), поэтому можно обновлять массивы, используя эти методы.
