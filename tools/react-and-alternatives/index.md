...
Vue реализует реактивность с помощью Proxy. Proxy – это специальный объект в JavaScript, который позволяет следить за изменениями свойств другого объекта. Это даёт Vue 3 возможность отслеживать, когда свойство объекта было изменено или считано, и реагировать на это. При этом объект, отслеживаемый с помощью Proxy, ничем не отличается от обычного.
...
В отличие от Vue 3, реактивность Vue 2 достигается с помощью геттеров и сеттеров, определённых в методе Object.defineProperty.

Реактивность Vue 2 в корне отличается от Vue 3 и выглядит следующим образом:
...
Используя этот подход реактивности, каждый раз, когда мы вносим изменения в свойство, оно уведомляет наблюдателей и зависимости, которым необходимо знать,
что произошло изменение. Настройка этого свойства происходит при инициализации компонента или при явном вызове Vue.set / vm.$set .

Однако требуется понимание следующего:
...
Такой способ модификации массива не вызовет обновления и компоненты интерфейса не узнает о том, что массив изменился.
Почему так происходит? Потому что сеттеры для массивов не имеют средств обнаружения присвоений по индексу.
...
Но разработчики Vue 2 обернули для нас [`несколько методов массивов`](https://github.com/vuejs/vue/blob/dev/dist/vue.js#L863),
поэтому можно обновлять массивы, используя эти методы.
