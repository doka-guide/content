...
JSON основан на JavaScript, но является независимой от языка спецификацией для данных и может использоваться почти с любым языком программирования, поэтому он не допускает использование некоторых типов JavaScript. В процессе преобразования объекта в JSON не попадут:
