...
docker volume prune
```
Тома расположены в файловой системе компьютера, на котором запущены службы Docker:
- в Windows — откройте проводник `\\wsl$\docker-desktop-data\version-pack-data\community\docker\volumes\`  этот путь можно просмотреть также используя подсистему Линукс для windows wsl2
каждый том представлен папкой с названием тома.
<<<<<<< HEAD

👀 в Windows c помощью утилиты DiskInternals Linux Reader можно увидеть *все* тома Docker в файле виртуальной файловой системы _vhdx.ext4_  `C:\Users\Username\AppData\Local\Docker\wsl\data\`
в случае, когда Docker Desktop забирает много места в файловой системе Windows можно оптимизировать этот файл в Powershell:
```powershell
    net stop com.docker.service
    taskkill /IM "docker.exe" /F
    taskkill /IM "Docker Desktop.exe" /F
    wsl --shutdown
    Optimize-VHD -Path C:\Users\Username\AppData\Local\Docker\wsl\data\vhdx.ext4  -Mode Full
```
В Линукс и MacOS такая оптимизация Docker не нужна,  если вы удаляете том, то занятый объем высвобождается.  проверить занятое пространство, как томами, так и контейнерами можно с помощью  
```bash
    docker system df -v   
```
Содержимое тома находится в поддиректории `_data`.
- в Linux — `/var/lib/docker/volumes/`. 
=======
👀 c помощью утилиты DiskInternals Linux Reader можно увидеть *все* тома Docker в файле виртуальной файловой системы vhdx.ext4  `C:\Users\Username\AppData\Local\Docker\wsl\data\`
в случае, когда Docker Desktop забирает много места в файловой системе Windows можно оптимизировать этот файл в Powershell:
```powershell
net stop com.docker.service
taskkill /IM "docker.exe" /F
...
- в Linux — `/var/lib/docker/volumes/`.
Содержимое тома находится в поддиректории `_data`.
>>>>>>> 03e8216448d11cd39b1467c7dbdcdbb0f542460d

При создании тома можно указать драйвер, позволяющий работать с различными файловыми системами. По умолчанию docker использует драйвер _local_. Конкретные настройки драйвера зависят от файловой системы хоста. Для ntfs, nfs, btrfs, zfs и других конфигурация и названия опций отличается. Docker может использовать и другие драйверы. Например если docker запущен на виртуальной машине то нужен специальный драйвер, или если понадобится доступ к специальной файловой системе с удалённым доступом через sshfs или к файловой системе в облаке. У Яндекс-облака есть [свой драйвер](https://github.com/yandex-cloud/docker-machine-driver-yandex), есть драйвер Digital Ocean,  Amazon и Azure.
По умолчанию том докера — «резиновый» и не имеет ограничений на максимальный размер, в отличии от физического диска компьютера. С помощью ключа _--opt_ можно задать максимальный размер тома, его расположение, права доступа.

пример создания тома с указанием параметров из командной строки
...
<<<<<<< HEAD
    ...
    --driver local \
    --opt type=overlay2 \
    --opt device=/mnt/bc72cea2-147f-4545-876a-92e32b23223a/port_vol \
    --opt o=size=100m,id=1000 \ 
    foo
```
=======
docker volume create \
...
``` 
>>>>>>> 03e8216448d11cd39b1467c7dbdcdbb0f542460d
Команда выше создаёт том с именем _foo_ типа _overlay2_ размером 100 Мб, расположенный в директории `/mnt/bc72cea2-147f-4545-876a-92e32b23223a/port_vol`, доступ к которому ограничен одним пользователем с id = 1000. Тип можно не указывать, так как это дефолтное значение для docker последних версий. 

ещё пример, с использованием сетевого хранилища для размещения тома
...
<<<<<<< HEAD
    ...
    --driver local \
    --opt type=cifs \
    --opt device=//uxxxxx.your-server.com/backup \
    --opt o=addr=uxxxxx.your-server.com,username=uxxxxxxx,password=*****,file_mode=0777,dir_mode=0777 \
    --name cif-volume
```
Команда выше создаёт том cif-volume, который является сетевой папкой, доступной по протоколу [_samba_](https://ru.wikipedia.org/wiki/Samba). Это может быть удобно, если вы работаете с папками с общим доступом в операционной системе windows или на сетевом накопителе.
Тома можно использовать в разных контейнерах. Результат работы приложения внутри одного контейнера становится доступен другим приложениям. Для этого при создании контейнера можно явно указать название тома или использовать тома из других контейнеров с помощью ключа _--volumes-from <id container>_. Вместо _<id container>_ подставляется имя контейнера или его id.
Список активных томов можно вывести с помощью команды `docker container ls`. Для просмотра томов неактивных контейнеров следует сначала получить имя или id контейнера из списка контейнеров с помощью команды `docker ps -a`, затем использовать docker inspect id. Docker выведет всю информацию, в которой, правда, трудно сразу увидеть что относится к томам контейнера. в Линукс, Windows и MacOS можно установить утилиту [jq](https://stedolan.github.io/jq/) для удобной работы с JSON. C её использованием можно получить лаконичный вывод - отфильтровать всё, кроме информации о Mounts: 

```bash docker inspect --format '{{ json .Mounts }}' <id нужного контейнера> | jq ```

Используя инструменты командной строки, можно подготовить тома Docker к совместному использованию.
Любой диск можно использовать в разных контейнерах, таким образом файловый результат работы одного приложения станет доступен другим приложениям в Docker.
=======
 docker volume create \
	--driver local \
	--opt type=cifs \
...
``` docker inspect --format '{{ json .Mounts }}' <id нужного контейнера> | jq ```  

Используя инструменты командной строки, можно подготовить тома Docker к совместному использованию. 
Любой диск можно использовать в разных контейнерах, таким образом файловый результат работы одного приложения станет доступен другим приложениям в Docker. 
>>>>>>> 03e8216448d11cd39b1467c7dbdcdbb0f542460d
для этого при создании диска нужно указать --volumes-from <id container>.
...
Команда выше запускает контейнер с именем _Container5_ на основе образа с именем  _ubuntu_, подключая к нему тома из контейнера с именем _Container4_. Если после имени _Container4_ добавить модификатор _:ro_ образ docker _ubuntu_. запись  или удаление общих файлов будет невозможна.

Нужно отметить, что выстраивание связей между томами и приложениями не очень/совсем  неудобно в командной строке, для этого следует использовать Dockerfile и/или Docker-compose которые предоставляют возможности описать в конфигурационном файле все отношения между томами и контейнерами.
