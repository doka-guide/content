...
```
В линукс и МасОS такая оптимизация Docker не нужна,  если вы удаляете том, то занятый объем высвобождается.  проверить занятое пространство, как томами, так и контейнерами можно с помощью  
```bash
docker system df -v   
```
