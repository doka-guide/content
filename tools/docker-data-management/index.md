...
При создании тома docker использует драйвер local, и настраивает его автоматически в зависимости от бэк (нижележащей) файловой системы, т.е. опции настройки для к примеру ntfs, nfs, btrfs, zfs - будут различными. Также  docker может использовать другие (не свои) драйвера в случае если он запущен в виртуальной машине например, или нужен доступ к особенной файловой системе sshfs например. По умолчанию том докера "резиновый" в отличии от физического диска компьютера, поэтому через параметр --opt можно задать граничный размер тома и задать расположение (точка монтирования неизменна), а также задать права доступа к тому.  
