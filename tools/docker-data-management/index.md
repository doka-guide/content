...
для этого при создании диска нужно указать --volumes-from <id container>  id контейнера можно посмотреть ```docker ps``` для работающего контейнера или  ```docker container ls``` вместо id также можно использовать container name.
