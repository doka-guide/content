...
```
в подпапке **_data** - содержимое тома

При создании тома можно указать драйвер, позволяющий работать с различными файловыми системами. По умолчанию docker использует драйвер _local_. Конкретные настройки драйвера зависят от файловой системы хоста. Для ntfs, nfs, btrfs, zfs и других конфигурация отличается. Docker может использовать и другие драйверы. Например docker может быть запущен на виртуальной машине или понадобится доступ к специальной файловой системе sshfs. Например, есть свой драйвер у Яндекс-облака, Digital Ocean и Amazon.

По умолчанию том докера — «резиновый» и не имеет ограничений на максимальный размер, в отличии от физического диска компьютера. С помощью ключа _--opt_ можно задать максимальный размер тома, его расположение, права доступа и так далее.  
...
Команда выше создаёт том с именем _foo_ типа _overlay2_ размером 100 Мб, расположенный в директории `/mnt/bc72cea2-147f-4545-876a-92e32b23223a/port_vol`, доступ к которому ограничен одним пользователем с id = 1000. Тип можно не указывать, так как это дефолтное значение для docker с версией старше 18.04.
```bash
 docker volume create \
	--driver local \
	--opt type=cifs \
	...
	--name cif-volume
```
том docker типа  _cif-volume_ будет создан из сетевой папки с общим доступом, это может быть папка в windows или общий ресурс на сетевом накопителе (nas).  Локальный драйвер будет использовать протокол SMB/CIFS для доступа по сети.

Диск можно использовать в разных контейнерах, таким образом файловый результат работы одного приложения станет доступен другим докер-приложениям. 
для этого при создании диска нужно указать --volumes-from <id container>  id контейнера можно посмотреть ```docker ps -a``` для работающего контейнера или  ```docker container ls``` вместо id также можно использовать container name.
