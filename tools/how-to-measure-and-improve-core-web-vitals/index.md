...
- **CLS различается.** В лаборатории фиксируется в основном сдвиг при загрузке (верх страницы). В поле - за весь жизненный цикл: пользователь скроллит, догружаются ленивые картинки без размеров, срабатывают A/B-тесты - все эти сдвиги лабораторные измерители не видит;

- **LCP различается.** Разным людям на разных экранах виден разный LCP-элемент; у реальных пользователей часть ресурсов уже в кеше, часть ресурсов уже находится в кеше браузера - в синтетике кеш холодный.
