...
1. Иначе прямые потребители (`sinks`) красятся в `dirty`.
1. Все транзитивные потребители переходят в `checked`. Если какой-то сигнал уже был `dirty`, состояние `checked` его не понижает.
1. Затронутые вотчеры (о них дальше) переходят в `pending`, и им синхронно вызывается колбэк `notify`.
...
1. Если `dirty` или `checked`, рекурсивно найти самый глубокий и самый левый (то есть раньше всех прочитанный) источник в состоянии `dirty`. Поиск обрывается, как только встречается `clean`-сигнал: туда лезть не нужно, он точно свежий.
1. Пересчитать найденный сигнал, сравнить новое значение со старым через `equals`.
1. Если значение не изменилось, потребители, которые были `checked`, возвращаются в `clean`, дальше по графу пересчёт не идёт.
1. Если изменилось, потребители красятся в `dirty`, и шаги 2–4 повторяются, пока весь путь до исходного сигнала не станет `clean`.
