...
1. Затронутые низкоуровневые наблюдатели (watchers, о них дальше) переходят в `pending`, и им синхронно вызывается колбэк `notify`.
...
⚠️ Наблюдатель держит наблюдаемые сигналы живыми до тех пор, пока живы их источники, это нужно, чтобы будущий `notify` мог сработать. Если не вызвать `unwatch()` при уничтожении эффекта, получится утечка памяти.
