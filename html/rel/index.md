...
`rel` — атрибут описывает, как связаны текущая страница и подключаемый ресурс. Работает с [<a>](/html/a/), [<link>](/html/link/) и [<area>](/html/area/).
...
Значение `rel="preload"` говорит браузеру заранее загрузить важные ресурсы, например, шрифты. Это уменьшает время отображения страницы.
...
**Альтернативные стили (через `<link>`)**

Позволяет выбрать другую тему оформления:
...
**RSS и Atom-фиды (через `<link>`)**

Когда значение `alternate` сочетается  с `type="application/rss+xml"` или `type="application/atom+xml"`, браузер и RSS-ридеры понимают, что элемент `<link>` указывает на ленту новостей (фид) для подписки.
...
1. Браузеры отображают иконку подписки в адресной строке.
1. RSS-ридеры автоматически обнаруживают фид для импорта.
...
**Ссылка на автора (через `<link>`)**

Добавляет метаинформацию об авторе всей страницы. Чаще всего используется с имейлом или профилем:
...
Атрибут `rel="bookmark"` используется для создания постоянной ссылки на определённую часть страницы. Он применяется в тегах `<a>` и `<area>`. Такая ссылка обычно ведёт к ближайшему элементу `<article>`, если он есть. Если же `<article>` нет, ссылка будет вести к ближайшему разделу страницы.
...
Атрибут `rel="canonical"` помогает бороться с дублирующим контентом на сайте. Используется в теге `<link>`, чтобы указать поисковым системам **предпочтительную версию страницы**. Это важно для SEO, так как предотвращает «размытие» рейтинга между копиями контента.

Как это работает:

- **Указывает главную страницу**. Если у вас несколько URL с одинаковым содержимым (например, `example.com/page` и `example.com/page?utm=1`), каноническая ссылка сообщает поисковикам, какой URL считать основным.
...
- **Сайты с внешними ресурсами**. Если вы подключаете шрифты, скрипты или изображения с других доменов.
- **Одностраничные приложения (SPA)**. Для предзагрузки API-эндпоинтов, которые понадобятся позже.
- **Мультимедийные страницы**. Чтобы видео или аудио с внешних платформ (YouTube, Vimeo) загружались быстрее.
...
1. При добавлении `<link>` в документ.
...
Ссылка ведёт на страницу с дополнительной информацией о теме, которую пользователь вводит в поле. Кликнув по ссылке, он получит подсказки.
...
Ссылка ведёт на страницу, которая объяснит, как правильно заполнить форму. Это полезно для тех, кто может не понять, что именно нужно ввести.
...
Ссылки ведут на страницы помощи, которые объясняют, как пользоваться сайтом или как найти нужную информацию.
...
В некоторых браузерах ссылка c `rel="help"` может подсвечиваться или изменять курсор, чтобы пользователь понял, что это страница помощь.

### `icon`

Атрибут `rel="icon"` — это способ сообщить браузеру, где взять иконку для страницы. Используется вместе с тегом `<link>` и помогает подключить внешний ресурс с фавиконом. При этом можно задать разные форматы и размеры, чтобы иконка выглядела хорошо на любом устройстве.

Размеры и форматы:

- Значения в атрибуте `sizes` указывают реальные размеры (сырые пиксели), а не те, что заданы в CSS. Например, для ретина-экранов, где 1 CSS-пиксель равен 2 сырым, иконка шириной 50 пикселей должна иметь фактическую ширину 100 пикселей.
- Если ресурс представляет собой масштабируемую иконку (например, SVG), используйте значение `any`.
...
В примере выше лицензия относится исключительно к изображению, а не ко всему документу. Лицензию на весь контент сайта обычно указывают в подвале.
...
В этом примере URL _http://example.com/pingback_ получает уведомление, если контент текущей страницы упоминается на других ресурсах. Такой механизм удобен для поддержания связи между сайтами и позволяет авторам быть в курсе, где их материалы находят применение.
...
При обнаружении ссылки с `rel="preconnect"` браузер начинает установку соединения до фактического запроса ресурса.
...
- Используется атрибут `as`, указывающий тип загружаемого ресурса (`style`, `script`, `font`, `image`, `video` и так далее).
...
Атрибут `rel="next"` можно использовать в элементах `<link>`, `<a>`, `<area>` и `<form>`. Ключевое слово `next` указывает, что текущий документ является частью последовательности, а ссылка ведёт к следующему документу.
...
Так же, как и `next`, может использоваться в `<link>`, `<a>`, `<area>` и `<form>`. Ключевое слово `prev` обозначает, что текущий документ связан с предыдущим документом в последовательности.
