...
`dns-prefetch` полезно использовать для сайтов с внешними ресурсами, где шрифты, скрипты или изображения подключаются с других доменов, а также для мультимедийных страниц, чтобы видео или аудио с внешних платформ (YouTube, Vimeo) загружались быстрее. Полезно и для одностраничных приложений (SPA), чтобы предзагружать API-эндпоинты, которые понадобятся позже.
...
Атрибут `rel="expect"` используется с элементами `<link>` просто чтобы указать, что конкретный элемент (указанный в атрибуте `href="#id-элемента"`) важен для отрисовки страницы.

Для того, чтобы элемент сразу отобразился на странице, необходимо задержать переход к странице до обработки элемента, добавив атрибут `blocking="render"`. В сочетании они гарантируют, что критическая часть DOM-дерева распарсена и отрендерена согласованно, прежде чем браузер начнет анимацию перехода к странице. Это предотвращает визуальные несоответствия, когда переход начинается раньше, чем контент стабилизируется.

Когда браузер встречает `<link rel="expect">`, он:

1. Извлекает идентификатор элемента из атрибута `href`. Значение должно быть в формате `"#id-элемента"` (якорь на элемент внутри текущего документа).
1. Проверяет наличие атрибута `blocking="render"`. Без этого атрибута блокировка отрисовки не происходит.
1. Ожидает парсинга указанного элемента. Браузер блокирует отрисовку до тех пор, пока элемент с указанным ID не будет найден в DOM-дереве и распарсен.
1. Учитывает атрибут `media`. Если он указан, то блокировка применяется только при соответствии текущим условиям (например, размеру экрана).
...
- при добавлении `<link rel="expect">` в документ во время парсинга;
- при изменении атрибутов `href` или `media` до завершения парсинга целевого элемента.

Здесь браузер задержит отрисовку, пока не загрузит элемент с id="lead-content".
...
Если у элемента с `rel="expect"` меняется `id`:

- ID элемента изменён до парсинга: Блокировка применяется к новому ID (если `href` обновлён).
- ID элемента изменён после парсинга: Блокировка уже завершена, изменения не влияют.
- Элемент с указанным ID не найден: Блокировка может никогда не завершиться (переход не запустится).
- Несколько элементов с одним ID: Поведение не определено, рекомендуется уникальность ID.
