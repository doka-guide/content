...
Атрибут `rel="expect"` используется с элементами `<link>`, чтобы указать, что связанный ресурс критически важен для первоначальной отрисовки страницы. В сочетании с атрибутом `blocking="render"` он задерживает отображение страницы до загрузки и обработки ресурса. Это помогает избежать проблем, когда часть контента ещё не готова, а отрисовка страницы уже началась.

Когда браузер встречает `<link rel="expect" blocking="render">`, он:

1. Извлекает URL из атрибута `href`.
1. Проверяет корректность URL. Если ресурс не удалось загрузить, отрисовка разблокируется — страница показывается пользователю даже без этого ресурса.
...
```html
<head>
  <link rel="expect" blocking="render" href="critical-styles.css" media="screen">
</head>
```
