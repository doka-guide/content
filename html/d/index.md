...
| `S`     | (`x2` `y2` `x` `y`)+  | Кубическая кривая Безье от текущей позиции до `x, y`, где `x2, y2` — координаты конечной опорной точки. Рисует более гладкую кривую за счёт того, что берёт координаты начальной опорной точки `x1, y1` на основании предыдущей команды. Если предыдущая команда была кривой Безье, то опорная точка будет отражением конечной опорной точки предыдущей кривой. В другом случае опорная точка совпадёт с текущей позицией. |
| `s`    | (`dx2` `dy2` `dx` `dy`)+ | Кубическая кривая Безье от текущей позиции до точки с координатами `текущий x + dx, текущий y + dy`, где конечная опорная точка будет иметь координаты `текущий x + dx2, текущий y + dy2`. Рисует более гладкую кривую благодаря тому, что берёт координаты начальной опорной точки `x1, y1` на основании предыдущей команды. Если предыдущая команда была кривой Безье, то опорная точка станет отражением конечной опорной точки предыдущей кривой. В противном случае  опорная точка будет совпадать с текущей позицией. |

Красными точками (`fill="red"`) отмечены опорные, начальные и конечные точки кривых. Синим цветом (`fill="blue"`) отмечена промежуточная опорная точка второй кривой, которую получили отражением промежуточной опорной точки первой кривой.
...
| `t`     | (`dx` `dy`)+                   | Квадратичная кривая Безье от текущей позиции до `текущий x + dx, текущий y + dy`. Рисует более гладкую квадратичную кривую Безье благодаря тому, что берёт координаты опорной точки `x1, y1` на основании предыдущей команды. В случае, если предыдущая команда была кривой Безье, опорная точка станет отражением конечной опорной точки предыдущей кривой. Иначе опорная точка совпадёт с текущей позицией. |

Красные точки (`fill="red"`) — опорные, начальные и конечные точки кривых. Синим цветом (`fill="blue"`) отмечена промежуточная опорная точка второй кривой, которую получили отражением промежуточной опорной точки первой кривой.
...
| Команда | Параметры                      | Описание                                                                                                   |
| ------- | ------------------------------ | ---------------------------------------------------------------------------------------------------------- |
| `A`     | (`rx` `ry` `angle` `large-arc-flag` `sweep-flag` `x` `y`)+ | Эллиптическая кривая с горизонтальным радиусом `rx` и вертикальным радиусом `ry`, которая начинается в текущей точке и завершается в точке с координатами `x, y`. `angle` отвечает за поворот кривой относительно оси абсцисс. Флаг `large-arc-flag` отвечает за выбор большей дуги при значении `1` и меньшей при значении `0`. Флаг `sweep-flag` отвечает за выбор направления дуги: `1` — по часовой стрелке, `0` — против часовой стрелки. |
| `a`     | (`rx` `ry` `angle` `large-arc-flag` `sweep-flag` `dx` `dy`)+ | Эллиптическая кривая с горизонтальным радиусом `rx` и вертикальным радиусом `ry`, которая начинается в текущей точке и завершается в точке с координатами `текущий x + dx, текущий y + dy`. `angle` отвечает за поворот кривой относительно оси абсцисс. Флаг `large-arc-flag` отвечает за выбор большей дуги при значении `1` и меньшей при значении `0`. Флаг `sweep-flag` отвечает за выбор направления дуги: `1` — по часовой стрелке, `0` — против часовой стрелки. |

Лучше понять как флаги влияют на отображение кривой помогут эти демки. Красными точками (`fill="red"`) отмечены начальная и конечная позиция кисти при рисовании кривой. Белым цветом отмечена дуга (`stroke="white"`), которая нарисована с указанными значениями флагов. Серым отмечены остальные комбинации флагов.
