...
Тегом `<option>` размечается каждый элемент выпадающего списка [`<select>`](/html/select), группы опций [`<optgroup>`](/html/optgroup) (в работе) или перечня [`<datalist>`](/html/datalist) (в работе).
