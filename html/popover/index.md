...
Удобный способ создавать всплываютщие элементы. Новая возможность HTML. Имеет много встроенных фишек типа закрытия по клику мимо элемента или по нажатию клавиши <kbd>Esc</kbd>.
...
_Поповер_ — это всплывающие элементы в интерфейсе: всплывающие подсказки, раскрывающиеся меню, превью товаров и другие. Поповеры относят к группе попапов. В неё же включают, например, [`<dialog>`}(/html/dialog/). 

![](./images/popups.jpg)

Долгое время в HTML не существовало простого способа реализовать поповеры. Самодельные решения требовали большого внимания к мелочам и часто не учитывали всех сценариев использования. Теперь при помощи нескольких атрибутов можно создать правильную, удобный для всех всплывающий элемент.
...
- `popover` — указывает, что блок теперь будет вести себя как поповер.
- `id` — уникальный идентификатор блока, используется для показа поповера.
- `popovertarget` — связывает элемент, кнопку или другой HTML-тег с поповером. В качестве значения используется `id` элемента, который хотим показать.
...
<div id="my-popover" popover>
  <p>Я — поповер с дополнительной информацией!</p>
</div>
```
...
В примере ниже кнопка «Показать поповер» будет только открывать всплывающий элемент, но не закрывать его. Для закрытия элемента внутрь добавлена кнопка закрытия.
...
- `auto` — значение по умолчанию, не требуется прописывать явно. При открытии поповера закроются все остальные всплывающие элементы на странице. Поповер может быть закрыт по клику за пределами элемента.
- `manual` — другие всплывающие элементы не закрываются. Поповер не может быть закрыт по клику за его пределами.
...
В чём отличие поповеров от тоже довольно нового [`<dialog>`](/html/dialog/)? Кажется, что оба создают всплывающие окна.
...
Для большинства сценариев работы с поповерами совсем не нужен JavaScript. Но если есть необходимость, то можно использовать следующие методы:
