...
Одно из самых популярных вспомогательных устройств — скринридер 🤖
...
Пользователи могут не только изменить движок или скорость речи, но ещё выбрать другие настройки пунктуации, объёма объявлений, голоса, способы навигации и др.
...
Ещё Accessibility API получает от браузеров сообщения о событиях на странице, обрабатывает их и помогает скринридерам на них подписаться. Это может быть всплытие окна с ошибкой, открытие или закрытие выпадающего списка или выбор чекбокса.
...
Это представление элементов документа в виде дерева на основе [DOM](https://doka.guide/js/dom/) (Document Object Model). Похоже на [DOM-дерево](https://doka.guide/js/dom/#iz-chego-sostoit-dom), только состоит не из HTML-элементов, а из _доступных объектов (accessible object)_.
...
Во многие [семантические теги](https://doka.guide/html/#semantika) роли уже встроены. Роль `<a>` — `link`, `<header>` — `banner`, `<ul>` — `list`. Полный список найдёте на [странице с HTML-элементами и именами](https://russmaxdesign.github.io/html-elements-names/). Поэтому для доступности важна семантическая вёрстка. Если вместо `<button>` использовать `<div>` с событием `onclick`, то у него не будет роли кнопки и её поведения. Пользователи скринридеров не узнают, что элемент кликабельный.
...
![В узел дерева доступности попала информация о свойствах картинки. name: Пример клавиатуры для поля ввода номера телефона. role: graphic. DOMNode: img.](images/2.png)
...
1. Браузер ищет и обрабатывает событие с учётом контекста.
1. Представим, что на сайте есть скрипт, который отслеживает события. В этом случае он выполняется и происходит нужное действие при клике на кнопке.
...
Пользователи мобильных скринридеров для перемещения по странице проводят пальцем по экрану, слушают объявления и так проходят через все элементы. Для взаимодействия с элементом нужно два раза тапнуть по экрану, а для перехода от одного к другому — свайпнуть вправо или влево. В мобильных скриридерах с помощью жестов тоже можно открыть навигацию по заголовкам, строкам, словам или ссылкам.
...
- [Semantics to Screen Readers](https://alistapart.com/article/semantics-to-screen-readers/), Мелани Ричардс.
- [Accessibility APIs: A Key To Web Accessibility](https://www.smashingmagazine.com/2015/03/web-accessibility-with-accessibility-api/), Леони Уотсон.
- [How A Screen Reader User Surfs The Web](https://youtu.be/OUDV1gqs9GA), видео с Леони Уотсон.
- [How I use technology as a blind person](https://youtu.be/TiP7aantnvE), видео Молли Берк.
- [Browsing with a desktop screen reader](https://tetralogical.com/blog/2021/09/29/browsing-with-a-desktop-screen-reader/), Хенни Сван.
- [Browsing with a mobile screen reader](https://tetralogical.com/blog/2021/10/05/browsing-with-a-mobile-screen-reader/), Хенни Сван.
