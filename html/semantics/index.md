...
- [`<footer>`](/html/footer/) — дополнительная информация о сайте или части одной страницы — подвале (футере). Тоже не привязан к части страницы или отдельной секции. Например, подвал может быть у превью поста и содержать информацию о дате публикации и авторе.

В этом примере карточки у `<article>` есть дата начала старта курса, которую удобно поместить в подвал:
...
Контентные теги точечно описывают смысл частичек и типов содержимого — контента. Перечислим только некоторые теги, в реальности их гораздо больше:
...
Поговорим поподробнее про пользу семантики для доступности. Зрячим пользователям обычно легче ориентироваться по сайту, чем тем, кто его не видит. Люди со слепотой и слабовидящие используют [скринридеры](/a11y/screenreaders/), экранные лупы и другие вспомогательные технологии для взаимодействия со страницами.
...
🪧 `<header>`, `<footer>`, `<section>` и другие элементы становятся доступными для навигации, если они напрямую вложены в [`<body>`](/html/body/).
...
Семантика дополняет другие факторы ранжирования и раскрывает больше информации о структуре сайта, темах страниц и связях между разными видами контента. Поисковые роботы точнее определяют главную тему страницы по заголовку `<h1>` и атрибуту [`title`](/html/title/). Ряд семантических тегов наводит мосты между типами контента. Например, закрытые субтитры к видео в `<track>`  поисковым роботам больше информации и железно связывают само видео с текстом субтитров.

<details>
  <summary>Другие факторы ранжирования</summary>
...
Изображения, бережно описанные в атрибуте `alt`, увеличивают шансы попасть в выдачу по картинкам. Графика индексируется наравне с другим контентом. Google и Bing совмещают индексацию альтернативных описаний, добавленных пользователями, и компьютерное зрение для автоматических описаний. У картинок с ручными альтернативными описаниями больше преимуществ, чем у сгенерированных искусственным интеллектом. Это снова связано с пониманием контекста. В отличие от роботов, вы точно знаете, что передавали картинкой с мемом про собаку на фоне пожара 🐕
...
🐳 Производительность сайтов стоит на трёх китах. Это скорость загрузки основного контента (LCP), скорость реакции страницы на действия пользователей (INP) и скорость, с которой элементы занимают свои места в загружающемся интерфейсе (CLS).
...
**Вклад в будущие поисковые алгоритмы**. Всё течёт, всё меняется, и SEO тоже. Компании разрабатывают новые алгоритмы поиска, основанные на искусственном интеллекте. Например, Microsoft уже запустила [поумневший движок Bing](https://blogs.microsoft.com/blog/2023/02/07/reinventing-search-with-a-new-ai-powered-microsoft-bing-and-edge-your-copilot-for-the-web/). Google работает над новым движком [«Проект "Маги"»](https://seo.ai/blog/google-project-magi) 🪄
...
Выбрать тег поможет правило большого пальца. Если задаёте дивам классы вроде `header`, `sidebar`, `content`, `post` и `footer`, подумайте, не подходят ли больше элементы `<header>`, `<aside>`, `<main>`, `<article>` и `<footer>`. Из любого правила есть исключение: контекст интерфейса и содержимого важнее.
...
![Устройство блока с карточкой.](images/example-semantic-markup.png)
