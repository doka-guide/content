...
![На карточке два прямоугольника. В оба из них вписаны четыре других прямоугольных блока. Слева прямоугольники подписаны тегами div с классами header, sidebar, content и footer. Справа в прямоугольнике правильная семантическая вёрстка, где div были заменены на теги header, nav, main и footer](images/example-semantic-markup.jpg)
...
- **Читаемость исходного кода**. Когда верстальщик делает сайт, в дальнейшем этот сайт может изменяться и поддерживаться в дальнейшем. Через год, когда клиент попросит внести правки в исходный макет, то проще найти `<button>` в вёрстке, чем `<div>`.
