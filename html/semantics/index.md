...
Теги и их встроенные роли перечислены в [спецификации ARIA в HTML (ARIA in HTML)](https://www.w3.org/TR/html-aria/#docconformance).
...
Кто-то может подумать, что раз даже [`<div>`](/html/div/) и [`<span>`](/html/span/) семантические, расставлять теги кнопок, ссылок и списков долго и бессмысленно. Всё прямо наоборот: у вёрстки, которая подробно описывает содержимое страниц, больше преимуществ, чем у «дивной».
...
☝️ `<header>`, `<footer>`, `<section>` и другие элементы становятся доступными для навигации, когда вложены прямо в [`<body>`](/html/body/).

</aside>

Пользователи скринридеров перемещаются и по заголовкам. Это распространённый [метод поиска информации по странице](https://webaim.org/projects/screenreadersurvey10/#finding).

Другая неочевидная польза семантики — вам не придётся стилизовать сайт отдельно под [режим принудительных цветов](/a11y/forced-colors/), например, режим высокой контрастности в Windows. В его случае количество цветов ограничивается до небольшой палитры. Если кнопка свёрстана на [`<div>`](/html/div/), система не заменит цвет элемента на системный.
...
🅰️ _Бионическое чтение_ ускоряет взаимодействие с текстом за счёт выделения первых букв слов. Так взгляд перемещается словам гораздо быстрее, без необходимости читать их целиком.
