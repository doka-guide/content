...
- [`<header>`](/html/header/) и [`<footer>`](/html/footer/) — теги для создания «шапки» и «подвала» или логического блока на странице. `<footer>`, например, используют не только как обёртку для «подвала» сайта, но и как метаданные в карточке статьи.
