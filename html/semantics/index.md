...
Кроме семантических тегов, выделяющих целые разделы документа, есть ещё и контентные семантические теги. Например:

- [`<h1>`-`<h6>`](/html/h1-h6/) — теги, предназначенные для определения заголовков на странице. `<h1>` используется для самого важного заголовка в документе, является основным заголовком всего документа или раздела. Остальные теги служат для подзаголовков, обозначая иерархию и структуру контента.
- [`<button>`](/html/button/) — элемент для создания кнопок на веб-страницах. Может использоваться для отправки форм, выполнения команд или вызова функций через JavaScript.
- [`<img>`](/html/img/) — тег для вставки изображений в документ.
- [`<table>`](/html/tables/) — используется для создания таблиц на веб-страницах. Состоит из ряда элементов для структурирования данных: `<tr>` для строк, `<th>` для заголовочных ячеек и `<td>` для стандартных ячеек.
- [`<ul>`](/html/ul/) — определяет неупорядоченный список, который обычно отображается с маркерами.
- [`<a>`](/html/a/) — тег для создания гиперссылок, позволяющих пользователям переходить от одного документа к другому.
- [`<p>`](/html/p/) — тег выделяющий абзац текста. Абзацы являются основными блоками текста в HTML и используются для разделения контента на управляемые, логически разграниченные секции, что делает текст легче для чтения и понимания.
...
- **Уменьшение исходного кода**. Семантические теги, которые несут в себе какую-то функциональную ценность, такие как `<button>`, [`<input>`](/html/input) обладают встроенными в браузер обработчиками событий, которые разработчику не нужно предусматривать. На примере всё той же кнопки — нажатие клавиш <kbd>Space</kbd> и <kbd>Enter</kbd>. Если создавать кнопку через тег `<div>`, то придётся предусматривать эти события самостоятельно.
...
![Две схемы разметки одной страницы.](images/example-semantic-markup.png)
