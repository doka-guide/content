...
<footer>Подписывайте на нас в Инстаграме</footer>
```
...
На странице может быть только один `<main>`. Точнее, [спецификация уточняет](https://html.spec.whatwg.org/multipage/grouping-content.html#the-main-element):

> В документе не должно быть больше одного `<main>`, у которого нет атрибута `hidden`

Получается, что можно иметь и другие `<main hidden>`, но это равносильно тому, что их не будет на странице. Это исключение сделано специально для навигации по модели одностраничных приложений (SPA).
