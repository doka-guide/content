...
`method` — определяет, каким способом будут отправлены на сервер данные, которые ввёл пользователь. Есть три варианта:
...
- `dialog` - если `form` находится внутри [`dialog`](/html/dialog), то поля формы не очищаются, а сама форма не делает запроса на сервер. Вместо этого, закрывается [диалоговое окно](/html/dialog).
