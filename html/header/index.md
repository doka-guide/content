...
💡 `<header>` может содержать заголовок, а может не содержать. Если заголовок `h1-h6` указан в `<header>`, то он будет формировать семантическую раскладку `Outline` страницы. `Outline` — это все заголовки всех уровней в виде дерева вложенностей (как оглавление):
