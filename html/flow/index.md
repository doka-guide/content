...
- Переопределить родителю свойство [`display`](/css/display/) на `flow-root`, либо на `flex` или `grid` (но в последнем случае его потомки будут раскладываться уже не по правилам нормального потока, а по правилам [флексбоксов](/css/flexbox-guide/) и [грид-раскладки](/css/grid-guide/) соответственно).
