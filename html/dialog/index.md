...
Атрибут closedby — это новый экспериментальный атрибут, который появляется у `<dialog>` после его закрытия. Он позволяет определить, какой пользовательский элемент или действие привело к закрытию окна.
