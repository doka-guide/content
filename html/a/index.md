...
💡 Ссылка не всегда по умолчанию становится строчным элементом. По спецификации — ссылка один из немногих элементов с прозрачной контентной моделью: в зависимости от родителя, ссылка может получить как `display: inline;` (например родитель блочный или строчный), так и `display: block;` (родитель `flex` или `grid`). Ссылкам-детям `flex`- и `grid`-контейнеров можно спокойно задать ширину, высоту, а также `padding-top` и `padding-bottom`, в то время как для ссылки-ребенка блочного родителя эти свойства действовать корректно не будут, как и для любого другого `inline`-элемента.
