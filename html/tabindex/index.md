...
Обычно в сложных компонентах, как в панели вкладок, используют трюк с перемещающимся `tabindex`. Это когда у элемента в фокусе нулевое значение, а у элементов без фокуса пока что отрицательное.
...
1. Когда пользователь сделал фокус на другом элементе, для элемента в фокусе по умолчанию устанавливаем `tabindex="-1"`, а для новоого — `tabindex="0"` и фокус при помощи `.focus()`.
1. Если пользователь вышел из компонента, оставляем `tabindex="0"` у последнего элемента, с которым взаимодействовал пользователь.
