...
В панели вкладок контейнеру с содержимым с [ролью `tabpanel`](/a11y/role-tabpanel/) часто задают `tabindex="0"`.
...
`tabindex` пригодится и в модальных окнах. Например, когда в окне много текста, первым элементом в фокусе делают первый параграф. Для этого ему тоже задают `tabindex="-1"`.
