...
Большим преимуществом установки правильного значения для `type` является то, что в мобильных браузерах будет появляться "специальная" клавиатура, упрощающая и улучшающая пользовательский опыт. Так, для `<input type=tel>` у пользователя отобразится клавиатура, содержащая лишь цифры и специальные символы, нужные для корректного ввода номера телефона. Пример такой клавиатуры можно увидеть на картинке ниже:
