...
- `color` — виджет для выбора цвета. Ингда его называют колорпикер (от английского colorpicker).
- `file` — выбор файла для отправки.

И отдельно стоит значение `hidden`. Поле ввода с этим значением атрибута `type` не видно на странице, но в то же время отправляет своё значение на сервер.
