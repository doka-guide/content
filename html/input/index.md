...
- [`pattern`](/html/pattern/)- задаёт регулярное выражение по которому будет проверяться значение элемента `<input>`.
- [`multiple`](/html/multiple/) - атрибут, который указывает, разрешено ли пользователю указывать более одного значения.
