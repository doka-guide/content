...
- `size` — устанавливает ширину для элементов ввода данных (`<input type="text | password | email | search | url | number">`). Единица измерения `size="1"` соответствует ширине одного символа набранного моноширинным (`font-style: monospace`) шрифтом. По умолчанию `size="20"`. При изменении `css` свойства `font-size` ширина и высота элемента ввода тоже изменится.
