...
Тег `<input>` позволяет создавать интерактивные элементы на сайте — такие как поле для ввода текста, кнопка, ползунок, переключатель и другие.
...
- `tabindex` — когда пользователь нажимает кнопку <kbd>Tab</kbd>, фокус падает на следующий интерактивный элемент на странице. `tabindex` задаёт порядковый номер для этого элемента. Например, чтобы перевести фокус на `<input tabindex="3">`, нужно нажать <kbd>Tab</kbd> три раза.
...
💡 Чтобы даже те, кто использует специальные средства для чтения с экрана, могли точно понять, что делает тот или иной `<input>`, используйте этот тег в паре с `label`. Вот так:

```html
<label for="peas">Это подпись, которая подскажет, что делает input ниже</label>
<input type="checkbox" name="peas" id="peas">
```

Для этого задайте `<input>` атрибут `id`, а `<label>` — атрибут `for` с таким же значением, что у `id`.
