...
🛠 Может показаться, что `<details>` и `<summary>` подходят для бургерного меню. Они ведут себя похоже, но это не самое доступное решение для выпадающего меню. Вот несколько причин:

- Пользователи голосового управлению не смогут открыть такой элемент с помощью привычных команд. К примеру, «Нажать на кнопку».
- Скринридеры не всегда могут понять, раскрыт сейчас такой элемент или нет.
...
- У пользователей скринридеров нет шорткатов для быстрого перехода к `<details>` или `<summary>`.
