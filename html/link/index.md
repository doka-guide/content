...
- `crossorigin` — указывает должен ли браузер отправлять кросс-доменные запросы с заголовками [CORS](/tools/cors/) при загрузке файлов. Данный атрибут имеет два доступных значения:
  - `anonymous` —  Cross-origin запрос выполняется без отправки учётных данных, например куки или заголовки аутентификации. Это может быть полезно для загрузки статических файлов, таких как изображения или шрифты;
...
При использовании атрибута `crossorigin` в теге `link`, браузер будет проверять, поддерживает ли сервер заголовки [CORS](/tools/cors/) для запрашиваемого ресурса. Если сервер не поддерживает [CORS](/tools/cors/) или не возвращает нужные заголовки, то браузер может блокировать загрузку ресурса.
