...
Тег `<param>` позволяет передавать параметры в объекты, встроенные на страницу с помощью тега [`<object>`](/html/object/). Использовался для указания значений, с которыми должен запускаться мультимедийный плагин или другой внешний ресурс.
