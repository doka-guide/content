...
- `formnovalidate` — булевый атрибут, работает как `novalidate` для тега [`<form>`](/html/form/).
...
💡 Тег [`<input>`](/html/input/) с атрибутом `type="button | reset | submit"` тоже создаёт кнопку, но `<button>` проще стилизовать, так как внутрь `<button>` можно добавить любой HTML-контент, например, `<em>`, [`<strong>`](/html/strong/) или [`<img>`](/html/img/).
