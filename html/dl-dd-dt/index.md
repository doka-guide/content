...
У тега `<dl>` есть встроенная [роль `list`](/a11y/role-list/), у `<dt>` — [`term`](/a11y/role-term/), а у `<dd>` — [`definition`](/a11y/role-definition/). Это делает теги полезными для [скринридеров](/a11y/screenreaders/).
...
Скринридеры взаимодействуют `<dl>` по-разному, и это не баг, а фича. Например, есть отличия в том, как они считают количество вложенных терминов `<dt>`, и как пользователя перемещаются между всеми элементами внутри списка.

Также не стоит задавать тегу `<dl>` явную роль `term`, чтобы преодолеть его странности взаимодействия с тегом. Это плохо влияет на мобильный и десктопный VoiceOver.
