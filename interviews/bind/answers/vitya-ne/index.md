...
При вызове [`.bind()`](/js/bind-call-apply/#bind) создаётся новая функция-обёртка (bound function). Эта функция хранит требуемый `this` и приоритетные аргументы и защищена от возможности изменить эти параметры при вызове. В дальнейшем вызов функции-обёртки приводит к вызову целевой (target) функции.
...
boundShow()
// имя: Румпельштильцхен, данные: true undefined

boundShow(false)
// имя: Румпельштильцхен, данные: true false

boundShow.call({name: 'Риннебист'}, 2)
// имя: Румпельштильцхен, данные: true 2
```
