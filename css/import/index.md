...
1. buttons.css — файл со стилями для кнопок.
1. main.css — основной файл стилей с остальным CSS-кодом.
...
```css
@import "path-to-file/file.css";
@import url("path-to-file/file.css");
```
...
```css
@import url("path-to-file/file.css") print;
@import "path-to-file/file.css" screen;
```
...
```css
@import "path-to-file/file.css" (min-width: 481px) and (max-width: 768px);
@import url("path-to-file/file.css") screen and (orientation: landscape);
```
...
```css
@import url("path-to-file/file.css") supports(not (display: flex));
