...
Что происходит, если на элемент одновременно наложены две анимации и обе меняют одно и то же свойство, например, `transform`? До 2023 года браузер просто ничего не гарантировал: результат зависел от порядка анимаций и мог отличаться в разных браузерах.

Свойство `animation-composition` явно определяет, как анимации складываются друг с другом:

- `replace` — значение по умолчанию. Анимация, идущая позже в списке, полностью заменяет результат предыдущей.
- `add` — эффекты анимаций складываются. Два `translate` сложатся в один общий сдвиг, а не заменят друг друга.
- `accumulate` — похоже на `add`, но для однотипных функций трансформации складывает их параметры по смыслу самой функции.
...
Значение можно задать отдельно для каждой анимации из списка, через запятую, в том же порядке, что и сами анимации.

## Анимации по скроллу

Всё, что мы разбирали выше, это анимации, где прогресс зависит от времени. Но браузер умеет привязывать прогресс анимации не ко времени, а к прокрутке страницы или к позиции элемента во вьюпорте. Такие анимации называют scroll-driven.

За это отвечает свойство `animation-timeline`. У него есть несколько вариантов значения:

- `auto` — значение по умолчанию, анимация работает как обычно, по времени.
- `scroll()` — прогресс анимации привязывается к прокрутке контейнера (по умолчанию — ближайшего скроллящегося родителя).
- `view()` — прогресс анимации привязывается к тому, насколько элемент виден во вьюпорте: анимация начинается, когда элемент входит в область видимости, и заканчивается, когда он её покидает.
...
Границы, в которых должна уложиться анимация, можно сузить свойством `animation-range` (и его частями `animation-range-start`, `animation-range-end`). Например, `animation-range: entry 0% cover 40%` означает: начать анимацию в момент входа элемента во вьюпорт и закончить, когда он «накроет» 40% отслеживаемой дистанции.

<iframe title="Появление карточек по скроллу через animation-timeline: view()" src="demos/scroll-appear/" height="450"></iframe>

На середину 2026 года `animation-timeline`, `scroll()`, `view()` и `animation-range` не поддерживаются везде. Chrome, Edge и Safari 26+ их поддерживают, а Firefox — пока нет. Используйте эти свойства как прогрессивное улучшение и проверяйте поддержку через [`@supports`](/css/supports/), а не как единственный способ показать контент. Например, покажите карточки сразу видимыми, если браузер не понимает `view()`:
