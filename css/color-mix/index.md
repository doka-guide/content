...
1. Название цветового пространства (обязательно) — система координат для описания цвета.
1. Метод интерполяции оттенка (необязательно, только для полярных пространств) — определяет путь перехода от одного оттенка к другому.
...
Подробнее про некоторые цветовые пространства можно прочитать в статье «[Цвета в вебе](https://doka.guide/css/web-colors/)».
...
- Функции: [`rgb()`](/css/web-colors/#rgb-rgba), [`rgba()`](/css/web-colors/#rgb-rgba), [`hsl()`](/css/web-colors/#hsl), [`hsla()`](/css/web-colors/#hsl), [`hwb()`](/css/web-colors/#hwb), `lab()`, `lch()`, `oklab()`, `oklch()`
...
.badge--muted {
  ...
  background: color-mix(in oklab, var(--bg) 60%, transparent 40%);
}
```
