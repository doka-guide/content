...
`@font-feature-values` позволяет дать понятные имена загадочным кодам вроде `ss01` или `cv02` для конкретного шрифта (семейства). Затем эти имена можно использовать в [`font-variant-alternates`](/css/font-variant-alternates/), не вспоминая номера `ss01`, `cv12` и т. п.
...
- `family-name` — одно или несколько имён семейства шрифта в кавычках. Имена должны точно совпадать с [`font-family](/css/font-family/)`.
...
- Далее эти идентификаторы используются в `font-variant-alternates` как функции: `styleset(name)`, `character-variant(name)` и так далее.
...
💡 Для кросс‑браузерности можно использовать как fallback через [`font-feature-settings`](/css/font-feature-settings/), если `font-variant-alternates` не поддерживается.
