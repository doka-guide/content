...
Пространство имён (`namespace`) — это механизм, который позволяет однозначно идентифицировать элементы и атрибуты, чтобы избежать конфликтов между разными технологиями, использующими одинаковые имена тегов. Представьте, что это "фамилия" для тегов: тег `<a>` может быть ссылкой в HTML, но в SVG это тоже ссылка (но с другим поведением и атрибутами). Пространство имён говорит браузеру: "Этот `<a>` — из семейства HTML, а этот — из семейства SVG".
