...
- `<anchor-name>` — имя якоря, относительно которого нужно спозиционировать текущий таргет элемент. Необязательный аргумент, если его опустить, то функция будет ссылаться на значение указанное в свойстве [`position-anchor`](/css/position-anchor/);
- `<anchor-side>` — сторона якорного элемента, относительно которой нужно спозиционировать текущий таргет элемент;
- `<length-percentage>` – фолбэк-значение, применится, если функция не нашла якорный элемент, на который мы пытаемся сослаться, или если передали некорректное значение для `<anchor-side>`. Необязательный аргумент.
