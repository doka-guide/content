...
С помощью этого свойства можно с легкостью задавать отступы между строками и столбцами.

Является краткой записью свойств `row-gap` и `column-gap`.

Возможные значения:

- `длина` — стандартные значения (`px`, `em`, `rem`, `vmin`  и тд).
- `проценты` — значение в процентах.
- `calc` — функция для вычисления размера.

- `initial` — применяет значения по умолчанию.
- `inherit` — значение отступа наследуется от родителя.
- `unset` — удаляет текущий отступ у элемента.
