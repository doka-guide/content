...
💡 Старайтесь **использовать** для анимации в первую очередь свойства `transform` и `opacity` — они самые производительные, потому что не вызывают [`Layout`](/js/how-the-browser-creates-pages/#vychislenie-pozicii-i-razmerov-layout) и [`Paint`](/js/how-the-browser-creates-pages/#neposredstvenno-otrisovka-paint). Старайтесь не изменять свойства `left`, `top`, `margin`, `padding`, `width`, `height` и т.д.
