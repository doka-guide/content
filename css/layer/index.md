...
Директива `@layer`, а также дополнительные функция `layer()` и ключевое слово `layer` дают вам управлять каскадными слоями.
...
Слои можно вкладывать в друг друга. Слой с другими внутри называется родительским, а вложенные в него — дочерними. Обратите внимание, что дочерний слой не может «выйти за пределы» своего родителя. Его не связать с другими дочерними и даже родительскими каскадными слоями.
...
У дочерних слоёв могут быть такие же имена, как у других за пределами его родителя. К примеру, здесь два совершенно разных слоя с именами `base` и `framework.base`, так как второй слой `base` находится внутри `framework`.
...
Слои можно перечислять в одном месте. Расположите их имена через запятую после директивы `@layer`. В примере собрали два слоя в одной строке и получили `@layer layer-1, layer-2`.
...
Когда перечисяете дочерние слои в одну строку, сначала укажите название родительского, а через точку — дочернего. К примеру, у нас есть родительский слой `framework` и дочерние `base` и `theme`. При перечислении имена детей будут `framework.base` и `framework.theme`.
...
`layer` пригодится для применения анонимных слоёв при импорте стилей из другого файла. Это делают с помощью директивы [`@import`](/css/import/).
...
Анонимный слой можно расположить не только перед, но и после строки с `@import`. В этом случае вы
...
С помощью функции `layer()` обращаетесь к конкретному слою при импорте стилей из одного файла в другой.
...
В начале файла перечислим слои после `@layer`. Расположим их так, чтобы у слоя `tailwind` для перезаписи сторонних стилей более низкий приоритет, а у `application` для собственных стилей — самый высокий. На второй строке импортируем стили из _tailwind.css_ и перезапишем нужные слоем `tailwind` с помощью `layer()`. После этого уже определим нужные нам CSS-правила.
