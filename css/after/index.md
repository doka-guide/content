...
Когда мы в CSS добавляем `::after` к селектору, для соответствующего элемента создаётся [псевдоэлемент](/css/pseudoelements/). Этот псевдоэлемент — его самый последний потомок, он идёт **после** всего внутреннего содержимого.
