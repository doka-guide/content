...
`::file-selector-button` — псевдоэлемент, представляющий кнопку внутри поля [`<input type="file">`](/html/input/). Позволяет стилизовать её так же, как любую кнопку интерфейса: менять фон, бордеры, скругления, состояния наведения/фокуса и другие стили.
