...
Изменить толщину линии можно указав для неё значение в единицах измерения, или используя ключево слово `from-font`, которое берёт значение из файла шрифта.

Если из файла получить толщину не удалось, то будет установлено значение по умолчанию `auto`.
...
💡 Нельзя управлять точным положением линии, заданным при помощи `text-decoration`.

💡 Если по дизайну требуется задать тексту или ссылке подчёркивание, отличающееся от стандартного по положению, а также если нужно анимировать появление / пропадание линии, то используй псевдоэлементы [`::before`](/css/before/) или [`::after`](/css/after/).
