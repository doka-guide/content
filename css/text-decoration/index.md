...
💡 Если по дизайну требуется задать тексту или ссылке подчёркивание, отличающееся от стандартного по положению, то можно использовать свойство [`text-underline-offset`](/css/text-underline-offset/). Оно позволяет управлять положением по вертикали у линий со значением `underline`. Значение можно указать в единицах измерения: при положительном значении линия будет
опускаться, а при отрицательном подниматься.
