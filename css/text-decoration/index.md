...
💡 Если по дизайну для подчёркивания нужно задать отступ от текста, используйте свойство [`text-underline-offset`](/css/text-underline-offset/). Оно управляет положением линий со значением `underline` по вертикали. Величину можно указать в единицах измерения: при положительном значении линия опустится, а при отрицательном — поднимется.
