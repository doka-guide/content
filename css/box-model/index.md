...
Элементы со строчным (`display: inline`) или строчно-блочным (`display: inline-block`) отображениями подстраивают и ширину и высоту под вложенный контент, если для строчно-блочного (`display: inline-block`) отображения - при помощи `width` и `height` не задано иное.
