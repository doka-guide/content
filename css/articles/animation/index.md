...
Функция, позволяющая вручную установить любую временную функцию со сложными законами. Значения `x1` и `x3` должны находиться в диапазоне от `0` до `1` включительно. Задавая значения `x2` и `x4` меньше `0` или больше `1`, можно добиться эффекта пружины.
