...
1. Сначала браузер считает стили вне директивы, то есть мы сразу обеспечиваем себе полупрозрачность фона, с помощью свойства `background-color`.
1. Дальше браузер проверит, может ли он обработать свойство `backdrop-filter` со значением `blur(8px)`:
