...
Например, мы верстаем блок и используем свойство [`backdrop-filter`](/css/backdrop-filter/), которое может не поддерживаться старыми версиями браузеров. Именно здесь может пригодиться `@supports`:
...
1. Сначала браузер считает стили вне директивы, то есть мы сразу обеспечиваем себе полупрозрачность фона, которая поддерживаются везде (как будто ничего и не произошло).
1. Дальше браузер проверит, может ли он обработать свойство `backdrop-filter` со значением `8px`:
  - Если поддержка этого свойства есть, стили внутри директивы `@supports` применятся и добавят фильтр вместо полупрозрачного фона.
...
<iframe title="Фолбэк для backdrop-filter" src="demos/backdrop-filter-support/" height="400"></iframe>
