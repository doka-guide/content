...
Чтобы решить эту проблему был придуман Anchor Positioning, который позволяет один элемент привязать к другому, как к якорю (*anchor — англ. якорь*), а также декларативно описать его поведение: в каких местах относительно якоря может располагаться элемент, в какие моменты он должен быть виден или скрыт, а также, как размер элемента зависит от размера якоря.
...
Функция `anchor()` помогает привязать таргет элемент к одной из сторон якорного элемента. Она ссылается на параметры позиционирования якорного элемента и может быть применена только в свойствах [`inset`](/css/inset/).
...
`anchor-center` – новое значение для `justify` и `align` свойств, при котором таргет элемент центруется относительно якоря по соответствующей оси.
...
Свойство `position-try-fallbacks` задаёт альтернативные варианты расположения таргет элемента, которые браузер может попробовать применить, если таргет элемент перестал помещаться целиком в своём начальном положении, заданном в свойстве [`position-area`](#position-area).
...
- `<anchor-name>` — список якорных имён через запятую, видимость которых должна быть ограничена поддеревом текущего элемента. Например, `--a, --b`;
...
Разбор реализации такого тултипа можете найти рецепте [«Тултип»](/recipes/tooltip/#adaptivnyy-na-osnove-popover-api-i-css-anchor-positioning).
...
Где Anchor Positioning точно может пригодиться.

### UI-компоненты

Anchor Positioning может быть полезен при проектировании тултипа. Также можно реализовать и другие компоненты, части которых должны быть привязаны друг к другу. Например, меню. Попробуйте открыть несколько вложенных уровней меню и проскроллить к разным частям экрана — выпадающие списки смогут адаптироваться.

<iframe title="Меню на базе Anchor Positioning" src="demos/menu/" height="450"></iframe>

### Переходы

Также можно использовать Anchor Positioning для реализации классных переходов. Например, для ховера элементов навигации. Попробуйте навести последовательно на несколько ссылок в демке ниже — задний фон будет перетекать от одного элемента к другому.

<iframe title="Переходы по ховеру на элементы навигации, реализованный с помощью Anchor Positioning" src="demos/nav-with-transition/" height="320"></iframe>
...
На базе Anchor Positioning можно построить граф. Если добавить поверх драг-н-дроп, то вершины можно двигать в разные стороны и ничего не будет отваливаться.
...
Используя Anchor Positioning важно не забывать про доступность. Данное API позволяет только визуально связать элементы друг с другом. Описание семантической связи лежит на плечах разработчика.

## Шпаргалка

Чтобы лучше сориентироваться в дальнейшем, какое из CSS-свойств отвечает за какую фичу Anchor Positioning, собрали для вас таблицу-шпаргалку.
