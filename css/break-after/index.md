...
| `page`   | После нужного блока произойдёт разрыв страницы. |
...
| `avoid-region` | После основной области контента не будет разрыва страницы. |
| `region` | После основной области контента будет обязательный разрыв страницы. |

## Как понять

Чаще всего контент не нужно разбивать по страницам принудительно, чтобы не считать [верхние](/css/orphans/) и [нижние висячие строки](/css/widows/) (одиночные строки абзаца, оторвавшиеся на соседнюю страницу). В этом случае можно указать «мягкие» переносы содержимого на следующую страницу при помощи значения [`page`](/css/page/).
...
Свойство `page-break-after` устарело, его заменили на `break-after`. Для улучшения поддержки используйте их вместе: значения почти всегда совпадают. Единственное исключение: значение `page` у `break-after` эквивалентно `always` для `page-break-after`.
