...
Свойство `list-style-type` пригодится, когда нужно стилизовать `<li>`-элемент [`<ul>`](/html/ul)- или [`<ol>`](/html/doka/ol)-списка, если оформления по умолчанию (или через атрибут `type`) недостаточно. Ещё один популярный «приём» — вообще убрать маркер (или число, если это `<ol>`-список), чтобы скрыть внешний вид, но оставить семантику тегов.
...
<iframe title="Варианты list-style-type" src="demos/list-style-type.html"></iframe>
...
<iframe title="Список без маркера" src="demos/ul-no-marker.html"></iframe>
...
<iframe title="Пример со свойством content" src="demos/empty-content.html"></iframe>
