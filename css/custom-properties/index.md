...
Но что, если мы заранее не знаем, какой будет вложенность элементов? Можно задать кастомное свойство корневому элементу страницы [`<html>`](/html/html/), и тогда оно гарантированно будет доступно в каждом элементе страницы. Но обычно для этих целей используют псевдокласс [`:root`](/css/root/), который является псевдонимом для `<html>`:
