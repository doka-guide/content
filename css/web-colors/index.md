...
У `rgb()` есть альтернативное имя — `rgba()`. Это полный синоним: синтаксис и поведение идентичны, включая необязательный альфа-канал.
...
.rgba {
  ...
  color: rgba(from hwb(120deg 10% 20%) r g calc(b + 200) / alpha);

  /* Относительное значение без наследования параметра (b) */
  color: rgba(from hsl(0 100% 50%) r g 48 / alpha);
}
```
...
- R — интенсивность красного (от 0 до 255, записывается в шестнадцатеричном виде от 00 до FF).
- G — интенсивность зелёного (от 0 до 255, записывается в шестнадцатеричном виде от 00 до FF).
- B — интенсивность синего (от 0 до 255, записывается в шестнадцатеричном виде от 00 до FF).
- α — альфа-канал (от 0 или 0% до 1 или 100%).
...
![Два прямоугольника с одинаковым градиентом от белого к синему: у Oklab переход выглядит равномернее, чем у CIELAB](images/oklabch.png)
...
- L — светлота (от 0 или 0% до 1 или 100%).
- C — насыщенность (от 0 или 0% до 0.4 и выше или 100%).
- H — тон в градусах (от 0 до 360).
- α — альфа-канал (от 0 или 0% до 1 или 100%).
...
Без `color-scheme: light dark` на `html` или `:root` функция всегда возвращает первый (светлый) вариант — переключения по системной теме не будет.
