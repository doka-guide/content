🛠 Мой любимый приём это обёртка `.container` на одном `clamp()` вместо пачки медиавыражений. Раньше для ширины контента приходилось писать что-то вроде «до 600px — 100%, от 600 до 1200 — 90%, дальше фикс 1140px». С `clamp()` это одна строка: минимум, максимум и предпочтительное значение в процентах между ними.
