...
Бывают случаи, когда вам приходится иметь дело с элементами HTML-документа у которых заданы стили при помощи атрибута style.
Пример:

```html
<h1 class="someclass" style="color: red">Sometext</h1>
```

Вы не сможете перебить стиль `color` у элемента `<h1>` даже усилением веса селектора.
Пример:
...
В результате, цвет текста останется красным.
Вот тут модификатор `!important` сможет вам помочь переопределить стили.
Пример:
