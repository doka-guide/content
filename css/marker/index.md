...
Например, в нумерованном списке `<ol>` каждый элемент `<li>` по умолчанию нумеруется в порядке возрастания. А в ненумерованном списке `<ul>` каждый элемент `<li>` помечается маркером в виде точки.
...
⚠️ В Safari на данный момент псевдоэлемент `::marker` поддерживается лишь частично. Нормально работает только свойство `color` и свойства для работы с текстом. Точно не работают свойства `content` и `direction`. А также отсутствует поддержка анимаций и переходов.
