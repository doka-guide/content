...
⚠️ В браузере Safari частично поддерживается псевдоэлемент `::marker`. Нормально работает только свойство `color` и свойства для работы с текстом. Точно не работают свойства `content` и `direction`. А также отсутствует поддержка анимаций и переходов.<br>
Помимо браузера Safari, анимации и переходы не поддерживаются в браузере Firefox на Android.
