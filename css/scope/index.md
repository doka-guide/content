...
💡 Иногда `to (...)` используют не только для внутренних частей блока, но и для исключения внешних или вложенных компонентов, которые визуально находятся внутри разметки, но логически должны жить отдельно. Например, внутри статьи есть встроенный виджет комментариев, который не должен наследовать стили статьи:
...
```css
@scope (.article) to (.comments-widget) {
  p {
    color: #333;
    line-height: 1.6;
  }
...
```
В этом случае стили применяются ко всей статье, кроме блока `.comments-widget`, даже несмотря на то, что он находится внутри DOM-дерева статьи.
