...
1. Текст параграфа будет очень большим:
```css
p {
  font-size: xx-large;
}
...
2. Заголовок `<h1>` будет в 2.5 раза больше, чем текст вокруг него:
```css
h1 {
  font-size: 250%;
}
...
3. Размер текста внутри тега `<span>` будет равен 16px, не зависимо ни от чего:
```css
span {
  font-size: 16px;
}
