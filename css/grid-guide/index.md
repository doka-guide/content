...
Предостережение: не используйте гриды, если сайт должен корректно работать в старых браузерах. Если в вашем зоопарке есть старые браузеры, то проверяйте поддержку технологии на [Can I use](https://caniuse.com/css-grid).
...
.container {
  ...
  grid-gap: 10px;
  /* Важно указать, что элементы должны вставать в колонки */
  grid-auto-flow: column;
}

.item {
  /* Чтобы элементы без контента были видны */
  min-height: 50px;
  background: lightblue;
}
