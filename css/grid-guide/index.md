...
В тех случаях, когда количество колоннок или строк не известно для свойств `grid-template-columns` и `grid-template-rows` можно установить значения `auto-fill` или `auto-fit` в формате:
...
```
Оба эти параметра (`auto-fill` и `auto-fit`) автоматически добавляют в grid-сетку из примера выше - колонки, но ведут себя немного по-разному:

`auto-fill` стремится заполнить колонками все доступное пространство, а когда элементы заканчиваются, создает пустые колонки, равномерно распределяя доступную область между существующими и "виртуальными" колоннками. `auto-fit` действует похожимо образом, но в отличие от `auto-fill` схлопывает пустые и отдает больше места под заполненные.
