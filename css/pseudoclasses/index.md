...
Применяется ко всем элементам, которые могут иметь атрибут `href` ([`<a>`](/html/a/), [`<area>`](/html/area/) и [`<link>`](/html/link/)). `:link` характеризует ещё не посещённые страницы, `:visited` — наоборот, посещённые (в рамках одного домена).
...
Применяется к элементам формы (`<input type="radio">`, `<input type="checkbox">`, [`<option>`](/html/option/) и [`<button>`](/html/button/)), у которых можно задать начальное состояние.
...
Позволяет найти элементы по направлению текста в нём (например, в арабском направление письма идёт справа налево). С начала 2024 года псевдокласс [поддерживается во всех основных браузерах](https://caniuse.com/css-dir-pseudo).
...
Позволяет выбрать элемент, уточнив основной селектор дополнительным, при этом не увеличив его вес. В отличие от привычных селекторов, правило применится только к тому селектору, который был описан **до** `:has()`. С начала 2024 года [поддерживается всеми основными браузерами](https://caniuse.com/css-has).
...
Выделим красной рамкой все [`<img>`](/html/img/), которым забыли прописать атрибут `alt`:
...
`:optional` находит любой `<input>`, у которого не установлен атрибут [`required`](/html/required/) — то есть находит необязательные поля ввода. А `:required` — наоборот, те `<input>`, у которых есть этот атрибут.

### `:read-only` и `:read-write`

Находит элементы, недоступные (`:read-only`) или наоборот (`:read-write`) для редактирования. Например, ориентируется на наличие атрибута `disabled` или [`readonly`](/html/readonly/).

### `:root`

[Основная статья](/css/root/).

Соответствует корневому тегу-элементу документа. Для HTML это, соответственно, [`<html>`](/html/html/), для SVG — [`<svg>`](/html/svg/). Специфичность селектора `:root` выше, чем у селектора по тегу.
...
Селектор `:valid` соответствует `<input>`, [`<fieldset>`](/html/fieldset/) или `<form>`-элементу, контент которого валиден в соответствии с типом поля или полей внутри. Обратный эффект у `:invalid` — сработает при ошибке HTML-валидации.
