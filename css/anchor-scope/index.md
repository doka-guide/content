...
Если значение `anchor-scope` будет равно `none`, то ни одно якорное имя не будет ограничено поддеревом текущего элемента и до них может будет "достучаться" из других частей DOM-дерева.

![Схема, объясняющая работу anchor-scope: none](images/anchor-scope-none.png)

### `<anchor-name>`

Если в качестве значения указать конкретные якорные имена, например, `--a, --b`, то только они будут ограничены поддеревом текущего элемента.

![Схема, объясняющая работу anchor-scope: --a, --b](images/anchor-scope-a-b.png)

### `all`

Если выбрать значение `all`, то все якорные имена будет видны только в рамках дерева текущего элемента.

![Схема, объясняющая работу anchor-scope: all](images/anchor-scope-all.png)
...
Чтобы решить подобную проблему и было придумано свойство `anchor-scope`, которое позволяет ограничить видимость якорного имени поддеревом конкретного элемента.

Используя свойство `anchor-scope`, починить нашу демку будет проще простого.
