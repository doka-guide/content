...
Увеличиваем значения счётчика `chapter` на элементах [`<h2>`](/html/h1-h6/) и устанавливаем конкретное числовое значение для 4 главы, а так же подставляем значение счётчика `chapter` через функцию [`counter()`](/css/counter-counters/):
