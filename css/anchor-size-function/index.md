...
С помощью функции `anchor-size()` можно получить значения ширины и высоты якорного элемента. Она пригодится, если размеры таргет элемента должны зависить от размеров якоря.

## Как пишется

Функция `anchor-size()` принимает три аргумента:

- `<anchor-name>` — имя якоря, размеры которого нужно получить. Необязятальный аргумент, если его опустить, то функция будет ссылаться на значение указаное в свойстве [`position-anchor`](/css/position-anchor/);
