...
С помощью функции `anchor-size()` можно получить значения ширины и высоты якорного элемента. Она пригодится, если размеры таргет элемента должны зависеть от размеров якоря.
