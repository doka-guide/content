...
💡 Если на форме есть группа связанных радиокнопок (`<input type="radio">`), то, если хотя бы у одной есть [атрибут `required`](/html/form/#атрибуты), псевдокласс `:invalid` будет применён ко всем радиокнопкам сразу.
