...
<iframe title="Демонстрация свойства" src="demos/css-counters-example" height="475"></iframe>

Основные свойства для работы со счётчиками:
- [`counter-reset`](/css/counter-reset) - свойство которое создаёт счётчик;
- [`counter-increment`](/css/counter-increment) - свойство которое увеличивает числовое значения счётчика;
- [`counter-set`](/css/counter-set) - свойство которое устанавливает точное значение счётчика;

Помимо свойств, с помощью которых можно управлять счётчиками, создавать их, есть две основных функции для подставления значения счётчика [`counter()`](/css/counter-counters) и [`counters()`](/css/counters-counters).
...
В качестве значения сначала указывается имя счётчика, а затем его целочисленное значение.
...
Как это работает будет более подробно расписано ниже. Но в конечном итоге пример выше выглядит так:
...
Может показаться, что второй элемент с классом `second` должен иметь значение 10, а не 9, потому что на него применяются сразу два свойства `counter-increment` и `counter-set`, однако при вычислении значения используется определённый [порядок](/css/css-counters/#poryadok-vychisleniya-znacheniya) о котором будет рассказано ниже.
...
В обоих функциях есть два необязательных аргумента, которые указывают стиль счётчика, например, вместо использования десятичной системы счисления - данное значение устанавливается по умолчанию, можно указать строчную римскую нумерацию, то есть тип `lower-roman`:
...
Рассмотрим пример. Создадим счётчик на элементе [`ul`](/html/ul) и проследим как счётчик будет наследоваться элементами, которые находятся внутри.
...
Укажем, чтобы на элементах [`li`](/html/li) с классом `first`, `paragraph`, `second` значение счётчика `new` увеличивалось на 1:
...
Подставим значение счётчика `new` в псевдоэлемент [`:before`](/css/before) для элемента `p` и `::marker` для элементов с классом `first`, `paragraph`, `second` чтобы пронумеровать их:
...
Рассмотрим пример. Создадим элемент [`div`](/html/div) с 3 элементами [`p`](/html/p) внутри и проследим как счётчик будет наследоваться элементами.
...
Теперь у элемента `p` с классом `first` в наборе два счётчика: один унаследованный от родственного элемента `div`, второй собственно созданный счётчик.
...
Получается примерно следующий набор у элемента `ul` с классом `second-list`:
...
Функция `counters()` выводит значения всех счётчиков в наборе:

![Пример работы функции `counters()`](images/9.png)

В то время, как функция `counter()` выводит только последний счётчик с указанным именем.
...
Свойства `counter-reset`, `counter-increment`, `counter-set` не будут действовать на элементах которые не создают блок, например такие элементы у которых свойство [`display`](/css/display) со значением `none` или псевдоэлемент со свойством [`content`](/css/content) у которого значение `none`.
...
🎰 На свойство [`visibility`](/css/visibility) со значением `hidden` данное правило не распространяется.
...
В дополнение к любым явно определённым счётчикам, которые вы указываете в своей таблице стилей, любые элементы со свойством `display` и значением `list-item` автоматически имеют в своём наборе специальный неявный счётчик с именем `list-item`.
...
Если в значении свойства `counter-increment` явно не указывается значение на которое счётчик `list-item` будет увеличиваться от элемента к элементу, то он автоматически увеличивается браузером на 1 для каждого элемента со свойство `display` значением `list-item`.

Именно поэтому, когда вы объявляете список [`ol`](/html/ol) элементы `li` в списке автоматически нумеруются.

Даже если вы попытаетесь указать свойство `counter-increment` со значением `none` на элементе `li` неявный счётчик `list-item` всё равно продолжит нумерацию. Это защита сделана специально.
...
Любые изменения нумерации списка вносимые через HTML также будут учитываться счётчиком.
