...
Также стоит отметить, что на текущий момент данное свойство не поддерживается в браузере Safari.

![Поддержка свойства `counter-set` браузерами](images/12.png)
...
В то время, как функция `counter()` выводит значение только последнего счётчика с указанным именем.
