...
`content` —&nbsp; обязательное свойтсов псевдоэлементов `::before` и `::after`. Позиционируем псевдоэлемент относительно родителя и растягиваем на всю ширину и высоту. После чего задаём полупрозрачный чёрный фон и убираем под текст при помощи `z-index`:
...
Для обоих элементов обязательным является свойство [`content`](/css/doka/content) — содержимое псевдоэлемента. С его помощью можно, например, вставить какое-нибудь слово до или после текста. Без свойства `content` псевдоэлемент не отобразится.
...
Он позволяет стилизовать подсказку, которая выводится в поле ввода текста ([`input`](/html/doka/input)).
