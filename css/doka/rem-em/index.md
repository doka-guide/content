...
Задаёт размер относительно размера шрифта родительского элемента — `em` (от *emphemeral unit* — «относительная единица измерения») или корневого элемента [`<html>`](/html/doka/html) / [`<svg>`](/html/svg) — `rem` (от **root** emphemeral unit — «главная относительная единица»).
