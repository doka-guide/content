...
Универсальный селектор `*` соответствует абсолютно **любому** тегу.
...
Такое CSS-правило применит цвет текста ко всем элементам, вложенных в блок с классом `.preview`:
...
Изменим селектор на вложенный и цвет изменится у всех прямых потомков блок с классом `.preview`:
...
Можно использовать `*` в [смежном селекторе](/css/doka/combined-selectors/#смежные%3A-.element1-%2B-.element2) чтобы правило применялось только в случае, если рядом с элементом есть сосед. Например, это CSS-правило задаст нижний отступ всем элементам при условии, что они не являются единственными детьми своего родителя:
