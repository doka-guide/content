...
Каждому ключевому слову соответствует свой график функции с определенными коэффициентами:
...
Второй параметр определяет логику, по которой будет разделен промежуток на отрезки.
...
ВАЖНО: значения с префиксом `jump-` не поддерживаются в браузерe Internet Explorer. Там работают только значения `start` и `end`, которые являются синонимами значений `jump-start` и `jump-end` соответственно.
