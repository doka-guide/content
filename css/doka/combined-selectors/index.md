...
И тут тоже всё в порядке — [`<h3>`](/html/doka/h1-h6) по-прежнему является прямым потомком [`<article>`](/html/doka/article), потому что располагается на ближайшем уровне вложенности, хоть и следует после `div.article__header`:
...
Код из примера применится только к такому [`<input>`](/html/doka/input), который стоит сразу после [`<label>`](/html/label):
