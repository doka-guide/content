...
Легко применяются к любым элементам, которым можно задать атрибут `disabled`: [`<button>`](/html/doka/button), `<command>`, [`<fieldset>`](/html/doka/fieldset), `<keygen>`, `<optgroup>`, [`<option>`](/html/doka/option), [`<select>`](/html/doka/select), [`<textarea>`](/html/doka/textarea) и [`<input>`](/html/input).
