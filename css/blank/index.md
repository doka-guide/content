...
⚠️ Экспериментальный псевдокласс, который пока не поддерживается ни одним браузером.
...
- Валидность ([`:valid / :invalid`](/css/invalid-valid/), `:user-invalid`) — другая плоскость: поле может быть одновременно `:blank` и `:invalid` (например, [`required`](/css/required/)).
...
💡 Важное уточнение про пробелы: Псевдокласс `:blank` (а также `:empty`) определяет «пустоту» элемента, основываясь на строгом определении пробельных символов из спецификации CSS. К ним относятся только обычный пробел (`U+0020`), табуляция (`U+0009`), перевод строки (`U+000A`), возврат каретки (`U+000D`) и перевод страницы (`U+000C`).
