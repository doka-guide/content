...
При таком подходе объём кода увеличился и читаемость стала значительно ниже. Также прямое использование кастомных свойств не дало возможности масштабирования таких классов.
...
Например, в `JS` и `Python` принято писать нижнее подчёркивание перед самим именем поля/метода:
```js
const someObj = {
  publicValue: 42,
  _privateValue: "42"
