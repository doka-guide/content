...
# Мета: сначала выгрузка фронтматтера, потом сортировка полей. Если выгрузка не
# удалась, сортировать нечего — `frontmatter.js` читает как раз `result.json`.
# Сам `result.json` служебный и в коммит не идёт: его нет среди `paths`.
meta() {
