...
Если результат функции это `false` или другое [falsy](/js/boolean/#vyrazheniya), то колбэк-функция будет вызываться для следующих значений итератора, пока не вернёт истинное значение. В этом случае значение, для которого функция вернула истинный результат, будет возвращено из итератора.

Если попытаться выполнить `Iterator.prototype.filter()` без аргумента или передать не функцию, будет брошена ошибка `TypeError`.

`Iterator.prototype.filter()` возвращает новый итератор.

## Как понять

Метод `Iterator.prototype.filter()` можно сравнить с методом массива [`Array.prototype.filter()`](/js/array-filter/). Оба метода применяются для фильтрации значений с помощью колбэк-функции.
