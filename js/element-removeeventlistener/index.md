...
Так происходит потому что пустой аргумент равен `undefined`, а `undefined` — это falsy значение. Когда браузер ищет обработчик на удаление, он сравнивает опции и видит, что `true !== false`, значит обработчик не будет удалён.
