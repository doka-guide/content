...
Форма будет выглядеть незамысловато и будет работать стандартным образом. Мы будем обрабатывать форму с помощью JavaScript. Чтобы узнать больше о том, как это работает, читайте статьи [«Валидация форм»](/js/form-validation) и [«Работа с формами в JS»](/js/forms).
...
<iframe title="Название — Debounce на примере формы поиска — Дока" src="demos/bespoyasov-BazXpNp/index.html"></iframe>
