...
Ошибки в программах это очень неприятно, но никто не застрахован от их появления. К тому же, ошибки могут появляться в тех ситуациях, которые не зависят от нас самих. Например, пользователь неправильно воспользовался программой. Поэтому в языке нужны конструкции, которые позволяют выполнить произвольный код, но в случае ошибки дать возможность её обработать.
...
Чтобы использовать `try...catch`, нужно в блоке `try` написать код, который нужно исполнить, а в блоке `catch` написать, что делать в случае ошибки.
...
Рассмотрим ситуацию, когда в случае успеха или неудачи выполнения какого-то участка кода нам нужно проводить какие-то действия, чтобы корректно завершить работу скрипта. Например, нужно закрыть [WebSocket](/js/websocket/)-соединение после работы с ним.
...
try {
  // Здесь также код выполнится корректно, потому что
  // установил таймаут без ошибок
  setTimeout(() => {
    throw Error('ошибка')
  }, 1000)
} catch (e) {
...
Однако, если записать асинхронный код в синхронном стиле с помощью [async/await](/js/async-await/), то в этом случае обработку ошибок можно выполнять с помощью `try...catch`.
