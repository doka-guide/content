...
Обратите внимание, что значения параметров `a` и `b` остаются доступны внутри вложенной функции. Эта особенность связана с [контекстом выполнения и лексическим окружением функции](/js/function-context/). Такой подход также активно используется при разработке на JavaScript.
