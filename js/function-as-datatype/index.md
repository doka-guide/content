...
Функция в JavaScript — это [объект](/js/object/), особым свойством которого является возможность вызова. Благодаря этому у неё есть встроенные методы, такие как [`call()`](/js/bind-call-apply/#call).
...
🤓 Функции, которые принимают другие функции в качестве аргументов или возвращают их как результат, называются функциями высшего порядка. Такие функции позволяют писать на JavaScript в [функциональном стиле](/tools/fp/).
...
Обратите внимание, что значения параметров `a` и `b` остаются доступны внутри вложенной функции. Эта особенность связана с [контекстом выполнения](/js/function-context/) и [лексическим окружением функции](/js/closures/#leksicheskoe-okruzhenie). Такой подход также активно используется при разработке на JavaScript.
