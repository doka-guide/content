...
- `window.history.pushState()` добавляет новую запись в истории сессии
- `window.history.replaceState()` изменяет текущую запись в истории сессии
...
Чтобы переместиться по истории вперёд, используется метод `window.history.forward()`. Он работает аналогично кнопке вперёд в браузере.
...
Метод `window.history.go()` является универсальным методом, чтобы переместиться по истории вперёд или назад относительно текущей страницы.
