...
💔 Такой код — плохой. Если в одном из последних `then` произойдёт ошибка, то вся дальейшая цепочка не отработает, при этом, из-за асинхронной природы промиса, прочий код вне промиса продолжит работать и приложение не упадет.
