...
У промиса есть методы `then()` и `catch()`, которые позволяют использовать результат работы промиса, и метод `finally()` для выполнения кода вне зависимости от результата.
