...
1. **Выполнение задач или кода в стеке:** прежде всего цикл событий ждёт, пока стек вызовов полностью освободится от текущих функций и задач.
...
3. **Выбор задачи из очереди:** специальный механизм в браузере определяет, какую задачу из очереди следует выполнить следующей. В этом процессе учитывается тип задачи и её приоритет. Говоря выполнить следующей, имеется в виду переместить в стек, где её обработает движок JavaScript.
...
4. **Повторение цикла:** этот процесс повторяется циклически. Цикл событий постоянно следит за стеком вызовов и очередью событий, обеспечивая непрерывное и эффективное выполнение задач.
...
**Асинхронность JavaScript-кода (псевдоасинхронность)**. Как вы думаете, что произойдёт, если возьмём функцию, которая выполняет цикл от 0 до 1 000 000 000, и завернём её в промис? Правильно, браузер зависнет. Почему? Дело в том, что, оборачивая код в промис, мы только просим браузер отложить выполнение этого кода на момент, пока очередь не будет обработана. Оборачивание синхронного кода в `Promise` или его запуск через `queueMickotask` помещают его _почти_ в конец очереди. Почему почти? С этим разберёмся чуть позже.
...
Даже если задержка `setTimeout` равна нулю миллисекунд, колбэк не выполняется немедленно. Он будет запланирован как макрозадача и выполнится в конце приоритета выполнения.
...
Давайте разберём вывод в консоли. Каждый `setTimeout` вызывает `rAF` и тяжёлую задачу, блокирующую цикл событий. Как только браузер понимает что накопилось достаточно `rAF` для выполнения рендеринга, он пытается применить изменения, которые были запрошены через `rAF` из `setTimeout`. Судя по логу, у браузера получилось выполнить 15–20 микрозадач перед тем, как выполнить запросы `rAF`. В среднем браузер планировал накопившиеся вызовы `rAF` каждые 100 миллисекунд.

Какой вывод можно сделать? Во-первых, браузер стремится выполнить rAF в ближайший кадр (каждые 17 миллисекунд). Во-вторых, браузер планирует как можно больше задач перед следующими вызовами rAF и старается положить в один кадр как можно больше вызовов rAF, инициированные макрозадачами.
...
Можно заметить, все вызовы rAF выполнились после микрозадач, и у них одинаковый `timestamp`. Это говорит о том, что **в нашем случае** браузер запланировал анимацию (сопутствующие изменения в DOM) в ближайший цикл обновления страницы.
...
В этой статье мы рассмотрели различия макро- и микрозадач, а также слегка коснулись рендеринга страницы. Понимание разницы между задачами и их приоритетности помогает создавать более производительные, отзывчивые и надёжные веб-приложения, а ещё эффективно управлять асинхронными операциями и событиями.
