...
`setTimeout` позволяет исполнить функцию через заданный промежуток времени. Метод возвращает числовой идентификатор установленного таймера. Этот идентификатор можно передать в метод `clearTimeout`, чтобы остановить таймер.
