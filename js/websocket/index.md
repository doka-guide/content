...
Если отправлять сообщения быстрее, чем браузер успевает передать их по сети, очередь может переполнится. В этом случае браузер закроет соединение, однако метод [`send()`](#send) не вызовет ошибку.
