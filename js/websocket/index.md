...
Если отправлять сообщения быстрее, чем браузер успевает передать их по сети, очередь переполнится, и браузер закроет соединение. [`send()`](#send) не выбросит исключение. Узнать о закрытии можно только по событию `close`.
