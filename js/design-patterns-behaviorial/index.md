...
Стратегия (англ. strategy) позволяет менять алгоритм работы в зависимости от ситуации.
...
Также мы можем поменять стратегию в рантайме, если пользователь, например, слез с велосипеда и пошёл пешком. Метод для построения маршрута останется тем же:
...
Вместе с цепочкой ответственности и перебором можно использовать Null-object для обработки случая, когда подходящий обработчик не бы найден.
...
В случае с цепочкой чаще всего он уже реализован в абстрактном классе обработчика. В случае с перебором используется в `default` кейсе.
...
Самый распространённый пример «почти-команды» — это [экшен в Redux](/js/architecture-data-flow). Экшен — это объект, который содержит название и данные:
...
Наблюдатель или обзёрвер (англ. observer) — шаблон, который создаёт механизм подписки, когда некоторые сущности могут реагировать на поведение других.
