...
В некоторых случаях было бы удобнее иметь возможность изменять состояния промиса после его создания. Для этого потребуется извлечение функций `resolve()` и  `reject()` из области видимости функции-исполнителя.

Рассмотрим пример. Представим, что нам необходимо обрабатывать события как промисы. Создадим функцию `addEventPromise`, которая добавляет слушателя для однократной обработки события и возвращает промис. Когда событие произойдёт, промис перейдёт в состояние выполнено (fulfilled), с результатом в виде объекта события `event`.
...
Обратите внимание как используется функция `resolve()`. Доступ к ней есть только при создании промиса, а вызываться она должна в обработчике события. Поэтому приходится использовать временную переменную `resolverFn`.

`Promise.withResolvers()` позволяет упростить этот код:
