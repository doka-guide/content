...
`Promise.withResolvers()` — это статический метод объекта Promise. Метод `Promise.withResolvers()` возвращает объект, содержащий новый промис и колбэк-функции изменения его состояния: `resolve()` и `reject()`. Доступ к колбэк-функциям упрощает разделение логики создания и управления состоянием промиса.
...
`Promise.withResolvers()` не имеет аргументов и возвращает объект, содержащий:
