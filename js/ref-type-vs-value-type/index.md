...
Таким образом создаётся новая сущность, которая содержит такие же значения. Любые изменения в новом объекте уже не затронут предыдущий объект.
...
С копированием массивов по ссылке ситуация такая же — если изменить содержимое, то изменения отразятся на всех владельцев ссылки. Для неизменяемого копирования массивов, кроме оператора троеточия, можно использовать методы массива `slice()`, [`map()`](/js/array-map/) и [`filter()`](/js/array-filter/).
