...
Причиной такого поведения является существование у `let` и `const` так называемой _temporal dead zone_ (TDZ) - часть скрипта до инициализации данной переменной. _TDZ_ есть так же и у `es6` классов, не смотря на то, что те в свою очередь являются «синтаксическим сахаром» над обычными функциями. А функции (объявленные как _Function Declaration_) _TDZ_ не имеют.
