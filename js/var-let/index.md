...
Во время выполнения функции формируется ещё одна, локальная/функциональная область видимости, в которой тоже присутствует переменная `a` равная `undefined`, так как присвоения значения еще не произошло

![код со всплытием переменных](images/scopes-local-variables.png)

Обращение к локальной переменной `a` и ее значение к тому моменту уже равно 10.

![код со всплытием переменных](images/scopes-local-variables-initialized.png)

После выполнения функции локальная область видимости была удалена. В консоле выводится глобальная переменная `a`.
