...
2. Если тип `x` и `y` Number, то:
  1. Если значение `x` **NaN** и значение `y` **NaN**, возвращаем **true**.
  2. Если значение `x` **-0**, а значение `y` **+0**, возвращаем **true**.
  3. Если значение `x` **+0**, а значение `y` **-0**, возвращаем **true**.
  4. Возвращаем **true**, если значение `x` равно значению `y`. В противном случае возвращаем **false**.
...
6. Если тип `x` и `y` String, то возвращаем **true**, если `x` и `y` одинаковые последовательности символов (одинаковая длина и такие же коды символов на соответствующих индексах). В противном случае возвращаем **false**.
...
8. Если тип `x` и `y` [Symbol](/js/symbol/), то возвращаем **true**, если `x` и `y` являются одним и тем же значением символа. В противном случае возвращаем **false**.
9. Если типы `x` и `y` наследуются от Object, то возвращаем **true**, если `x` и `y` [ссылаются на один и тот же объект](/js/ref-type-vs-value-type/). В противном случае возвращаем **false**.
</details>
