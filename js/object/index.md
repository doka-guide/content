...
Если значения, которые необходимо добавить в объект как свойства уже хранятся в переменных, достаточно указать их имена:

```js
const name = 'Ушуая'
const country = 'Аргентина'
const coordinates = ['54°48′26″S', '68°18′29″W']
...
Есть ещё один способ создать объект — с помощью статического метода `Object.create()`:
...
`Object.create()` позволяет указать прототип создаваемого объекта и определить свойства и их [конфигурацию](/js/descriptors/).

<aside>

☝️ C помощью `Object.create()` можно создать объект у которого не будет прототипа, а точнее прототипом будет null. У такого объекта не будет никаких наследуемых свойств, например попытка вызвать `toString()` приведёт к ошибке.

Объект с null-прототипом может понадобиться если:

- объект используется как словарь , где все свойства добавлены пользователем;
- необходима защита от изменения свойств прототипа;
