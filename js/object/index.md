...
Если значения, которые нужно добавить в объект как свойства, уже хранятся в переменных, достаточно указать их имена:
...
☝️ С помощью `Object.create()` можно создать объект, у которого не будет прототипа, а точнее прототипом будет null. У такого объекта не будет никаких наследуемых свойств, например, попытка вызвать `toString()` приведёт к ошибке.

Объект с null-прототипом может понадобиться, если:

- объект используется как словарь, где все свойства добавлены пользователем;
- нужна защита от изменения свойств прототипа;
