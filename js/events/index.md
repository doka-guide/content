...
Важным аспектом событийной модели является механизм _распространение событий (event propagation)_, который определяет как события взаимодействуют с узлами DOM при достижении целевого элемента.

Возникая, событие проходит через все родительские элементы (сapturing phase), достигает целевой элемент (target phase) и затем вновь поднимается по иерархии родительских элементов (bubbling phase).

![Событийная модель](images/event-model.png)
...
Как было показано выше, при всплытии, очерёдность обработки события направлена от дочерних элементов к родительским. Это не позволяет  элементу иметь полный контроль над событиями дочерних элементов. Например, обработчик привязанный к элементу может "узнать" о произошедшем событии дочерних элементов только если всплытие не было остановлено в одном из дочерних элементов.

Для решения некоторых задач требуется поменять порядок обработки событий. Родительский элемент должен уметь отреагировать на событие ещё до того как оно будет получено и обработано дочерним элементом. Например, это необходимо при создании элементов-"обёрток", не зависящих от реализации логики дочерних элементов.

В качестве иллюстрации этого подхода рассмотрим предыдущий пример с котиком в коробке. Допустим, требуется воспроизводить видео только в том случае если фон родительского элемента салатовый или голубой, при этом не меняя логику дочернего элемента.

Нам нужно только немного изменить обработчик клика по контейнеру:
...
container.addEventListener('click', function(event) {
  ...
  // Если индекс цвета не соответсвует условию (зелёный или голубой ),
  // останавливаем распространение события
  if (randomColorIndex !== 0 && randomColorIndex !== 2 ) {
        event.stopPropagation()
  }
}, true) // Событие будет обработано на стадии захвата
```

<iframe title="Захват событий" src="demos/capturing/" height="460"></iframe>

Что бы обработать событие на стадии захвата необходимо добавить `true` в качестве третьего параметра в вызов [`addEventListener()`](/js/element-addeventlistener/#kak-pishetsya):
...
element.addEventListener(
  ...
  true // Регистрация обработчика для срабатывания на стадии захвата
)  //
```

Теперь событие будет обработано сначала родительским элементом `<div>`, а затем если не будет остановлено, будет доступно целевому элементу `<video>`.

☝️ Обратие внимание, так как обработка события происходит на стадии захвата метод `stopPropagation()` останавливает распространение события от родительского элемента к дочерним и целевой элемент его не получает.

А вот иллюстрация, распространения события на стадии захвата.
Если кликнуть по блокам на демо, то можно увидеть, как событие достигает целевой элемент:
