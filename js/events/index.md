...
Возникая, событие проходит несколько стадий, основными из которых являются: захват (сapturing) и всплытие (bubbling).

![Событийная модель](images/event-model.png)

Обычно события обрабатывают на стадии всплытия.
...
Что бы обработать событие на стадии захвата необходимо добавить `true` в качестве третьего параметра в вызов `addEventListener()`:
...
Теперь событие будет обработано сначала родительским элементом `<div>`, а затем если не будет остановлено, будет доступно дочернему элементу `<video>`.

А вот иллюстрация, распространение события на стадии захвата.
