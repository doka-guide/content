...
Как было показано выше, при всплытии события, очерёдность обработки события направлена от дочерних элементов к родительским. Это не позволяет  элементу иметь полный контроль над событиями дочерних элементов. Например, обработчик привязанный к элементу может "узнать" о произшедшем событии дочернего элемента только если всплытие не было остановлено в дочернем элементе.
...
В качестве иллюстрации этого подхода рассмотрим предыдущий пример с котиком в коробке. Допустим, требуется воспроизводить видео только в том случае если фон родительского элемента зелёный или голубой, при этом не меняя логику дочернего элемента.
