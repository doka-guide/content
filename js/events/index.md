...
Возникая, событие проходит через все родительские элементы (сapturing phase), достигает целевого элемента (target phase), и затем вновь поднимается по иерархии родительских элементов (bubbling phase).
...
Для решения некоторых задач требуется менять порядок обработки событий. Родительский элемент должен уметь отреагировать на событие ещё до того, как оно будет получено и обработано дочерним элементом. Например, это необходимо при создании элементов-«обёрток», не зависящих от реализации логики дочерних элементов.
...
Чтобы обработать событие на стадии захвата необходимо добавить `true` в качестве третьего параметра в вызов [`addEventListener()`](/js/element-addeventlistener/#kak-pishetsya):
...
☝️ Обратите внимание, так как обработка события происходит на стадии захвата, метод `stopPropagation()` останавливает распространение события от родительского элемента к дочерним и целевой элемент его не получает.

А вот иллюстрация распространения события на стадии захвата. Если кликнуть по блокам на демо, то можно увидеть, как событие достигает целевого элемента:
