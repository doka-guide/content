...
Если итератор не является наследником глобального объекта `Iterator`, метод `take()` можно вызвать через [`call()`](/js/bind-call-apply/#call). Это пригодится для итераторов, реализованных вручную.
...
Вызвать `take()` можно с помощью [`call()`](/js/bind-call-apply/#call):
