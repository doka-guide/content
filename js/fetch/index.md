...
С помощью глобальной функции `fetch()` можно выполнять [HTTP-запросы]((/tools/http-protocol/)) — как получать, так и отправлять данные. Функция возвращает [промис](/js/promise/), которые резолвится в объект ответа, где находится дополнительная информация (статус ответа, [заголовки](/tools/http-protocol/#ispolzovanie-zagolovkov)) и ответ на запрос.
...
Объект Request помогает разделять логику формирования и выполнения запросов, позволяя динамически формировать и изменить параметры запроса.
