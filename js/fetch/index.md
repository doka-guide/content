...
В большинстве случаев ошибки, возникающие при выполнении `fetch()`, можно обработать с помощью `.catch()`, потому что они асинхронные: промис переходит в состояние _rejected_. При этом при подготовке данных запроса может возникнуть и синхронная ошибка, например:
