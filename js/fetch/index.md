...
В большинстве случаев ошибки, возникающие при выполнении `fetch()` асинхронные: промис переходит в состояние _rejected_. Такие ошибки можно обработать с помощью `.catch()`. При этом при подготовке данных запроса может возникнуть и синхронная ошибка, например:
