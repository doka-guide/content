...
Далее выполняется вызов консоли `console.log("Bye!")`. В WebAPI находится функция `setTimeout(greet)`. Она будет там до тех пор, пока не пройдет 2 секунды:
...
`main` отработал, стек пуст. 2 секунды еще не прошло, поэтому `setTimeout(greet)` все ещё в WebApi:
