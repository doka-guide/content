...
Далее выполняется вызов консоли `console.log("Bye!")`, в очереди задач в это время всё ещё находится функция `setTimeout(greet)`. Она будет там до тех пор, пока не истечёт 2 секунды:
...
Отработал `console.log`, заканчивается работа `main`. Предположим, что 2 секунды уже прошло - 'greet' перемещается в Очередь задач:
...
Теперь цикл событий проталкивает функцию `greet` из списка задач в вызов:
