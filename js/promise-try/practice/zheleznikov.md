...
`Promise.try()` позволяет использовать переданную функцию как асинхронную. Результат выполнения можно единообразно обработать как промис, даже если переданная функция синхронная.