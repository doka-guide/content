...
Объект, созданный с помощью функции `makeIterator()` из примера в начале статьи, не является итерируемым и не имеет доступа к методам `Iterator.prototype`.

Массив или Set-множество являются итерируемыми объектами, но не считаются итераторами, так как не содержат метода 'next()' и не имеет доступа к методам `Iterator.prototype`.

Встроенные объекты-итераторы (например `['a', 'b', 'c'].values()`) обладают всеми перечисленными признаками.
