...
- `isTrusted` — указывает на происхождение события, будет в значении `true`, если событие инициировано браузером в ответ на действие пользователя или вызов метода элемента, например, `element.focus()`. `false` — если событие инициировано с помощью [`dispatchEvent()`](/js/dispatch-event/).
