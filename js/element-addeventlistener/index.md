...
Если добавить один и тот же обработчик несколько раз с одинаковыми параметрами `capture` — новые копии не добавятся. Сработает только первый добавленный обработчик, остальные удалять не нужно.
...
Когда вы слушаете события, связанные с возможностью прокрутки ([`touchmove`](/js/element-touch/), [`touchstart`](/js/element-touch/), [`wheel`](/js/element-wheel/) и `mousewheel`), браузер не знает, вызовете ли вы [`preventDefault()`](/js/event-prevent-default/) внутри обработчика.

Поэтому браузер ждёт выполнения вашего обработчика, чтобы понять:

- Нужно ли останавливать прокрутку (если вызван [`preventDefault()`](/js/event-prevent-default/))?
- Или разрешить прокрутку?

Результат: задержка при прокрутке, особенно на мобильных устройствах.

Флаг `{ passive: true }` решает эту проблему. Вы говорите браузеру: «Обещаю, я не буду вызывать [`preventDefault()`](/js/event-prevent-default/) внутри этого обработчика». Получив такое обещание, браузер больше не ждет и начинает прокручивать немедленно, параллельно выполняя ваш код.
