...
// - callback, функция, которую надо вызывать (функция обратного вызова);
// - ms, интервал в мс, в течение которого следует пропускать вызовы.
function throttle(callback, ms) {
  // Флаг isThrottled будет определять,
  // надо ли нам пропускать текущий вызов.
  // savedArgs аргументы для вызова которые должны выполниться в конце если проигнорированный вызов является последним
  ...
  return (...args) => {
    ...
    // а так же сохранить аргументы последней пропущенной функции
    if (isThrottled) {
      savedArgs = args
      return
    }

    // Иначе вызываем функцию с аргументами и меняем флаг на true
    callback(...args)
    isThrottled = true

    setTimeout(() => {
      // по окончанию тайм аута меням снова флаг на false
      isThrottled = false

      // Делаем проверку на наличие сохраненных аргументов 
      if (savedArgs) {
        // Передаем сохраненные аргументы и вызываем функцию:
        callback(...savedArgs)

        // Чистим сохраненные аргументы:
        savedArgs = null
      }
    }, ms)
  }
