...
Важно: В этой реализации `throttle` первый вызов переданной функции (`callee`) происходит **не сразу**, а **только после задержки**, заданной параметром `timeout`.

Если вам нужно, чтобы функция **выполнилась немедленно при первом вызове**, но затем работала с задержками, используйте модифицированную версию `throttle`, где первый вызов выполняется без таймера.
