...
Метод `Array.prototype.concat()` можно вызвать и для значений других типов (например: строка, число, объект). Если `this` является значением примитивного типа (строка, число, `true` или `false`), то оно преобразуется в объект.
