...
Метод `concat()` обычно используется когда необходимо получить новый массив путём объединения нескольких массивов. В отличии от метода `splice()`, который может использоваться для добавления элементов к исходному массиву, метод `concat()` создаёт новый массив и не изменяет исходный.

Метод `concat()` позволяет объединять с исходным массивов не только другие массивы, но также и отдельные элементы.
