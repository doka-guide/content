...
Так как генератор это еще и **итерируемый объект**, то можно использовать его в цикле `for..of`.
...
<aside>
  ...
  ☝️ Оператор `yield` можно использовать исключительно внутри функции-генератора. То есть в колбеках использование `yield` приведëт к синтаксической ошибке.
