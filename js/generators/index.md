...
Генератор – это специальная функция, которая возвращает [объект-генератор](/js/iterator/). Объект-генератор это расширенная версия объекта-итератора. Можно сказать, что генератор – это функция, которая создаёт итератор.
...
Метод `return()` закрывает генератор и передаёт в поле `value` значение аргумента, переданного в `return()`.
