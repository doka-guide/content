...
Спецификация ECMAScript определяет следующий порядок обхода :
Внутри каждого компонента цепочки прототипов все неотрицательные целочисленные ключи (те, которые могут быть индексами массива) будут пройдены сначала в порядке возрастания значения, а затем другие строковые ключи в возрастающем хронологическом порядке создания свойства
