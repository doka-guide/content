...
При копировании объекта, его примитивные свойства (числа, строки, булевы значения) дублируются, а вложенные объекты (nested properties) остаются общими с оригиналом в виде ссылок. Этот тип копирования называется _поверхностным_ (shallow). Например, если объект содержит поле с массивом, то при поверхностном копировании это поле будет ссылаться на тот же массив, что и в оригинале.
...
Во многих случаях предпочтительным будет применить глобальную функцию [`structuredClone()`](https://html.spec.whatwg.org/multipage/structured-data.html#dom-structuredclone). Она не описывается спецификацией ECMAScript (и поэтому не является частью языка), но доступна в браузерах благодаря Web API, а также реализована в Node.js и в других средах исполнения кода.
