...
💡 Метод `map()` не может использоваться для первоначальной установки значения элемента. Функция-колбэк будет вызвана толька для элементов имеющих значение.

Создадим массив и попытаемся использовать метод `map()` для установки в качестве значения элемента его индекс. В результате массив `initedArray` останется таким же пустым как и исходный массив `array`:
...
const initedArray = array.map(function (item, index) {
...
💡 Для наполнения массива одинаковыми значениями можно воспользоваться рецептом [Создание массива из большого количества повторяющихся элементов](/recipes/array-filling/).
