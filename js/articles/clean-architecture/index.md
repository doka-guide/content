...
Трёхслойная архитектура (или чистая архитектура) предполагает разделение кода приложения на «слои» с чётко разделёнными обязанностями между каждым из слоёв.

В этой статье мы рассмотрим:

- какие слои архитектура предполагает;
- по каким признакам модуль должен быть отнесён к одному из слоёв;
...
Трёхслойная архитектура подразумевает разделение кода, как это отмечено в названии, на 3 слоя:

- домен;
- прикладной слой;
- слой адаптеров и портов.

Обычно на схемах слои вкладывают друг в друга подобным образом:
...
В доменном слое находятся код и данные из _предметной области_ приложения. Код доменного слоя — это самое важное, что отличает одно приложение от другого.

Например, в интернет-магазине доменным слоем (или просто доменом) могут быть сущности пользователя, товара, корзины, заказа, а также код для обработки этих данных.

![Внутри домена будут данные и код для обработки пользователя, товара, корзины](images/domain.png)

На схеме доменный слой расположен в центре, потому что именно он определяет, как будет организован остальной код приложения.

- Домен определяет, в каком виде ему необходимо передать данные из внешнего мира;
...
Типы, которые описывают структуры данных домена, называют доменными типами. В JavaScript есть механизмы для описания структур данных: можно использовать объекты, массивы и встроенные примитивы.
...
А вот для описания _типов данных_ JS может предложить разве что классы.
...
Но, конечно, удобнее для описания типов использовать TypeScript:
...
Как правило, это обработчики команд, которые содержат знание о том, что именно надо сделать, когда вызвана какая-то команда. Кроме этого слой содержит _интерфейсы_ портов и адаптеров.
...
Прикладной слой реализует собой [функциональную архитектуру](https://blog.ploeh.dk/2020/03/02/impureim-sandwich/), где получением и отправкой данных занимаются функции с сайд-эффектами, а преобразованием данных занимаются чистые функции.
...
Во фронтенде порт — чаще всего интерфейс пользователя. Обработка пользовательских событий, перерисовка экрана — это всё задачи как раз для слоя адаптеров и портов.

![Слой портов и адаптеров связывает внешний мир и наше приложение](images/adapters.png)

Если мы пишем какой-то консольный сервис или сервер API, то портами в этих случаях были бы соответственно консоль и [эндпоинты API](/js/articles/api/).
...
Под инфраструктурой понимают код, который соединяет приложение с базой данных, внешними сервисами типа отправки SMS и прочего. В случае с фронтендом инфраструктурой мы можем считать сервер API и сторонние сервисы, например, сервис оплаты.

Shared Kernel — это код, который доступен любому модулю, но зависимость от которого не повыашет их [зацепление](<https://ru.wikipedia.org/wiki/Зацепление_(программирование)>). В фронтенде к shared kernel можно отнести, например, глобально доступные объекты типа `window`. В каких-то случаях к нему относят и сам язык программирования.

Иногда shared kernel и слои могут переходить друг в друга — это зависит от глубины проектирования. Если наш код изоморфный и запускается ещё и на сервере, то от глобальных объектов зависеть напрямую уже нельзя, и надо использовать адаптеры.
...
Справа — _управляемые адаптеры_ (driven adapters). Эти адаптеры получают сигналы от нашего приложения, оно говорит им, что надо сделать. Управляемые адаптеры _реализуют интерфейсы портов прикладного слоя_, становятся обёрткой над внешними сервисами, адаптируя их под нужды приложения.

Внешние сервисы за управляемыми адаптерами называют инфраструктурой. В бэкенд-приложениях инфраструктурой могут быть сервисы отправки писем, рассылки SMS, база данных и т. д. Во фронтенде чаще всего инфраструктура — это бэкенд-сервер и сторонние API типа систем оплаты.
...
- Трёхслойная архитектура требует времени на проектирование и реализацию.
