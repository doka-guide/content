...
`DataView` помогает работать с содержимым [`ArrayBuffer`](/js/array-buffer/). С его помощью можно читать и записывать данные разных типов, а также [указывать порядок байтов](/js/data-view/#ukazanie-poryadka-baytov).
