---
title: "`DataView`"
description: "Читаем и записываем байты в буфере как хочется"
authors:
  - marss-hub
keywords:
  - typed array
...
Его особенность — возможность указать порядок байтов при чтении и записи. Это позволяет работать с данными, у которых порядок байтов отличается от порядка байтов в системе.

Если в буфере хранятся данные разного типа (например, сами данные и служебная информация к ним) или нужно контролировать порядок байтов — `DataView` будет полезен.