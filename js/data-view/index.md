...
`DataView` помогает работать с содержимым [`ArrayBuffer`](/js/array-buffer/). С его помощью можно читать и записывать данные разных типов, а также [указывать порядок интерпретации байтов](#ukazanie-poryadka-baytov).
...
`DataView` позволяет явно указать порядок интерпретации байтов т. к. принимает флаг little-endian при чтении или записи. В обычных типизированных массивах такой возможности нет. Это особенно важно, когда вы читаете данные из внешнего источника (например, из файла или по сети), который использует порядок интерпретации байтов, отличный от порядка, принятого в вашей операционной системе. Например, в сетевых протоколах часто используется big-endian, а в операционных системах персональных компьютеров — little-endian и если не указать правильный порядок, числа будут прочитаны неверно. `DataView` позволяет легко подстроиться под нужный формат.
...
<details>
  ...
  Вот пример, как с помощью `DataView` можно узнать порядок интерпретации байтов в используемом окружении:
...
Его особенность — возможность указать порядок интерпретации байтов при чтении и записи. Это позволяет работать с данными, у которых порядок интерпретации байтов отличается от порядка интерпретации байтов в системе. А ещё `DataView` полезен, если в буфере хранятся данные разного типа (например, сами данные и служебная информация к ним).
