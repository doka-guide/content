...
Функциональное программирование — одна из парадигм, которые помогают спроектировать программу так, чтобы она верно отражала эти правила и была устойчивой к изменениям. Прочитайте [обзор парадигм программирования](/js/programming-paradigms/), чтобы узнать про альтернативные подходы и разницу в декларативном и императивном стилях.

## Функция как юнит композиции

Любая система состоит из частей. Программы — тоже системы со своими целями и средствами достижения этих целей. Сопоставление разных частей для получения системы называется _композицией_, а эти части — _юнитами композиции_.
...
- Границы должны быть проведены так, чтобы юнит решал только одну проблему, а не несколько (принцип [разделения ответственности](https://ru.wikipedia.org/wiki/Разделение_ответственности)).
...
В функциональном программировании юнит композиции — это функция. А вызов нескольких функций подряд — это и есть их композиция. Например, если мы хотим к числу 10 прибавить 5, а потом умножить результат на 42, то последовательно вызовем функции `add` и `multiply`:
...
Если разбить весь процесс на несколько шагов, то сперва мы получаем результат сложения, а затем его передаём как аргумент умножения:
...
Такая последовательная передача данных на вход следующей функции и есть простейшая композиция функций.
...
Например, `pureFn` при вводе 10 и 20 всегда будет возвращать 15, поэтому она чистая:
...
Оба способа берут начало в математике. Рекурсия помогает даже выразить некоторые задачи в том виде, в котором они формулируются в математике, например, вычисление факториала:
...
Иногда нам попадаются задачи, которые концептуально похожи, но отличаются деталями. Например, нам может быть нужно отфильтровать из массива только положительные числа или только чётные числа. Мы могли бы написать нечто вроде:
...
Здесь новая функция `filter`, которая непосредственно перебирает значения, принимает на вход кроме массива другую функцию, которая проверяет каждое значение массива по своему условию.

Давайте реализуем `filter` самостоятельно, чтобы понять, как он работает. Объявим функцию `filter`, в которую передадим два аргумента: массив и функцию, проверяющую условие — предикат.
...
Таким образом мы _абстрагируемся_ от деталей проверки каждого элемента. Вместо того, чтобы писать несколько почти одинаковых фильтров для массивов мы написали один фильтр и несколько условий. Условия мы можем передавать в `filter` как аргументы, чтобы `filter` их использовал для проверки каждого из элементов.
...
Хорошо, мы научились абстрагировать похожие задачи с разными аргументами. А что делать, если нам надо «запомнить» часть аргументов перед выполнением?

Например, у нас есть функция умножения `multiply`, но мы хотим дополнительно создать ещё и удваиватель `double`, потому что будем использовать его чаще остального умножения. Решением в лоб было бы просто написать ещё одну функцию:
...
Тогда создать удваиватель мы сможем, написав:
...
Таким же образом мы можем создать и утраиватель и умножатель на 10:
...
Поэтому чаще оригинальную функцию переделывают не руками, а каррируют.

### Каррирование

_Каррирование_ – это трансформация функций таким образом, чтобы они принимали аргументы не как `f(a, b, c)`, а как `f(a)(b)(c)`. То есть буквально то, что мы сделали с функцией `multiply`, только автоматизированно.
...
В примере выше мы проверяем, закончились ли аргументы. Если закончились, то передаём их все в оригинальную функцию и вызываем её. Если аргументы ещё есть, то используем [рекурсию](/js/recursion/), чтобы вызывать каррировать ещё раз.
...
С другой стороны, это делает взаимодействие с реальностью несколько затруднительным, потому что реальность _вся_ состоит из побочных эффектов.
...
Получается такой сендвич: сайд-эффект, чистое преобразование, сайд-эффект:

![Почипо-сендвич: побочный эффект, чистое преобразование, побочный эффект](./images/impureim.png)
...
Мы создали функцию `Box`, которая возвращает объект с двумя методами. Первый, `map` принимает функцию-преобразование и возвращает новый контейнер, чтобы к нему тоже можно было применить новое преобразование. Второй, `fold` «разворачивает» контейнер, находит в замыкании значение и возвращает его.
...
Обратите внимание, что `Left` при вызове `map` _не выполняет_ переданную функцию. Также при вызове `fold` мы выполняем только одну из двух переданных функций. Это позволяет разветвлять код и обрабатывать разные случаи и ошибки, не заботясь о каждом этапе обработки ошибок отдельно.
...
Самое классное, что мы можем применять сколько угодно преобразований, и они не вызовут ошибок, так как `Left` не применяет их, а просто возвращает значение:
...
В примере выше если `x` — строка, к нему применится 3 преобразования из `map` и в конце его выведет `alert` на экран. В случае если `x === undefined`, то преобразования будут проигнорированы, а в консоль выведет `"No name found."`.
...
Во многих функциональных языках проверяемое значение можно сопоставлять не только с другими значениями, но можно использовать предикаты, типы данных и другое.
...
В JavaScript тоже можно (с костылями) использовать предикаты в паттерн-матчинге. Мы можем проверять результат выражений в `case`, чтобы:
...
Функциональное программирование в строгом определении — это понимание функции как математического понятия. Функция здесь — это [_отображение_ входных данных на выходные](<https://ru.wikipedia.org/wiki/Функция_(математика)>). Отсюда как раз следует, что у функции не должно быть побочных эффектов — у математических функций их просто нет! У каждого входного значение есть одно и только одно выходное, исключений не бывает.
...
Чистые функции, которые лежат в основе ФП, надёжны в том плане, что _всегда_ выдают одинаковый результат при одинаковых входных данных.

Это значит, что в какой бы момент времени мы ни запускали такую функцию, мы всегда может рассчитывать на предсказуемый результат. Более того, сам вызов чистой функции можно заменить на её значение-результат, и программа не сломается. Это свойство называется [ссылочной прозрачностью](https://ru.wikipedia.org/wiki/Ссылочная_прозрачность).
...
Функциональное программирование запрещает менять состояние, а значит не случится ситуации, когда две функции пытаются записать разные значения в одну переменную. Это значит, что выполнение кода можно разбивать на несколько параллельных потоков или процессов.
...
Так как состояние программы неизменяемо, при его «изменении» приходится создавать его полную копию. Это требует грамотной и своевременной работой с памятью — выделения, мониторинга и очищения неиспользуемых участков памяти.
