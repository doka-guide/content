...
- `domain` — определяет домен, для которого действует кука. Если параметр не указан, то кука будет работать только для текущего домена (без поддоменов). Если домен указан явно, кука будет доступна и для поддоменов;
