...
Любой **PRNG**-алгоритм генерирует числа, которые, в конечном счёте, будут повторяться. Поэтому числа, генерируемые `Math.random()`, можно предугадать. И именно поэтому вместо него предпочтительней использовать `crypto.getRandomValues()` для формирования ключа доступа, потому как последний криптографически устойчив.
