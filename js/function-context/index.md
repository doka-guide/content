...
Обратите внимание, что `this` определятся в момент вызова функции. Если записать метод объекта в переменную и вызвать её, значение `this` изменится.
...
// хотя на первый взгляд разницы нет,
// и работает будто бы правильно.
```
