...
Чтобы узнать, что переход на страницу закончен, необходимо добавить обработчик на глобальное событие `popstate`. Пример использования этого события смотрите в разделе «В работе».
...
Предположим, что мы зашли на страницу `https://doka.guide/js/` с новой вкладки. Добавим новую запись в историю:

```js
console.log(window.history.length)
// Вернёт 1, так как у нас новая вкладка
...
console.log(window.history.length)
// Вернёт 2, так как добавили новую запись
```

Адрес в браузерной сроке изменился и если нажать кнопку перезагрузки страницы, то мы увидим статью про [var и let](/js/var-let), а не страницу раздела.
...
console.log(window.history.length)
// Вернёт 2, так как ничего в историю не добавляли
```
