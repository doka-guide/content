...
- После того, как JavaScript убеждается, в том, что стек пуст — в него добавляются задачи из очереди микрозадач;
- Если в процессе выполнения одной микрозадачи в очередь добавится другая микрозадача, то обе эти задачи выполнятся в этот же проход. Процесс выполнения продолжается до тех пор, пока очередь не опустеет. Как только это произойдёт — выполняется одна задача из очереди макрозадач;
- Сразу после каждой макрозадачи в стек снова добавятся все микрозадачи перед тем, как выполнить следующую макрозадачу, отобразить изменения на странице или сделать что-то ещё.
