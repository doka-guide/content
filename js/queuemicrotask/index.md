...
- Процесс выполнения продолжается до тех пор, пока не станет ясно, что очередь опустела. Как только это произойдёт — выполняется одна задача из очереди макрозадач;
- Сразу после каждой макрозадачи в стек снова добавятся все микрозадачи перед тем, как выполнить следующую макрозадачу или отобразить изменения на странице, или сделать что-то ещё.
