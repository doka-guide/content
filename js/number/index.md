...
Если в математическом выражении есть NaN, то в результате тоже получится NaN. Кроме одного исключения - NaN ** 0 = 1.

Для проверки, что значение в переменной является конечным числом, а не специальным значением, используйте функцию [`Number.isFinite()`](/js/number-isfinite/). Она возвращает `true`, если переданный аргумент — число.
