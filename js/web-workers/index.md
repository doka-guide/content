...
Web Worker — это функциональность JavaScript, которая позволяет выполнять код вне основного потока, при этом не блокируя его. Это означает, что долгие или сложные вычисления в потоке Web Workers не блокируют пользовательский интерфейс.

В основе своей Web Worker - это отдельный скрипт js, который запускается в отдельном потоке браузера и выполняет код параллельно основному потоку браузера. Но это ещё не всё, так же Web Workers имеют свой собственный контекст, отличный от `window`. Если в основном потоке браузера, глобальным объектом является `window`, то в потоке Web Worker это объект `self`. 

Давайте разберёмся что означает “собственный контекст”? Контекст выполнения Web Workers [WorkerGlobalScope](https://html.spec.whatwg.org/multipage/workers.html#workerglobalscope), отличается от контекста выполнения основного потока `window`. Главным отличием, является отсутствие доступа к `document` и недоступность `DOM API` внутри потока Web Worker. Так же стоит заметить что основной поток браузера и поток Web Worker это два изолированных потока, например Chrome в Web Worker запускает отдельный инстанс движка JS. Вся связь между потоками происходит через отправку сообщений, а точее с помощью [postMessage Web API](https://developer.mozilla.org/en-US/docs/Web/API/Worker/postMessage). 

> Через механизм postMessage, из Web Worker, косвенно можно управлять DOM основного потока, но стоит учитывать, что механизм postMessage - асинхронный. 
...
Давайте теперь разберёмся, как нам связать главный поток браузера и поток выполнения Worker. Они общаются между собой с помощью сообщений: 
...
Далее в Web Worker с помощью свойства onmessage, которое находится в глобальном контексте Worker или в `self`, мы записываем туда наш обработчик сообщений из вышестоящего контекста.  Обратите внимание, что postMessage или self.postMessage мы можем вызвать из любого места нашего Worker.
