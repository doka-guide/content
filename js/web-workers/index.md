...
Web Worker — это API, которое позволяет выполнять код вне основного потока. Это означает, что долгие или сложные вычисления на Web Workers не блокируют пользовательский интерфейс.

На Web Worker запускается отдельный файл JavaScript, который выполняется параллельно с основным потоком браузера в своем собственном потоке. Кроме этого Web Workers имеют свой собственный контекст, отличный от `window`. Если в основном потоке браузера, глобальным объектом является `window`, то в потоке Web Worker это объект `self`.
