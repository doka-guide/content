...
Ещё есть методы для операций над двумя множествами сразу: `union()`, `intersection()`, `difference()`, `symmetricDifference()`, `isSubsetOf()`, `isSupersetOf()`, `isDisjointFrom()`. Подробнее в разделе «Операции над множествами».
...
Коллекция `Set` может хранить произвольный набор значений — нет разницы, хранить в коллекции примитивные типы, объекты или массивы. `Set` гарантирует, что одно и то же значение не может попасть в коллекцию больше одного раза. Если нужно получить коллекцию уникальных значений из массива неуникальных, `Set` — один из способов этого достичь.
...
1️⃣ Использовать метод `forEach()`, который работает аналогично одноимённому методу [массива](/js/arrays/):
...
У `Set` есть семь методов, которые работают сразу с двумя множествами и возвращают результат операции над ними — объединение, пересечение и так далее. Каждый метод принимает второе множество и не изменяет исходные:
...
a.union(b)
// Set(6) {1, 2, 3, 4, 5, 6} — все элементы из обоих множеств

a.intersection(b)
// Set(2) {3, 4} — только общие элементы

a.difference(b)
// Set(2) {1, 2} — элементы a, которых нет в b

a.symmetricDifference(b)
// Set(4) {1, 2, 5, 6} — элементы, которые есть только в одном из множеств
```

Ещё три метода не создают новое множество, а возвращают `true` или `false`:

```js
a.isSubsetOf(b)
// false — не все элементы a есть в b

a.isSupersetOf(b)
// false — не все элементы b есть в a

a.isDisjointFrom(b)
// false — у множеств есть общие элементы
```

Поменяйте значения множеств и посмотрите, как меняется результат каждого метода:

<iframe title="Операции над множествами Set" src="demos/set-methods/" height="520"></iframe>
