...
- [`forEach`](/js/set-foreach) — выполнить функцию для каждого элемента в коллекции, аналогично [одноименному методу массива](/js/array-foreach).
...
Коллекция создаётся при помощи [конструктора](/js/set-new).
...
`Set` предоставляет минималистичный набор методов, но их достаточно в большинстве случаев.

Добавляют элемент в `Set` с помощью метода [`add`](/js/set-add), а удаляют с помощью [`delete`](/js/delete). В оба метода передаётся элемент, который нужно добавить или удалить:
...
`Set` — это неиндексированная коллекция. В этой структуре данных нет понятия индекса элемента, поэтому нельзя получить произвольный элемент коллекции. В коллекцию можно только положить значение, а достатать отдельное значение нельзя.
...
1️⃣ Использовать метод [`forEach`](/js/set-foreach), который работает аналогично одноименному методу массива:
