...
Область видимости — абстрактная концепция, определяющая правила, которые должны соблюдаться. Для реализации этих правил в JavaScript используется лексическое окружение (lexical environment). Это особая структура данных в JavaScript, которая отслеживает область видимости переменных и функций в определенной точке кода.

Когда в JavaScript нужно определить новую область видимости, создается лексическое окружение — пространство в памяти для хранения переменных, объявляемых в функции или блоке в процессе выполнения. Так же лексическое окружение хранит ссылку на внешнее/родительское лексическое окружение.

![Схема ссылок лексических окружений на родительское лексическое окружение](images/4.png)

При каждом новом вызове функции или блока создаётся своё лексическое окружение. Если на выделенную область памяти (лексическое окружение) больше нет ссылок - сборщик мусора удаляет её.
...
Теперь в переменной `accessToInner` находится функция `inner`, у которой всё ещё есть доступ к локальной переменной `a` функции `outer`! При этом лексическое окружение, где существует `a`, не удаляется сборщиком мусора, потому что на него есть ссылка в виде функции `accessToInner`.
