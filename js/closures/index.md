...
При каждом новом вызове функции или блока создаётся своё лексическое окружение. Пока к значениям из функции можно обратиться из программы, движок сохраняет их. Когда они становятся недостижимыми, сборщик мусора может освободить связанные ресурсы.

### Замыкание

Замыкание — это функция вместе со ссылками на внешнее окружение, в котором она была создана.
...
Теперь в переменной `accessToInner` находится функция `inner`, у которой всё ещё есть доступ к локальной переменной `a` функции `outer`! При этом лексическое окружение функции, где существует `a`, не освобождается сборщиком мусора, потому что на него есть ссылка в виде функции `accessToInner`.
