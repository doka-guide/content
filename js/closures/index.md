...
Переменная в примере сейчас находится в глобальной области видимости.
...
Глобальный объект `window` — это объект, который даёт доступ к Web API браузера. `window` рекурсивно «содержит сам себя», потому что все глобальные объекты находится в `window`:
...
В этом примере в `case` нам надо выполнить несколько строчек. Удобно обернуть все операции в блок при помощи фигурных скобок — тогда все переменные и операции будут ограничены этим блоком, то есть, блочной областью видимости.
