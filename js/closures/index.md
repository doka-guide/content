...
<details>
  ...
  При создании глобальных переменных с использованием `let` и `const` попадание значений в глобальном объекте `window` не предусмотренно. Это позволяет избежать конфликтов имён и улучшить безопасность.
</details>
...
Чтобы понять, почему доступ к переменной `a` до её объявления не вызывал ошибки, разберёмся, как работает `var` и объявление переменных.
...
«Поднятие» произойдёт даже в том случае если строка, содержащая объявление переменной не будет доступна при обычном выполнении:
...
Может возникнуть вопрос: как механизм «поднятие» переменных связан с темой «Области видимости» ?

При выполнении «поднятия», var-переменная становится доступна (видна) для всего кода своей (функциональной или глобальной) области видимости.
