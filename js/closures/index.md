...
<details>
  ...
  В глобальной области видимости `var`- переменная становится свойством объекта `window`, подробнее об этом можно прочитать в спецификации [ECMAScript](https://tc39.es/ecma262/multipage/executable-code-and-execution-contexts.html#sec-createglobalvarbinding). Для `let` и `const` этого не происходит, они остаются глобальными, но не добавляются в `window`. Это позволяет избежать конфликтов имён и улучшить безопасность кода.

  Важно понимать, что если при создании var-переменной в глобальном объекте `window` уже есть неудаляемое (configurable: false) свойство с тем же именем, новое значение этому свойству назначено не будет.

  Например, создадим глобальную переменную `document` и попытаемся получить её значение из объекта `window`:
...
Область видимости определяет, в какой части программы можно обратиться к переменной, функции или объекту. Для реализации этих правил в JavaScript используется лексическое окружение (lexical environment). Это особая структура данных в JavaScript, которая отслеживает область видимости переменных и функций в определенной точке кода. Так же лексическое окружение хранит ссылку на внешнее/родительское лексическое окружение, поэтому функция может обращаться к внешним переменным.

![Схема ссылок лексических окружений на родительское лексическое окружение](images/4.png)

Каждый вызов функции и каждый новый блок создают собственное лексическое окружение. Пока к значениям из функции можно обратиться из программы, движок сохраняет их. Когда они становятся недостижимыми, сборщик мусора может освободить связанные ресурсы.

### Замыкание

Замыкание — это функция вместе со ссылкой на внешнее окружение, в котором она была создана. Замыкание возникает в момент создания вложенной функции. Функция сохраняет доступ к своему внешнему лексическому окружению и может использовать переменные из него даже после того, как выполнение внешней функции завершилось.

В примере у дочерней функции есть доступ к области видимости родительской функции:
...
Теперь в переменной `accessToInner` находится функция `inner`, у которой всё ещё есть доступ к локальной переменной `a` функции `outer`! При этом лексическое окружение функции `outer` не освобождается сборщиком мусора, потому что на него есть ссылка в виде функции `accessToInner`.
...
На практике замыкания часто используют, чтобы сохранить доступ к значениям из внешней области видимости и не выносить их в глобальную область. Замкнутые переменные никак нельзя изменить, кроме как дочерними функциями.
...
JavaScript автоматически выполняет декларацию (создание) для var-переменной до того, как будет выполнен код, использующий эту переменную. Иными словами, происходит «поднятие» (_hoisting_) объявления переменной наверх функции. Фактически выполняется вот такой код:
...
Может возникнуть вопрос: как механизм «поднятия» связан с темой «Области видимости»? Благодаря «поднятию», var-переменная (со значением `undefined`) становится доступна (видна) для всего кода своей области видимости.

Термин «поднятие» (_hoisting_) включает также возможность выполнить функцию до её объявления с помощью [Function Declaration](/js/function/#kak-pishetsya):
...
В этом случае происходит поднятие значения переменной (_value hoisting_), и функция может быть выполнена в любом месте своей области видимости.
