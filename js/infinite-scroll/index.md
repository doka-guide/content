...
Стратегий борьбы с лишним кодом много: [код-сплиттинг](/js/web-app-works/#код-сплиттинг) и минификация, оптимизация изображений, кэширование, рефакторинг и удаление старого кода.
...
Когда сайты были проще, а [AJAX](/js/web-app-works/#клиент-серверная-архитектура) и [JSON](/js/json/) ещё не были распространены, пагинаторы были единственным способом поделить большое количество контента на куски.
...
Чтобы решить эти проблемы, нужно завести переменные, которые будут следить за [состоянием приложения](/js/web-app-works/#состояние-приложения):
...
Для таких задач можно использовать [History API](/js/bom/#history).
...
<iframe title="Название — Подгрузка контента при прокрутке — Дока" src="demos/bespoyasov-wvzWGMM/index.html"></iframe>
