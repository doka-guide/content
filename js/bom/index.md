...
👎 Изменение заголовка страницы через `pushState()` пока [плохо поддерживается браузерами](https://caniuse.com/mdn-api_history_pushstate). Похожего эффекта можно добиться, используя `pushState()` вместе с `document.title`.
