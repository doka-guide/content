...
## Как пишется
Кейс, где пользователь прокручивает страницу с большим количеством контента. Заглушка подменяется на нужное изображение, когда до него остаётся докрутить ещё 200px.
...
Наблюдатель создаётся конструкцией `new IntersectionObserver(callback, options?)`
...
- **rootMargin** — строка с отступами для области наблюдения. Синтаксис почти такой же, как у CSS-свойства _margin_ — `'0px 0px 0px 0px'` _(top, right, bottom, left)_, так же доступна короткая запись, например `'50px 100p'x` или `'200px'`. По умолчанию — `'0px 0px 0px 0px'`.
...
![Визуализация работы rootMargin](rootMargin.jpg)
...
<details>
  ...
  - `0` — сработает даже при пересечении на `1px`,
  - `1` — сработает только при полном появлении элемента в наблюдаемой области).
  - `[0, 0.5, 1]` — колбэк сработает 3 раза, сначала при попадании хотя бы 1 пикселя элемента в область наблюдения, затем на половине элемента и ещё один раз, когда элемент окажется полностью в области наблюдения.
</details>
...
_Callback_ будет вызываться каждый раз, когда происходит пересечение наблюдаемой области и целевого элемента.
