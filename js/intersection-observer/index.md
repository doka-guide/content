...
_delay_ — задержка перед срабатыванием наблюдателя (в миллисекундах).
...
_scrollMargin_ — строка с отступами, определяющая смещения, добавляемые к контейнеру прокрутки при вычислении пересечений. Синтаксис почти такой же, как у CSS-свойства [`margin`](/css/margin/) — `'0px 0px 0px 0px'` _(top, right, bottom, left)_. Также доступна короткая запись, например, `'50px 100px'` или `'200px'`. По умолчанию — `'0px 0px 0px 0px'`. Если они установлены, то пересечение будет учитывать эти отступы.
...
_trackVisibility_ — логическое значение, указывающее, следует ли наблюдателю отслеживать видимость целевого элемента. Если значение равно `true`, браузер будет учитывать реальную видимость целевого элемента. Например, если элемент перекрыт попапом, колбэк не будет вызван при пересечении с наблюдаемой областью.
Отслеживание видимости требует дополнительных ресурсов, поэтому его следует использовать только при необходимости.
Значение по умолчанию — false. Пересечение определяется только по геометрии. Например, даже если элемент скрыт попапом,  колбэк будет вызван при пересечении с наблюдаемой областью.
